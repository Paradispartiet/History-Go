...
- Added one extremely narrow, local JSDoc/type-only cast in `js/Civication/systems/day/dayChoiceDirector.js` at the existing `state.activeFaction` write site.
...
## Phase 75: narrow civicationMailRuntime inspect state typecheck pass

- Added a narrow, file-local JSDoc/type-only cast in `js/Civication/systems/civicationMailRuntime.js` after the Phase 72 audit / PR #741, Phase 73 / PR #749, and Phase 74 / PR #751.
- Before the change the file held ten diagnostics: `civicationMailRuntime.js(81,31)` (`TS2339`, `window.CivicationMailEngine`), `civicationMailRuntime.js(141,18)` (`TS2339`, `window.DEBUG`), `civicationMailRuntime.js(553,18)` (`TS2339`, `window.DEBUG`), `civicationMailRuntime.js(590,26)` (`TS2551`, `window.CivicationEventEngine`), `civicationMailRuntime.js(627,22)` (`TS2339`, `window.DEBUG`), `civicationMailRuntime.js(681,43)` (`TS2339`, `window.CivicationBrandJobState`), `civicationMailRuntime.js(707,26)` (`TS2551`, `window.CivicationEventEngine`), `civicationMailRuntime.js(717,34)` (`TS2339`, `mail_plan_progress` on `unknown`), `civicationMailRuntime.js(718,27)` (`TS2339`, `mail_system` on `unknown`), and `civicationMailRuntime.js(735,10)` (`TS2339`, `window.CivicationMailRuntime`).
- Cast the existing `getState()` binding inside `inspect()` to the existing-read shape `{ mail_plan_progress?: unknown, mail_system?: unknown } & Record<string, unknown>`, so the existing `state?.[RUNTIME_KEY]`, `state?.mail_plan_progress`, and `state?.mail_system` reads resolve through the local inspect-state shape.
- Removed the local `TS2339` unknown/property-access diagnostics for `mail_plan_progress` and `mail_system` without changing runtime behavior; the existing optional chaining and `|| null` fallbacks are preserved.
- The remaining diagnostics were left standing because they are global/window-shape or API-shape errors for `CivicationMailEngine`, `DEBUG`, `CivicationEventEngine`, `CivicationBrandJobState`, and `CivicationMailRuntime` that require schema/global or broader API-scope work outside this narrow pass.
...
## Phase 77: narrow dayNarrativeConsequencesUI active career typecheck pass

- Added a narrow, file-local JSDoc/type-only cast in `js/Civication/systems/day/dayNarrativeConsequencesUI.js` after the Phase 72 audit / PR #741, Phase 73 / PR #749, Phase 74 / PR #751, Phase 75 / PR #755, and Phase 76 / PR #757.
- Before the change the file held four diagnostics: `dayNarrativeConsequencesUI.js(12,67)` (`TS2339`, `career_id` on `unknown`), `dayNarrativeConsequencesUI.js(168,52)` (`TS2339`, `__civiNarrativeWrapped` on `never`), `dayNarrativeConsequencesUI.js(171,28)` (`TS2339`, `apply` on `never`), and `dayNarrativeConsequencesUI.js(177,5)` (`TS2740`, renderer wrapper assignment shape mismatch against `Window`).
- Cast the existing active-position expression inside `activeCareerId()` to `{ career_id?: unknown } | undefined`, so the existing `.career_id` read resolves through the local active-position shape.
- Removed the local `TS2339` unknown/property-access diagnostic for `career_id` without changing runtime behavior; the existing `normStr(...)`, optional chaining, and returned active career id are preserved.
- The remaining diagnostics were left standing because `__civiNarrativeWrapped`, `apply`, and the wrapper assignment are renderer-wrapper/function-contract diagnostics that require broader wrapper/API-shape work outside this narrow pass.
- Runtime logic, narrative-consequences logic, snapshot logic, delta calculation, DOM injection, renderer patching, event listener, state flow, DOM/CSS/layout/text, and schemas/globals remain unchanged.
- `schemas/civication-globals.d.ts` was not modified; `CivicationCalendar?: any` from Phase 46 was not changed; no declarations were added for `CivicationState`, `CivicationPsyche`, `renderCivicationInbox`, `renderWorkdayPanel`, `CivicationMailEngine`, `CivicationTaskEngine`, `CivicationEventEngine`, `CivicationMailRuntime`, `CivicationLifeMailRuntime`, `CivicationDebateEngine`, `CiviMailPlanBridge`, or other globals.
- AHA PR #635, the place/emne track, the `js/ui/**` track, the `js/psychologyRoom.js` track, and the Lesespor/Leksikon track were not mixed in.

## Phase 78: narrow dayPatches onboarding/career typecheck pass

- Added two narrow, file-local JSDoc/type-only casts in `js/Civication/systems/day/dayPatches.js` after the Phase 72 audit / PR #741 and Phase 73–77 Civication-first passes.
- Before the change the file held fourteen diagnostics: `dayPatches.js(373,24)` (`TS2551`, `window.CivicationEventEngine`), `dayPatches.js(648,28)` and `dayPatches.js(648,51)` (`TS2339`, `getPendingEvent` API shape), `dayPatches.js(687,27)` (`TS2339`, `complete` on `unknown`), `dayPatches.js(688,24)`, `dayPatches.js(735,22)`, and `dayPatches.js(774,20)` (`TS2339`, `onAppOpen` API shape), `dayPatches.js(716,8)`, `dayPatches.js(785,25)`, and `dayPatches.js(920,14)` (`TS2339`, `window.CivicationTaskEngine`), `dayPatches.js(763,56)` (`TS2339`, `career_id` on `unknown`), `dayPatches.js(845,5)` (`TS2322`, patch assignment/function-contract shape), and `dayPatches.js(864,14)` / `dayPatches.js(873,10)` (`TS2339`, `window.__civiDayPhaseUiPatched`).
- Cast the existing onboarding-state expression to `{ complete?: unknown } | null | undefined`, so the existing `onboarding?.complete === true` read resolves through the local onboarding shape.
- Cast the existing active-position expression to `{ career_id?: unknown } | null | undefined`, so the existing `activePosition?.career_id || ""` read resolves through the local active-position shape.
- Removed the two local `TS2339` unknown/property-access diagnostics for `complete` and `career_id` without changing runtime behavior; optional chaining, fallback values, `onAppOpen`, `finalizeWeekIfNeeded`, and all existing day-patch behavior are preserved.
- The remaining diagnostics were left standing because they are global/window-shape, API-shape, prototype/patch, or function-contract diagnostics for `CivicationEventEngine`, `getPendingEvent`, `onAppOpen`, `CivicationTaskEngine`, `__civiDayPhaseUiPatched`, and patch assignment behavior that require schema/global or broader API/runtime-scope work outside this narrow pass.
- Runtime logic, day-patch logic, onboarding logic, calendar phase-flow, weekly summary/finalize logic, task completion, contact creation, event-engine patching, state flow, DOM/CSS/layout/text, and schemas/globals remain unchanged.
- `schemas/civication-globals.d.ts` was not modified; `CivicationCalendar?: any` from Phase 46 was not changed; no declarations were added for `CivicationState`, `CivicationCalendar`, `CivicationJobs`, `CivicationTaskEngine`, `CivicationMailEngine`, `CivicationEventEngine`, `CivicationMailRuntime`, `CivicationChoiceDirector`, `CiviMailPlanBridge`, or other globals.
- AHA PR #635, the place/emne track, the `js/ui/**` track, the `js/psychologyRoom.js` track, and the Lesespor/Leksikon track were not mixed in.

## Phase 80: narrow CivicationSectionsUI summary typecheck pass

- Added three narrow, file-local JSDoc/type-only casts in `js/Civication/ui/CivicationSectionsUI.js` after the Phase 79 audit / PR #773.
- Before the change the file held nine diagnostics: `CivicationSectionsUI.js(106,28)` (`TS2339`, `event` on `unknown`), `CivicationSectionsUI.js(121,17)` (`TS2339`, `title` on `unknown`), `CivicationSectionsUI.js(122,38)` (`TS2339`, `career_name` on `unknown`), `CivicationSectionsUI.js(122,60)` (`TS2339`, `career_id` on `unknown`), `CivicationSectionsUI.js(123,29)` (`TS2339`, `title` on `unknown`), `CivicationSectionsUI.js(134,16)` (`TS2339`, `home` on `unknown`), `CivicationSectionsUI.js(135,23)` (`TS2339`, `home` on `unknown`), `CivicationSectionsUI.js(135,63)` (`TS2339`, `home` on `unknown`), and `CivicationSectionsUI.js(266,10)` (`TS2339`, `window.CivicationSectionsUI`).
- Cast the existing pending-event expression to `{ event?: { subject?: unknown, title?: unknown } } | undefined`, so the existing `pending?.event` read resolves through the local inbox-summary shape.
- Cast the existing active-position expression to `{ title?: unknown, career_name?: unknown, career_id?: unknown } | undefined`, so the existing `active?.title`, `active.career_name`, `active.career_id`, and `active.title` reads resolve through the local role-summary shape.
- Cast the existing home-state expression to `{ home?: { status?: unknown, district?: unknown, level?: unknown } } | undefined`, so the existing `state?.home`, `state.home.district`, and `state.home.level` reads resolve through the local home-summary shape.
- Removed the eight local `TS2339` unknown/property-access diagnostics without changing runtime behavior; the existing `cleanText(...)`, `shortText(...)`, optional chaining, fallback values, and returned summary text are preserved.
- The remaining `TS2339` diagnostic for `window.CivicationSectionsUI` was left standing because it is an export/global-window shape diagnostic that requires schema/global scope outside this narrow pass.
...
- Limited the code pass to `renderPsycheDashboard()`, adding broad local psyche snapshot/trust typedefs for the values the dashboard reads while keeping the rest of `CivicationUI.js` unchanged.
- Cast the active-position and `CivicationPsyche.getSnapshot()` values locally before reading psyche dashboard scores and trust/collapse state, without changing engine calls.
...
- Created a fresh replacement for PR #857 after it was reported not mergeable, starting again from the current main baseline and reapplying only the narrow `renderCivicationInbox()` DOM local-cast fix.
- Added local JSDoc casts for the existing `#civiInboxChoices`, `#civiInboxFeedback`, and `#civiInboxOK` query results as `HTMLElement|null`, `HTMLElement|null`, and `HTMLButtonElement|null`.
- Targeted the `CivicationUI.js` inbox DOM/querySelector diagnostic cluster around `style`, `onclick`, and choice container DOM access without changing selectors, markup, DOM structure, event logic, answer flow, visible text, CSS classes/IDs, storage keys, engine calls, public API, or runtime behavior.
- Regenerated `reports/typecheck-baseline-report.md`: total diagnostics went from 1535 to 1529, `js/Civication/**` diagnostics went from 271 to 265, and `js/Civication/ui/CivicationUI.js` diagnostics went from 37 to 31.
- `js/profile.js`, `js/boot.js`, `js/hgKnowledgeEngine.js`, `js/routes.js`, `js/ui/**`, data/story/import/place/people/manifest files, package metadata, CSS, HTML, and unrelated runtime files were not changed.
