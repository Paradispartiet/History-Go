...
function drawPlaceMarkers() {
  ...
  PLACES.forEach(p=>{
    ...
    // Stor usynlig hitbox for fingre
...
function drawPeopleMarkers() {
  ...
  PEOPLE.forEach(pr=>{
    // Personikon (initialer i sirkel, farget av kategori)
    ...
    const mk = L.marker([pr.lat ?? findPlaceLat(pr.placeId), pr.lon ?? findPlaceLon(pr.placeId)], { icon }).addTo(peopleLayer);

    // Treffflate
    const hb = L.circle([pr.lat ?? findPlaceLat(pr.placeId), pr.lon ?? findPlaceLon(pr.placeId)], {radius: 36, opacity:0, fillOpacity:0}).addTo(peopleLayer);
...
function findPlaceLat(pid){ return PLACES.find(x=>x.id===pid)?.lat ?? START.lat; }
function findPlaceLon(pid){ return PLACES.find(x=>x.id===pid)?.lon ?? START.lon; }
...
function openPlaceCard(p){
  ...
  el.pcMeta.textContent  = `${CAT_ICON[p.category]||""} ${p.category} • radius ${p.r||120} m`;
  ...
  el.pcUnlock.onclick = ()=> {
    ...
    el.pcUnlock.textContent = "Allerede låst";
...
function renderNearbyPeople(){
  if (!el.nearPeople) return;
  if (!currentPos) { el.nearPeople.innerHTML = ""; return; }
  const candidates = PEOPLE
    .map(pr=>{
      const lat = pr.lat ?? findPlaceLat(pr.placeId);
      const lon = pr.lon ?? findPlaceLon(pr.placeId);
      return { ...pr, _d: Math.round(distMeters(currentPos,{lat,lon})) };
    })
  ...
  return `
    <article class="card">
      <div>
        ...
        <div class="meta">${CAT_ICON[cat]||""} ${cat} • Oslo</div>
  ...
  return `
    <article class="card">
      <div>
        ...
        <div class="meta">${CAT_ICON[cat]||""} ${cat}</div>
...
function renderCollection(){
  ...
  el.collectionCount && (el.collectionCount.textContent = items.length);

  const first = items.slice(0, 6);
  const rest  = items.slice(6);

  el.collectionGrid.innerHTML = first.map(p => `
    <span class="badge ${catClass(p.category)}" title="${p.category}">
...
function renderMerits(){
  ...
  el.merits.innerHTML = [...cats].map(cat=>{
    ...
    return `
      <div class="card">
        <div class="name">${CAT_ICON[cat]||""} ${cat}</div>
...
function renderGallery(){
  ...
  el.gallery.innerHTML = got.length ? got.map(p=>{
    ...
    return `
      <span class="badge ${catClass(cat)}" title="${cat}">
        <span class="i" style="background:${catColor(cat)}"></span> ${p.name}
      </span>
    `;
...
document.addEventListener('click', (e)=>{
  ...
  const quizId = tgt.getAttribute('data-quiz');
  if (quizId){
...
function startQuizForPerson(personId){
  const qs = QUIZZES.filter(x=>x.personId===personId);
  if (!qs || !qs.length) { showToast("Ingen quiz enda – kommer!"); return; }

  quizState = { quiz: qs[0], i: 0, answers: [] };
