(() => {
  ...
  function captureQuizKnowledgeClaims(quizValue, contextValue = {}) {
    ...
    const conceptIds = unique3([explicitConceptIds, concepts.map((concept) => generatedCanonicalId("co", subjectId, concept))]);
    const explicitTermIds = explicitIdList(quizItem, "term_ids", "termIds");
    const termIds = unique3([explicitTermIds, terms.map((term) => generatedCanonicalId("term", subjectId, term))]);
    const storyIds = unique3([quizItem.story_ids, quizItem.storyIds]);
  ...
  function cleanStoredEntry(entryValue) {
    ...
    return claims.map((claim, index) => {
      ...
      const next = {
        ...
        text: claim,
        concept_ids: unique3([explicitConceptIds, concepts.map((concept) => generatedCanonicalId("co", subjectId, concept))]),
        term_ids: unique3([explicitTermIds, normalizeTerms(entry).map((term) => generatedCanonicalId("term", subjectId, term))]),
