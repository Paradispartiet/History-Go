...
- the shared interaction gate rejects restricted profiles and bidirectionally blocked profile pairs;
...
The identity migration restricts the authenticated browser role so it cannot directly self-authorize publication or write server-owned consent/fingerprint/visibility fields. Safety and moderation mutations are owned by FastAPI rather than new direct browser write paths.
