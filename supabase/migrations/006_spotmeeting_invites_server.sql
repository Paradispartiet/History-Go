...
comment on column public.hg_spotmeeting_invites.version is
  'Monotonic record version incremented by authoritative lifecycle transitions.';
...
-- Fail deployment rather than silently rewriting legacy participant data if active
-- duplicates already exist. Resolve such rows explicitly before applying the server
-- uniqueness contract.
do $$
begin
  if exists (
...
create index if not exists hg_spotmeeting_invites_creator_sync_idx
  on public.hg_spotmeeting_invites (created_by, updated_at, id);

create index if not exists hg_spotmeeting_invites_recipient_sync_idx
  on public.hg_spotmeeting_invites (target_user_id, updated_at, id);
