...
// PlaceCard eier bare ekte lokale stedsfunksjoner.
// Globale Events- og Møtes-flater ligger under Utforsk i venstre panel.
(function installPlaceOnSiteSurface(global) {
  ...
  function button(actionId, placeId) {
    ...
    return `<button class="pc-onsite-action pc-onsite-action-${esc(actionId)}" type="button" data-hg-onsite-action="${esc(actionId)}" data-place-id="${esc(placeId)}"><span class="pc-onsite-action-icon">${esc(def.icon || "•")}</span><span class="pc-onsite-action-label">${esc(def.label || actionId)}</span></button>`;
  }
  ...
  ["hg:appReady", "hg:place-selected", "hg:placesUpdated"].forEach(name => {
    global.addEventListener?.(name, () => decorate());
  });
})(window);
