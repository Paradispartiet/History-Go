...
(function (global) {
  ...
  function nextActionLabel(summary) {
    ...
    if (summary?.nextAction === "quiz") return "Quiz neste";
    return "Start";
  }
  ...
  function appendStatusToMeta(item, place) {
    ...
    const existing = normalize(meta.textContent)
      ...
      .filter(Boolean)
      .filter(part => !["Ikke startet", "Besøkt", "Quiz fullført", "Fullført", "Start", "Quiz neste", "Ferdig"].includes(part));

    existing.push(statusLabel(summary));
    existing.push(nextActionLabel(summary));
