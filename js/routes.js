...
function computeNearestStop(route, userPosObj, visitedMap = null) {
  ...
  // Bonus: prioriter nærmeste ubesøkte stopp hvis mulig
...
function focusRouteOnMap(routeId, startIndex = 0) {
  ...
  if (!MAP.__hgRouteStopsClickBound) {
    MAP.on("click", HG_ROUTE_STOPS, (e) => {
      ...
      if (p && typeof openPlaceCard === "function") openPlaceCard(p);
    });
  ...
  if (typeof showToast === "function") showToast("Rute vist på kartet");
}
...
async function showRouteToPlace(place) {
  ...
  try {
    ...
    if (typeof showToast === "function") showToast("Rute lagt.");
  } catch (e) {
    MAP.addSource(HG_NAV_SRC, {
      type: "geojson",
    ...
    if (typeof showToast === "function") showToast("Vis linje (ingen rutetjeneste)");
  }
}
