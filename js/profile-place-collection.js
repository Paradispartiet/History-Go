...
// in places_collected. The profile collection is the union of both.
(function () {
  "use strict";
  ...
  function getVisitedPlaceIds() {
    return readIdSet(VISITED_KEY);
  }

  function getQuizCollectedPlaceIds() {
    return readIdSet(COLLECTED_KEY);
  }

  function getCollectedPlaceIds() {
    return new Set([
      ...getVisitedPlaceIds(),
      ...getQuizCollectedPlaceIds()
    ]);
  }
  ...
  function renderPlacesCollection() {
    ...
    if (!collectedPlaces.length) {
      grid.innerHTML = '<div class="muted">Ingen steder samlet ennå.</div>';
      return;
    }
    ...
    grid.innerHTML = collectedPlaces.map((place) => {
      ...
      return `
        <div class="card place-card" data-place="${esc(place.id)}">
    ...
    grid.querySelectorAll?.(".place-card").forEach((el) => {
      el.onclick = () => {
        const place = places.find((candidate) => String(candidate?.id || "") === String(el.dataset?.place || ""));
  ...
  function renderTimeline() {
    ...
    const items = [
      ...places
        .filter((place) => collectedPlaceIds.has(String(place?.id || "").trim()))
        .map((place) => ({
          type: "place",
          id: place.id,
          name: place.name,
          year: Number(place.year) || 0,
          image: place.image || `bilder/places/${place.id}.PNG`
        })),
      ...people
        .filter((person) => collectedPeopleIds.has(String(person?.id || "").trim()))
        .map((person) => ({
          type: "person",
          id: person.id,
          name: person.name,
          year: Number(person.year) || 0,
          image: person.image || `bilder/people/${person.id}.PNG`
        }))
    ...
    if (!items.length) {
      ...
      if (text) text.textContent = "Du har låst opp 0 kort";
      return;
    }

    body.innerHTML = items.map((item) => `
      <div class="timeline-card ${esc(item.type)}" data-id="${esc(item.id)}">
    ...
    body.querySelectorAll?.(".timeline-card").forEach((el) => {
      el.onclick = () => {
        ...
        const place = places.find((candidate) => String(candidate?.id || "") === id);
        if (place) window.showPlacePopup?.(place);
      };
    });
  }
  ...
  function renderCollectionCards() {
    ...
    const personCards = people
      ...
      .map((person) => ({
        ...
        name: person.name,
        year: Number(person.year) || 0,
        image: person.imageCard || person.cardImage || person.image || ""
      }));
    ...
    body.innerHTML = items.map((item) => `
      <div class="collection-card" data-id="${esc(item.id)}">
        ${item.image ? `<img src="${esc(item.image)}" alt="${esc(item.name)}">` : ""}
        <div class="collection-card-name">${esc(item.name)}</div>
        <div class="collection-card-year">${item.year || ""}</div>
      </div>`).join("");
  ...
  function install() {
    window.renderPlacesCollection = renderPlacesCollection;
  ...
  function refreshWhenReady(attempt = 0) {
    install();
    const ready = Array.isArray(window.PLACES) && !!document.getElementById?.("collectionGrid");
    if (ready) {
      refresh();
      return;
    }
    if (attempt < 80) setTimeout(() => refreshWhenReady(attempt + 1), 50);
  }

  window.HGProfilePlaceCollection = {
    getVisitedPlaceIds,
    getQuizCollectedPlaceIds,
    getCollectedPlaceIds,
    getCollectedSource,
    install,
    refresh
  };
