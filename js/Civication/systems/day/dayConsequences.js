(function () {
  ...
  function applyChoiceConsequences(eventObj, choice, result) {
  ...
  function patchAnswer() {
    ...
    if (!proto || proto.__dayConsequencesPatched) return;
    if (typeof proto.answer !== "function") return;

    const previous = proto.answer;
    proto.__dayConsequencesPatched = true;
    ...
    proto.answer = async function (eventId, choiceId) {
      ...
      if (result?.ok && eventObj && choice) {
        try {
          applyChoiceConsequences(eventObj, choice, result);
        } catch (err) {
          console.warn("[dayConsequences] failed to apply consequences", err);
        }
      }
