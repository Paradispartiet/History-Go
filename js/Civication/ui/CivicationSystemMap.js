...
(function(){
  ...
  let contexts=FALLBACK_CONTEXTS,selectedId=null,resizeTimer=null,hudMinimized=false,panelMinimized=false;
  ...
  ["civi:booted","civi:dataReady","civi:homeChanged","updateProfile"].forEach(ev=>window.addEventListener(ev,render));
  window.addEventListener("storage",render);
  window.addEventListener("resize",()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(render,120)});
  window.CivicationSystemMap={render};
})();
