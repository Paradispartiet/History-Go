 ...
 * @typedef {{ at?: string|number|Date, [key: string]: unknown }} CiviUiPsycheCollapse
 * @typedef {{ value?: number, max?: number, collapses?: number, lastCollapse?: CiviUiPsycheCollapse, [key: string]: unknown }} CiviUiPsycheTrust
 ...
 * @typedef {Record<string, number|boolean|string|null|undefined|CiviUiRecord>} CiviUiPsycheModifiers
 */
...
function renderPsycheDashboard() {
  ...
  const activePosition =
    /** @type {CiviUiActivePosition|null} */ (window.CivicationState?.getActivePosition?.() || null);
  const activeCareerId = activePosition?.career_id || null;
  
  /** @type {CiviUiPsycheSnapshot|null} */
  const snapshot =
    /** @type {CiviUiPsycheSnapshot|null} */ (window.CivicationPsyche?.getSnapshot?.(activeCareerId) || null);
  if (!snapshot) return;
