...
(function () {
  ...
  function nameHint(hay) {
    ...
    if (/stasjon|t-bane|metro|jernbane|holdeplass/.test(hay)) return "station_miniature";
    if (/museum|galleri/.test(hay)) return "museum_miniature";
    if (/bibliotek/.test(hay)) return "library_miniature";
    if (/opera|teater|scene|konserthus|kino/.test(hay)) return "theatre_miniature";
    ...
    if (/brygge|havn|kai|verft/.test(hay)) return "waterfront_miniature";
  ...
  function categoryAsset(place, hay) {
    ...
    switch (cat) {
      ...
      case "politikk": return "default_miniature";
  ...
  function resolveAssetType(place) {
    ...
    if (place.raw.mapAssetType) return place.raw.mapAssetType;
    const qp = place.raw.quiz_profile || {};
    const pt = String(qp.place_type || qp.subtype || "").toLowerCase();
    const PT = {
      stadium: "stadium_miniature", arena: "stadium_miniature", museum: "museum_miniature",
      church: "church_miniature", school: "school_miniature", university: "school_miniature",
      station: "station_miniature", park: "park_miniature", playground: "playground_miniature",
      street: "street_miniature", fortress: "fortress_miniature", library: "library_miniature",
      theatre: "theatre_miniature", theater: "theatre_miniature", waterfront: "waterfront_miniature"
    };
    if (pt && PT[pt]) return PT[pt];
  ...
  function priorityOf(p) {
    ...
    if (p.category === "sport" && /stadion|arena/.test(hay)) s += 5;
  ...
  function visibleSet(places, zoom) {
    if (zoom > 2.2) return places;
    const sorted = places.slice().sort((a, b) => priorityOf(b) - priorityOf(a));
    if (zoom < 1.4) return sorted.slice(0, 30);
    return sorted.slice(0, Math.min(sorted.length, 120));
  }
  ...
  function render() {
    ...
    const zoom = getZoom();
    g.setAttribute("data-civi-zoom-level", zoom > 2.6 ? "high" : zoom > 1.4 ? "mid" : "low");
    const sc = clamp(1 / Math.sqrt(zoom), 0.5, 1.15);
  ...
  ["civi:dataReady", "civi:booted", "civi:mapRendered", "civi:mapZoomChanged", "resize", "storage"]
