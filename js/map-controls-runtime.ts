...
function ensureControls(): HTMLElement | null {
  ...
  let utilityRow = controls.querySelector<HTMLDivElement>(".hg-map-utility-row");
  if (!utilityRow) {
    utilityRow = createUtilityRow();
    controls.appendChild(utilityRow);
  }
  let categoryFilter = controls.querySelector<HTMLDivElement>(".hg-map-category-filter");
  if (!categoryFilter) {
