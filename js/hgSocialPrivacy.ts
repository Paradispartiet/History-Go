...
(function () {
  ...
  function getProfiles() { return (typeof window.getAllProfiles === "function" ? window.getAllProfiles() : readJson(KEYS.profiles, [])).filter(Boolean); }
  function getProfile(id) { return getProfiles().find((p) => String(p?.userId) === String(id)) || (String(id) === currentUserId() ? readJson(KEYS.currentProfile, {}) : null); }
  function getCircles() { return typeof window.getLearningCircles === "function" ? window.getLearningCircles() : readJson(KEYS.circles, []); }
  function getTrust(a, b) { return typeof window.calculateTrustScore === "function" ? window.calculateTrustScore(a, b) : { trustScore: 0, trustTier: "new" }; }
  ...
  function isBlocked(userId, targetId) { return getBlockedUsers(userId).includes(String(targetId)) || getBlockedUsers(targetId).includes(String(userId)); }
  function removeTrustLink(userId, targetId) {
    const log = readJson(KEYS.log, {});
    log.relations = (Array.isArray(log.relations) ? log.relations : []).filter((r) => String(r?.userId) !== String(targetId));
  ...
  function renderPrivacySettings() { const root = document.getElementById("social-privacy-settings"); if (!root) return; const settings = getPrivacySettings(); const controls = [["publicProfile","Offentlig profil"],["visibleInMatchLists","Synlig i kunnskapsmatcher"],["allowMeetInvites","Tillat møteinvitasjoner"],["allowCircleInvites","Tillat sirkelinvitasjoner"],["showSocialReputation","Vis sosial tillit"]]; root.innerHTML = `<section class="meet-invite-inbox-panel social-privacy-panel"><div class="section-head"><h2>Personvern</h2><span class="section-meta">Kunnskapsgraf, aldri lokasjonsgraf.</span></div>${controls.map(([key,label]) => `<label class="social-privacy-toggle"><span>${escapeHtml(label)}</span><input type="checkbox" data-social-privacy="${key}" ${settings[key] ? "checked" : ""}></label>`).join("")}</section>`; root.querySelectorAll("[data-social-privacy]").forEach((el) => el.addEventListener("change", () => savePrivacySettings(currentUserId(), { [el.dataset.socialPrivacy]: el.checked }))); }
