  ...
  function load(subjectId, { allowPlanned = false } = {}) {
    ...
    if (!subjectPromises.has(cacheKey)) {
      subjectPromises.set(cacheKey, (async () => {
        ...
        const [pensum, emners, fagkart, methods, badge, finalOverlay] = await Promise.all([
        ...
        const composed = id === 'natur'
          ? composeNaturFinal({ pensum, emners, fagkart, methods, registry: controls.registry, statusEntry: baseStatusEntry, overlay: finalOverlay })
          : { pensum, emners, fagkart, methods, registry: controls.registry, statusEntry: baseStatusEntry };
