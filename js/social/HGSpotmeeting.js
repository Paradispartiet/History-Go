(function(){
  ...
  function createSpotmeetingInvite(targetUserId,context,presetMessageId){const can=canCreateSpotmeetingInvite(targetUserId,context);if(!can.ok)return can;const preset=PRESETS.find(p=>p.presetMessageId===presetMessageId);if(!preset)return {ok:false,reason:'invalid_preset_message'};const s=read();s.sequence=Number(s.sequence||0)+1;const invite={inviteId:`spotmeeting-${s.sequence}`,targetUserId:String(targetUserId),context:can.context,presetMessageId:preset.presetMessageId,presetLabel:preset.label,status:'pending',manual:true,private:true,demoOnly:testMode()};const privacy=scanForbidden(invite);if(!privacy.ok)return {ok:false,reason:'forbidden_privacy_field',privacy};s.invites=list(s.invites);s.invites.push(invite);s.timeline=list(s.timeline);s.timeline.push({eventId:`spotmeeting-event-${s.sequence}`,inviteId:invite.inviteId,type:'created',context:invite.context,private:true,demoOnly:invite.demoOnly});write(s);return {ok:true,invite:clone(invite),warning:testMode()?null:'backend_not_enabled'};}
  function update(inviteId,status){const s=read();const inv=list(s.invites).find(i=>i.inviteId===inviteId);if(!inv)return {ok:false,reason:'unknown_invite'};if(status==='completed'&&inv.status==='completed')return {ok:true,invite:clone(inv),duplicate:true,trustDelta:0};inv.status=status;s.timeline=list(s.timeline);s.timeline.push({eventId:`spotmeeting-event-${Number(s.sequence||0)+s.timeline.length+1}`,inviteId,type:status,private:true,demoOnly:inv.demoOnly});write(s);return {ok:true,invite:clone(inv),trustDelta:0};}
  ...
  function getSpotmeetingInbox(){const invites=list(read().invites);return {pending:invites.filter(i=>i.status==='pending'),accepted:invites.filter(i=>i.status==='accepted'),completed:invites.filter(i=>i.status==='completed'),blockedOrReported:list(read().blockedUserIds).length+list(read().reportedUserIds).length>0,privacyChecklist:['Manuell','Kunnskapsbasert','Forhåndsmelding','Privat','Kan avbrytes/blokkeres/rapporteres']};}
