// History Go map-mode controls and marker category filtering.
const win = window;
...
function categoryMeta(categoryId) {
  ...
  return categories().find((category) => String(category?.id || "") === categoryId) ||
    { id: categoryId, name: categoryId, icon: "•", color: "#60758a" };
}
...
function applyFilter(categoryId) {
  ...
  if (!sourcePlaces.length && Array.isArray(win.PLACES)) sourcePlaces = win.PLACES;
  if (originalSetPlaces) originalSetPlaces(filteredPlaces(sourcePlaces));
  renderCategoryUi();
  win.dispatchEvent(new CustomEvent("hg:map-category-filter", { detail: { category: activeCategory } }));
}
...
async function centerMap() {
  ...
  try {
    let pos = win.getPos?.();
    if ((!Number.isFinite(Number(pos?.lat)) || !Number.isFinite(Number(pos?.lon))) && win.HGPos?.request) {
      try { await win.HGPos.request(); } catch {}
      pos = win.getPos?.();
    }
    const lat = Number(pos?.lat);
    const lon = Number(pos?.lon);
    const map = win.HGMap?.getMap?.();
...
function bindUi() {
  ...
  if (options && options.dataset.hgBound !== "1") {
    ...
    options.addEventListener("click", (event) => {
      const option = event.target instanceof Element ? event.target.closest("[data-map-category]") : null;
      if (!option) return;
      applyFilter(option.getAttribute("data-map-category") || ALL);
      closeMenu();
    });
  }
