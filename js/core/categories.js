(() => {
  ...
  const catColor = (categoryId) => CAT_BY_ID[norm(categoryId)]?.color || "#6c757d";
  const catSecondaryColor = (categoryId) => {
    const category = CAT_BY_ID[norm(categoryId)];
    return category?.secondaryColor || category?.color || "#6c757d";
  };
  const catClass = (categoryId) => {
  ...
  const catIdFromDisplay = (display) => {
    ...
    return CAT_BY_NAME[normalizedDisplay]?.id || null;
  };
