...
`window.HG_SpotmeetingUI` is exposed by `js/social/HGSpotmeetingUI.js` and is the canonical user surface for starting a Kunnskapsmøte (Spotmeeting). It owns the `#hgSpotmeetingSheet` sheet with the four choices (Se kunnskapsmatcher / Inviter til quiz / Inviter til observasjon / Inviter til rute), and is opened from PlaceCard "På stedet" (`data-knowledge-spot-match`, main entry) and the single People CTA (`data-hg-spotmeeting-open="people"`). It delegates all state to `window.HG_Spotmeeting`, shows demo candidates only in TEST_MODE, renders a backend-disabled state in production, and dispatches `hg:spotmeetingChanged` + `updateProfile` after an invite. `window.openSpotMatchList` is kept as a legacy wrapper into `HG_SpotmeetingUI.open(...)`. `js/social/HGSpotmeetingPlaceCardDemo.js` (`window.HG_SpotmeetingPlaceCardDemo`) is only a compatibility/demo wrapper that delegates to `HG_SpotmeetingUI`. The footer has no Spotmeeting entry; follow-up runs through `window.HG_SocialMeetUI` (the sent state and the backend-disabled state both open the Social Meet sheet, not `profile.html`).

### `window.HG_SocialMeetUI`

`window.HG_SocialMeetUI` is exposed by `js/social/HGSocialMeetUI.js` and is the canonical Social Meet surface for **following up** invitations, answers and meeting status. It owns the `#hgSocialMeetSheet` sheet (its own sheet, not a `profile.html` tab), with status tabs (Venter / Avtalt / Gjennomført / Avslått) read from `window.HG_Spotmeeting`'s inbox — no chat, free text, live location, nearby, followers, feed, distance, last seen or auto-invites. Product split: **Kunnskapsmøte** (`HG_SpotmeetingUI`) starts a concrete meeting; **Social Meet** (`HG_SocialMeetUI`) follows it up; **Profile** keeps settings/privacy/history and can still link to Social Meet but is not the only surface. `HG_SocialMeetUI.open({ filter, placeId, sourceSurface })`: entry points are the header/menu opener (`filter: "all"`, `sourceSurface: "globalMenu"`, `#btnSocialMeet` in `index.html`), PlaceCard "På stedet" (`filter: "place"`, `sourceSurface: "placeCardOnSite"`), and the Kunnskapsmøte follow-up (`sourceSurface: "spotmeetingFollowUp"`, filtered by place when the context is a place). A delegated click handler reads `data-hg-social-meet-open` (`"place"`/`"all"`), `data-hg-social-meet-place` and optional `data-hg-social-meet-source`. It also renders the on-site status card (`renderPlaceSummary`/`enhanceOnSiteLinks`) under "På stedet". Local-only, privacy-safe, no gameplay mutation, no backend.
