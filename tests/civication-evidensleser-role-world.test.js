...
const stream=read('data/Civication/narratives/leisure/helse_evidensleser.json');
const world=read('data/Civication/roleWorlds/helse/helse_evidensleser.json');
const index=read('data/Civication/roleWorlds/index.json');
const row=audit.positions.find(x=>x.key==='helse/evidensleser');
assert.ok(row);
assert.equal(world.status,'role_world_complete');
assert.equal(world.subject_type,'life_position');
assert.deepEqual(world.life_position_ref,{badge_id:'helse',id:'evidensleser',label:'Evidensleser'});
...
assert.equal(row.role_world_path,'data/Civication/roleWorlds/helse/helse_evidensleser.json');
assert.equal(row.authored_depth.max_narrative_depth,14);
assert.ok(!audit.queue.some(x=>x.key==='helse/evidensleser'));
...
for(const id of ['headline_vs_design','association_vs_causation','source_conflict_vs_quality','uncertainty_vs_action','correction_vs_identity','evidence_literacy_vs_certainty'])assert.ok(world.primary_threads.some(x=>x.id===id),id);
for(const id of ['lenkevenn','overskriftsformidler','metodevenn','fagperson_med_grenser','sikker_debattant','rolig_kildejeger'])assert.ok(world.recurring_people_archetypes.some(x=>x.id===id),id);
assert.match(world.sociological_core.description,/ingen klinisk rolle/i);
console.log('Evidensleser Role World gate ok');
