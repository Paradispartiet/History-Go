...
test('Religion history/comparison har full claim-, kilde-, metode- og scenariointegritet', () => {
  ...
  assert.equal(report.methods.materializedRequiredMethodCount, 10);
...
test('Alle tolv Religion-artikler består egenart og seksdelt 29/30-port uten for tidlig complete', () => {
  const { report } = auditReligionHistoryComparisonArticles({ checkReport: false });
  assert.equal(report.subject.editorialStatus, 'chapters_in_progress');
  assert.equal(report.subject.completeReady, false);
  assert.equal(report.editorial.allReligionArticleCountReviewed, 12);
