...
assert.equal(index.roles.filter(x=>x.subject_type==='life_position').length, 22);
assert.equal(index.roles.length, 107);
assert.equal(index.status,'107_role_worlds_materialized');
assert.deepEqual(index.summary,{role_worlds_total: 107,career_role_worlds:85,life_position_role_worlds: 22});
...
console.log('civication Liv alternativ Nomade Role World ok: 56/56 / 14 anchors / 105 total / 20 life-position worlds');
