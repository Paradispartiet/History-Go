...
const placePath = 'data/places/natur/oslo/places_oslo_natur_alnaelva_rute/alnaparken.json';
const quizPath = 'data/quiz/natur/alnaparken_sets.json';
const storyPath = 'data/stories/stories_alnaparken.json';
const articlePath = 'data/leksikon/places/oslo/natur/leksikon_alnaparken.json';
...
assert.strictEqual(place.id, 'alnaparken');
assert.strictEqual(place.name, 'Alnaparken');
assert.strictEqual(place.category, 'natur');
assert.deepStrictEqual([place.lat, place.lon, place.r, place.year ?? null], [59.94261, 10.87749, 140, null]);
...
assert(/ikke gå ned i elva|ikke.*amfibiedammen|ikke plukk/i.test(place.training_profile.safety));
...
assert.deepStrictEqual(place.nature_profile.nearby_place_ids, ['alnsjoen_alna_kilde','groruddammen','alna_smalvoll']);
...
for (const file of mapFiles) { const raw=readJson(file); const entry=(raw.places||raw).alnaparken; if(!entry) continue; merged.flora.push(...(entry.flora||[])); merged.fauna.push(...(entry.fauna||[])); }
merged.flora=[...new Set(merged.flora)].sort(); merged.fauna=[...new Set(merged.fauna)].sort();
assert.deepStrictEqual(merged.flora,['emne_flora_kanadagullris']);
...
assert.deepStrictEqual(inventory.flora.map(x=>x.id),['emne_flora_kanadagullris']);
...
for (const token of ['Alnaparken','Nylandsparken','kanadagullris','Solidago canadensis','svært høy risiko','1958','2001','2005','amfibiedam','Alna miljøpark']) assert(all.includes(token),`mangler ${token}`);
assert(/ikke en garanti|ikke.*garanti/i.test(all));
console.log('Alnaparken nature rounds batch 1 OK');
