...
const streamPath = 'data/Civication/narratives/leisure/litteratur_skald.json';
const worldPath = 'data/Civication/roleWorlds/litteratur/litteratur_skald.json';
const stream = read(streamPath);

const tier = badge.tiers.find((entry) => entry.life_position?.id === 'skald');
assert.ok(tier);
assert.equal(tier.threshold, 1000);
assert.equal(tier.life_position.kind, 'historical_stylistic_identity');
assert.equal(tier.life_position.employment_independent, true);

assert.equal(stream.schema, 'civication_narrative_stream_v1');
assert.equal(stream.id, 'litteratur_skald_stream');
...
const row = audit.positions.find((entry) => entry.key === 'litteratur/skald');
assert.ok(row);
...
const indexed = index.roles.find((entry) => entry.life_position_key === 'litteratur/skald');
assert.ok(indexed);
assert.equal(indexed.role_scope, 'litteratur_skald');
assert.equal(audit.first_ready, null);
assert.equal(audit.summary.pending_ready_positions, 0);

console.log('Skald readiness gate ok: ready + role_world_complete');
