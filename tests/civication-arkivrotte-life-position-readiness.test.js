...
const streamPath='data/Civication/narratives/leisure/historie_arkivrotte.json';
...
assert.equal(row.kind,'alternative_life_status');
assert.equal(stream.storylets.length,14);
assert.equal(new Set(stream.storylets.map(x=>x.id)).size,14);
assert.equal(stream.applies_when.any_tags[0],'historie:arkivrotte');
...
console.log('Arkivrotte readiness and lifecycle complete ok');
