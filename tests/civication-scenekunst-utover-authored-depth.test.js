...
const lifeId='utover';
const lifeKey='scenekunst/utover';
const lifeScope='scenekunst_utover';
const streamPath='data/Civication/narratives/leisure/scenekunst_utover.json';
const worldPath='data/Civication/roleWorlds/scenekunst/scenekunst_utover.json';

const stream=read(streamPath);
const tier=badge.tiers.find(x=>x.life_position?.id===lifeId);
assert.ok(tier,'Scenekunst Utøver tier must exist');
assert.equal(tier.life_position.label,'Utøver');
assert.equal(tier.life_position.kind,'artistic_performance_practice');
...
assert.ok(row,'Scenekunst Utøver must remain in life-position readiness');
assert.equal(row.classification,'ready','The authored Scenekunst Utøver stream must make the position ready before Role World production');
...
if(!worldExists){
  ...
  assert.equal(indexed,undefined,'Source-depth phase must not pre-register an unbuilt Utøver Role World');
  const queued=audit.queue.find(x=>x.key===lifeKey);
  assert.ok(queued,'Ready but unbuilt Utøver must remain in queue');
  assert.equal(queued.rank,1,'Utøver should become the next canonical ready Role World after authored-depth repair');
  ...
  assert.ok(!audit.queue.some(x=>x.key===lifeKey),'Completed Utøver must leave the readiness queue');
  assert.ok(indexed,'Completed Utøver must be indexed');
  assert.equal(indexed.role_scope,lifeScope);
  assert.equal(indexed.status,'role_world_complete');
  assert.equal(world.schema,'civication_role_world_v1');
  assert.equal(world.subject_type,'life_position');
  assert.equal(world.status,'role_world_complete');
  assert.deepEqual(world.life_position_ref,{badge_id:'scenekunst',id:lifeId,label:'Utøver'});
  ...
  assert.match(world.sociological_core.description,/Publikum/);
  assert.match(world.sociological_core.description,/Improentusiast/);
  assert.match(world.sociological_core.description,/Skuespiller \/ danser/);
  assert.match(world.sociological_core.description,/Regissør/);
  assert.match(world.sociological_core.description,/Koreograf/);
  assert.match(world.sociological_core.description,/Dramaturg/);
}
console.log(worldExists
 ? 'Scenekunst Utøver authored-depth and completed Role World gate ok'
 : 'Scenekunst Utøver authored-depth gate ok: exact 14-storylet source makes artistic performance practice ready without professional authority');
