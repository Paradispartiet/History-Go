...
click(appWindow, '[data-knowledge-spot-match]', 'People → Møt folk');
sheet = appWindow.document.getElementById('hgSpotmeetingSheet');
assert(sheet && !sheet.hidden, 'contextual People shortcut opens canonical Spotmeeting sheet');
assert(sheet.textContent.includes('Factory Memory'), 'contextual shortcut preserves active place context');
assertNoForbiddenUi(sheet, 'contextual Spotmeeting sheet');
