...
const stream=read('data/Civication/narratives/leisure/vitenskap_folkeforsker.json');
const world=read('data/Civication/roleWorlds/vitenskap/vitenskap_folkeforsker.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'vitenskap',id:'folkeforsker',label:'Folkeforsker'});
...
const row=audit.positions.find(x=>x.key==='vitenskap/folkeforsker');
...
assert.match(world.sociological_core.description,/ingen automatisk forskerstilling/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('nullobservasjoner'));
assert.ok(world.sociological_core.description.includes('replikasjon'));
console.log('Folkeforsker Role World gate ok');
