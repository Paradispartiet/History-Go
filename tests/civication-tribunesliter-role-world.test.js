...
const stream=read('data/Civication/narratives/leisure/sport_tribunesliter.json');
const world=read('data/Civication/roleWorlds/sport/sport_tribunesliter.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'sport',id:'tribunesliter',label:'Tribunesliter'});
...
const row=audit.positions.find(x=>x.key==='sport/tribunesliter');
...
assert.match(world.sociological_core.description,/alternative_life_status/i);
assert.match(world.sociological_core.description,/ingen automatisk myndighet som supporterkoordinator/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('bortereiseøkonomi'));
assert.ok(world.sociological_core.description.includes('nykommertilgang'));
console.log('Tribunesliter Role World gate ok');
