...
const streamPath='data/Civication/narratives/leisure/religion_trosstedsvandrer.json';
const stream=read(streamPath),row=audit.positions.find(x=>x.key==='religion/trosstedsvandrer');
...
assert.equal(stream.applies_when.any_tags[0],'religion:trosstedsvandrer');
assert.equal(row.classification,'ready');assert.equal(row.role_world_status,'role_world_complete');
assert.equal(row.role_world_path,'data/Civication/roleWorlds/religion/religion_trosstedsvandrer.json');assert.equal(row.priority_score,405);
...
console.log('Trosstedsvandrer readiness and lifecycle complete ok');
