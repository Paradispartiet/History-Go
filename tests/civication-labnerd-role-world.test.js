...
const stream=read('data/Civication/narratives/leisure/vitenskap_labnerd.json');
const world=read('data/Civication/roleWorlds/vitenskap/vitenskap_labnerd.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'vitenskap',id:'labnerd',label:'Labnerd'});
...
const row=audit.positions.find(x=>x.key==='vitenskap/labnerd');
...
assert.match(world.sociological_core.description,/ingen automatisk laboratorielederrolle/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('måleusikkerhet'));
assert.ok(world.sociological_core.description.includes('data lineage'));
console.log('Labnerd Role World gate ok');
