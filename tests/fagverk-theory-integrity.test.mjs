...
test('eksisterende subject-gates beholdes og Media + Litteratur er field-level strictly proven', () => {
  const r=auditFagverkTheoryIntegrity();
  const byId=new Map(r.subjects.map(s=>[s.id,s]));
  for(const id of ['film_tv','religion','subkultur','scenekunst','historie','by','kunst','media','litteratur']){
...
test('completion-status er read-only og proof-køen er redusert til 9', () => {
  ...
  for(const id of ['film_tv','religion','subkultur','scenekunst','historie','by','kunst','media','litteratur'])assert.ok(!r.proofReconciliationQueue.includes(id));
});
