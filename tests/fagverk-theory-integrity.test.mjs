...
test('alle reconciled subject-gates, inkludert Næringsliv og Psykologi, er field-level strictly proven', () => {
  const r=auditFagverkTheoryIntegrity();
  const byId=new Map(r.subjects.map(s=>[s.id,s]));
  for(const id of ['film_tv','religion','subkultur','scenekunst','historie','by','kunst','media','musikk','litteratur','natur','naeringsliv','psykologi']){
...
test('completion-status er read-only og proof-køen er redusert til 5', () => {
  const r=auditFagverkTheoryIntegrity();
  assert.equal(r.rules.completionStatusReadOnly,true);
  assert.equal(r.proofReconciliationQueue.length,5);
  for(const id of ['film_tv','religion','subkultur','scenekunst','historie','by','kunst','media','musikk','litteratur','natur','naeringsliv','psykologi'])assert.ok(!r.proofReconciliationQueue.includes(id));
});
