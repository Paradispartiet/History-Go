...
test('eksisterende subject-gates beholdes og Historie, By og Kunst er field-level strictly proven', () => {
  const r=auditFagverkTheoryIntegrity();
  const byId=new Map(r.subjects.map(s=>[s.id,s]));
  for(const id of ['film_tv','religion','subkultur','scenekunst','historie','by','kunst']){
...
test('completion-status er read-only og proof-køen er redusert til 11', () => {
  const r=auditFagverkTheoryIntegrity();
  assert.equal(r.rules.completionStatusReadOnly,true);
  assert.equal(r.proofReconciliationQueue.length,11);
  for(const id of ['film_tv','religion','subkultur','scenekunst','historie','by','kunst'])assert.ok(!r.proofReconciliationQueue.includes(id));
});
