...
test('baseline strong blir ikke feilaktig oppgradert uten strict proof', () => {
  ...
  assert.equal(r.summary.strictly_proven,16);
...
test('alle reconciled subject-gates, inkludert Psykologi, Sport, Vitenskap og Politikk, er field-level strictly proven', () => {
  const r=auditFagverkTheoryIntegrity();
  const byId=new Map(r.subjects.map(s=>[s.id,s]));
  for(const id of ['film_tv','religion','subkultur','scenekunst','historie','by','kunst','media','musikk','litteratur','natur','naeringsliv','psykologi','sport','vitenskap','politikk']){
...
test('completion-status er read-only og proof-køen er redusert til 2', () => {
  const r=auditFagverkTheoryIntegrity();
  assert.equal(r.rules.completionStatusReadOnly,true);
  assert.equal(r.proofReconciliationQueue.length,2);
  for(const id of ['film_tv','religion','subkultur','scenekunst','historie','by','kunst','media','musikk','litteratur','natur','naeringsliv','psykologi','sport','vitenskap','politikk'])assert.ok(!r.proofReconciliationQueue.includes(id));
});
