...
assert.ok(exists(WORLD), 'Advisory Role World must exist before strict rollout proof runs');
...
assert.equal(world.existing_work_continuity.persistent_work_object, 'saksgrunnlag_og_radgivningsspor');
assert.equal(world.existing_work_continuity.new_runtime_state, false);
assert.equal(read(GRAMMAR).day_one_contract.entry, 'direct');
...
const audienceIds = [
  'senior_advisers_and_quality_leads',
  'legal_authority_and_mandate_stewards',
  'documentation_and_records_stewards',
  'commissioners_and_decision_owners',
  'affected_stakeholders_and_hearing_voices',
  'peer_advisers_and_cross_function_partners',
  'institutional_leaders_and_future_employers',
  'private_relations'
];
...
for (const audience of rep.audiences) {
  ...
  assert.match(audience.cannot_grant, /kan ikke|ikke gi|gir ikke|skaper ikke/i);
}
assert.ok(rep.divergence_examples.length >= 6);
for (const term of [/global/i,/evidens|kilde|fakta/i,/vedtak|beslutning/i,/hjemmel/i,/deleg/i,/ansett|utnevn/i,/budsjett|ressurs/i,/History Go|Badge/i]) assert.match(rep.authority_separation, term);
...
assert.match(world.history_go_affordance.better_question, /kilde|historiografi|arkiv|institusjon|precedens|presedens/i);
for (const term of [/kan ikke|ikke gi/i,/vedtak|beslutning/i,/hjemmel/i,/deleg/i,/autentiser/i,/utnevn|ansett/i,/budsjett|ressurs/i]) assert.match(world.history_go_affordance.authority_boundary, term);
...
assert.match(world.cross_role_proof.rule, /candidate_when_shared_work_is_real|shared work object|delt arbeidsobjekt|reelt delt/i);
...
assert.ok(readiness.summary.role_world_complete_or_pilot >= 58);
assert.ok(readiness.summary.rollout_queue_roles <= 27);
...
assert.equal(career.audit.salary.rows.length, 5);
for (const salary of career.audit.salary.rows) assert.equal(salary.offer_policy, 'direct');

const source = fs.readFileSync(path.join(ROOT, 'reports/CIVICATION_HISTORIE_FORVALTNING_OG_RADGIVNING_ROLE_WORLD_ROLLOUT_SOURCE_FIRST.md'), 'utf8');
...
console.log('Civication Historie Forvaltning og rådgivning Role World rollout: OK');