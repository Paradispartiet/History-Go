...
const streamPath='data/Civication/narratives/leisure/psykologi_monsterjeger.json';
const stream=read(streamPath),row=audit.positions.find(x=>x.key==='psykologi/monsterjeger');
...
assert.equal(stream.applies_when.any_tags[0],'psykologi:monsterjeger');
assert.equal(row.classification,'ready');assert.equal(row.role_world_status,'role_world_complete');
assert.equal(row.role_world_path,'data/Civication/roleWorlds/psykologi/psykologi_monsterjeger.json');assert.equal(row.priority_score,405);
...
console.log('Mønsterjeger readiness and lifecycle complete ok');
