...
const lifeKey='naeringsliv/bedriftseier';
const lifeScope='naeringsliv_bedriftseier';
const streamPath='data/Civication/narratives/leisure/naeringsliv_bedriftseier.json';
const worldPath='data/Civication/roleWorlds/naeringsliv/naeringsliv_bedriftseier.json';

const tier=contracts.tiers.find(x=>x.life_position?.id==='bedriftseier');
assert.ok(tier);
assert.equal(tier.label,'Bedriftseier');
assert.equal(tier.life_position.kind,'business_ownership');
...
assert.equal(stream.id,'naeringsliv_bedriftseier_stream');
assert.deepEqual(stream.applies_when.any_tags,['naeringsliv:bedriftseier']);
...
assert.deepEqual(indexed.life_position_ref,{badge_id:'naeringsliv',id:'bedriftseier',label:'Bedriftseier'});
...
assert.deepEqual(world.life_position_ref,{badge_id:'naeringsliv',id:'bedriftseier',label:'Bedriftseier'});
assert.equal(world.materialization.no_new_runtime,true);
assert.match(world.sociological_core.description,/business_ownership|kapitalinnskudd|selskapsmidler/i);
assert.match(world.sociological_core.description,/Gründer|Investor|Industrieier|Daglig leder/i);
assert.match(world.sociological_core.description,/ingen automatisk daglig leder-status|fri tilgang til selskapets midler|garanti for avkastning/i);
...
assert.match(stream.storylets.find(x=>x.id==='garantien_med_privathuset_i_bakgrunnen').situation.join(' '),/personlig sikkerhet|private økonomi|beløp/i);
assert.match(stream.storylets.find(x=>x.id==='sesongslutt_hva_er_en_bedriftseier').situation.join(' '),/Gründer|Investor|Industrieier|Daglig leder/i);
...
console.log('Næringsliv Bedriftseier authored-depth and Role World gate ok');
