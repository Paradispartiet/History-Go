...
const place = read("data/places/politikk/oslo/places_politikk/stortinget.json");
const production = read("data/places/production/stortinget.json");
const politics = read("data/places/politikk-production/stortinget.json");
const quiz = read("data/quiz/politikk/stortinget_sets.json");
const brief = read("data/quiz/production_briefs/politikk/stortinget.json");
const runtime = read("data/runtime/place-open/stortinget.json");
const audit = read("reports/place-production/stortinget-phase1-24-gate-audit-v1.json");
const actors = read("data/brands/actors_by_place.json").stortinget;

test("Stortinget preserves its verified coordinate and has four canonical collections", () => {
  assert.equal(place.lat, 59.91321312337565);
  assert.equal(place.lon, 10.74032524097933);
  ...
  assert.deepEqual(place.objects.map(item => item.id), ["stortinget_stortingslove_ii"]);
  assert.deepEqual(place.productions.map(item => item.id), ["haakon_vii_edsavleggelse_1905"]);
  assert.equal(runtime.people.some(item => item.id === "anna_rogstad"), true);
...
test("collection imagery is local, source-labelled and visually reviewed", () => {
  const brand = actors.find(item => item.id === "stortinget_nasjonalforsamling");
  const person = runtime.people.find(item => item.id === "anna_rogstad");
  ...
  assert.equal(person.imageMeta.license, "CC BY-SA 3.0 Norway");
  assert.match(place.objects[0].imageMeta.note, /faktiske granittskulpturen/);
  assert.match(brand.imageMeta.note, /ingen godkjenning/);
  assert.equal(place.productions[0].imageMeta.date, "1905-11-27");
});

test("description and Politics packets pass with institutional boundaries explicit", () => {
  const packetResult = validatePacket({ packet: production, place, packetFile: "data/places/production/stortinget.json", now: new Date("2026-08-29T12:00:00Z") });
  ...
  const politicsIssues = validatePolitikkPlaceReport({ report: politics, place, canonicalEmneIds, root, now: new Date("2026-08-29T12:00:00Z") });
  assert.deepEqual(politicsIssues, []);
  assert.ok(Object.values(politics.gates).every(gate => gate.status === "PASS"));
  assert.match(place.popupDesc, /Komiteene forbereder sakene/);
  assert.match(place.popupDesc, /Eidsvolls plass foran huset, som er et offentlig byrom/);
});
