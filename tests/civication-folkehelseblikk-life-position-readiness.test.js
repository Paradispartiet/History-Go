...
const streamPath='data/Civication/narratives/leisure/helse_folkehelseblikk.json';
...
assert.equal(row.kind,'public_health_perspective');
assert.equal(stream.storylets.length,14);
assert.equal(new Set(stream.storylets.map(x=>x.id)).size,14);
assert.equal(stream.applies_when.any_tags[0],'helse:folkehelseblikk');
...
console.log('Folkehelseblikk readiness and lifecycle complete ok');
