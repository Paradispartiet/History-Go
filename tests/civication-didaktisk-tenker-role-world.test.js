...
const stream=read('data/Civication/narratives/leisure/utdanning_didaktisk_tenker.json');
const world=read('data/Civication/roleWorlds/utdanning/utdanning_didaktisk_tenker.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'utdanning',id:'didaktisk_tenker',label:'Didaktisk tenker'});
...
const row=audit.positions.find(x=>x.key==='utdanning/didaktisk_tenker');
...
assert.match(world.sociological_core.description,/didactic_reflection/i);
...
assert.ok(world.sociological_core.description.includes('målprioritering'));
assert.ok(world.sociological_core.description.includes('læringsrettet tilbakemelding'));
console.log('Didaktisk tenker Role World gate ok');
