...
const stream=read('data/Civication/narratives/leisure/scenekunst_premieregjenger.json');
const world=read('data/Civication/roleWorlds/scenekunst/scenekunst_premieregjenger.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'scenekunst',id:'premieregjenger',label:'Premieregjenger'});
...
const row=audit.positions.find(x=>x.key==='scenekunst/premieregjenger');
...
assert.ok(world.sociological_core.description.includes('premierehype'));
assert.ok(world.sociological_core.description.includes('førsteinntrykk'));
console.log('Premieregjenger Role World gate ok');
