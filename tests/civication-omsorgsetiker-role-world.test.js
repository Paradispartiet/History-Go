...
const stream=read('data/Civication/narratives/leisure/helse_omsorgsetiker.json');
const world=read('data/Civication/roleWorlds/helse/helse_omsorgsetiker.json');
const index=read('data/Civication/roleWorlds/index.json');
const row=audit.positions.find(x=>x.key==='helse/omsorgsetiker');
assert.ok(row);
assert.equal(world.status,'role_world_complete');
assert.equal(world.subject_type,'life_position');
assert.deepEqual(world.life_position_ref,{badge_id:'helse',id:'omsorgsetiker',label:'Omsorgsetiker'});
...
assert.equal(row.role_world_path,'data/Civication/roleWorlds/helse/helse_omsorgsetiker.json');
assert.equal(row.authored_depth.max_narrative_depth,14);
assert.ok(!audit.queue.some(x=>x.key==='helse/omsorgsetiker'));
...
for(const id of ['care_vs_autonomy','consent_vs_pressure','equality_vs_justice','dignity_vs_efficiency','care_vs_control','care_ethics_vs_authority'])assert.ok(world.primary_threads.some(x=>x.id===id),id);
for(const id of ['handlekraftig_hjelper','egenstemme','systemforvalter','parorendestemme','prinsippdebattant','grensebevisst_fagperson'])assert.ok(world.recurring_people_archetypes.some(x=>x.id===id),id);
assert.match(world.sociological_core.description,/ingen klinisk behandlerrolle/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
console.log('Omsorgsetiker Role World gate ok');
