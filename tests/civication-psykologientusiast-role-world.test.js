...
const stream=read('data/Civication/narratives/leisure/psykologi_psykologientusiast.json');
const world=read('data/Civication/roleWorlds/psykologi/psykologi_psykologientusiast.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'psykologi',id:'psykologientusiast',label:'Psykologientusiast'});
...
const row=audit.positions.find(x=>x.key==='psykologi/psykologientusiast');
...
assert.ok(world.sociological_core.description.includes('korrelasjon mot årsak'));
assert.ok(world.sociological_core.description.includes('replisering'));
console.log('Psykologientusiast Role World gate ok');
