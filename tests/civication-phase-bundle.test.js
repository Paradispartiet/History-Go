...
const runtime={date:new Date().toISOString().slice(0,10),role_scope:"arealplanlegger",current_index:0,items:[
 ...
 {phase:'forenoon',slot:'people_ping',status:'queued',event:{id:'w2',subject:'Kollega',phase_tag:'forenoon'}},
...
(async()=>{
 ...
 assert.strictEqual(b.phase,'forenoon'); assert.strictEqual(b.phaseLabel,'Formiddag'); assert.strictEqual(b.queuedItems.length,2); assert.strictEqual(b.completedCount,1); assert.strictEqual(b.isComplete,false);
 let res=await global.CivicationDailyMailBuilder.enqueuePhaseBundle(global.HG_CiviEngine,{limit:5});
 assert.strictEqual(res.count,2,'opens entire queued phase bundle');
 assert.strictEqual(runtime.items.filter(x=>x.phase==='forenoon'&&x.status==='delivered').length,2);
