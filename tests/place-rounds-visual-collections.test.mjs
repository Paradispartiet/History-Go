...
function make(place,globals={}){const dom=new JSDOM(`<!doctype html><body><div id="placeCard" data-current-place-id="${place.id}"><div class="pc-body"><div class="pc-title-row"><h2 id="pcTitle"></h2></div><div class="pc-icons-quad">${ICONS.map(x=>`<div id="pc${x}Icon" class="pc-round" hidden></div>`).join("")}</div><div id="pcPeopleList"></div><div id="pcBadgesList"></div><div id="pcBrandsList"></div></div></div></body>`,{url:"https://history-go.test/",runScripts:"outside-only"});const w=dom.window;windows.add(w);w.PLACES=[place];Object.assign(w,globals);w.eval(source);w.document.dispatchEvent(new w.Event("DOMContentLoaded",{bubbles:true}));return w;}
const ids=(w,p)=>Array.from(w.HGPlaceRounds.get(p),d=>d.id);

test("canonical pool contains badge plus only approved round types",()=>{const p={id:"x",category:"historie"};const w=make(p);assert.deepEqual(Array.from(w.HGVisualPlaceRounds.ids),["badges","people","objects","brands","map","flora","fauna"]);for(const bad of ["nature","works","details","spots","civication","før_nå","fortellinger","leksikon","play","training","tasks"])assert.ok(!w.HGVisualPlaceRounds.ids.includes(bad),bad);});
test("ordinary places use people objects brands beside frontImage",()=>{const p={id:"x",category:"historie",rounds:["works","nature"]};const w=make(p);assert.deepEqual(ids(w,p),["people","objects","brands"]);assert.equal(w.HGPlaceRounds.badge.id,"badges");});
test("nature places use map flora fauna beside frontImage",()=>{const p={id:"n",category:"natur",people:[{}],objects:[{}]};const w=make(p);assert.deepEqual(ids(w,p),["map","flora","fauna"]);assert.equal(w.HGPlaceRounds.badge.id,"badges");});
