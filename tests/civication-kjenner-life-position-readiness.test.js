...
const streamPath='data/Civication/narratives/leisure/film_tv_kjenner.json';
const worldPath='data/Civication/roleWorlds/film_tv/film_tv_kjenner.json';
...
assert.equal(stream.applies_when.any_tags[0],'film_tv:kjenner');
...
assert.ok(!audit.queue.some(x=>x.key==='film_tv/kjenner'));
...
for(const term of [/Filminteressert/,/Filmfantast/,/Filmnerd/,/Connaisseur/,/Ingen ny runtime/i])assert.match(world.sociological_core.description,term);
...
console.log('Kjenner readiness + Role World ok');
