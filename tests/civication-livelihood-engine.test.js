...
assert.strictEqual(economySnapshot.weeklyLivelihoodProjectedNet, 4, 'economy snapshot exposes livelihood projection');
assert.strictEqual(economySnapshot.estimatedNetAfterHome, 14, 'economy snapshot combines formal and livelihood projection');

// Unemployment-only support pauses while formally employed, rather than deleting itself or changing job state.
env = setup({ employed: true });
result = env.api.createOpportunity({
  id: 'stottevedtak_1',
...
// Occasional income may be zero in a week, but stays deterministic and auditable.
env = setup();
result = env.api.createOpportunity({
  id: 'royalty_1',
