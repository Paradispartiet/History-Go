...
const stream=read('data/Civication/narratives/leisure/natur_sanker.json');
const world=read('data/Civication/roleWorlds/natur/natur_sanker.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'natur',id:'sanker',label:'Sanker'});
...
const row=audit.positions.find(x=>x.key==='natur/sanker');
...
assert.match(world.sociological_core.description,/ingen automatisk myndighet som botaniker/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
console.log('Sanker Role World gate ok');
