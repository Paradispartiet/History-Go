...
const worldPath='data/Civication/roleWorlds/filosofi/filosofi_nysgjerrig.json';
const narrativePath='data/Civication/narratives/leisure/filosofi_nysgjerrig.json';
...
assert.equal(world.role_scope,'filosofi_nysgjerrig');
assert.equal(world.subject_type,'life_position');
assert.deepEqual(world.life_position_ref,{badge_id:'filosofi',id:'nysgjerrig',label:'Nysgjerrig'});
assert.equal(world.status,'role_world_complete');
assert.equal(world.materialization?.no_new_runtime,true);
assert.match(world.sociological_core.description,/exploration_interest/i);
assert.match(world.sociological_core.description,/ingen automatisk jobb|ingen automatisk.*lønn/i);
assert.match(world.sociological_core.description,/forskerstilling|grad|profesjonskvalifikasjon|ekspertmyndighet/i);
...
assert.deepEqual(themeBank.reference_profiles['filosofi/filosofi_nysgjerrig'],world.theme_ids);
const entry=index.roles.find(x=>x.life_position_key==='filosofi/nysgjerrig');
assert.ok(entry);
assert.equal(entry.role_scope,'filosofi_nysgjerrig');
...
console.log('Nysgjerrig Role World gate ok: 14 storylets / 56 beats / 14 threads / 6 people / 5 aftermath / 6 delayed');
