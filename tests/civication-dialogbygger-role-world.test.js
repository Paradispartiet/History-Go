...
const stream=read('data/Civication/narratives/leisure/religion_dialogbygger.json');
const world=read('data/Civication/roleWorlds/religion/religion_dialogbygger.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'religion',id:'dialogbygger',label:'Dialogbygger'});
...
const row=audit.positions.find(x=>x.key==='religion/dialogbygger');
...
assert.ok(world.sociological_core.description.includes('uenighet'));
assert.ok(world.sociological_core.description.includes('representasjon'));
console.log('Dialogbygger Role World gate ok');
