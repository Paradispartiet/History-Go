...
const stream=read('data/Civication/narratives/leisure/filosofi_etiker.json');
const world=read('data/Civication/roleWorlds/filosofi/filosofi_etiker.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'filosofi',id:'etiker',label:'Etiker'});
...
const row=audit.positions.find(x=>x.key==='filosofi/etiker');
...
assert.match(world.sociological_core.description,/ethics_specialization/i);
assert.match(world.sociological_core.description,/ingen automatisk fagdommerrolle/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('irreversibel skade'));
assert.ok(world.sociological_core.description.includes('presedens'));
console.log('Etiker Role World gate ok');
