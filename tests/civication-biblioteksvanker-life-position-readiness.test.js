...
const streamPath='data/Civication/narratives/leisure/litteratur_biblioteksvanker.json';
...
assert.equal(row.kind,'place_identity');
assert.equal(stream.storylets.length,14);
assert.equal(new Set(stream.storylets.map(x=>x.id)).size,14);
assert.equal(stream.applies_when.any_tags[0],'litteratur:biblioteksvanker');
...
console.log('Biblioteksvanker readiness and lifecycle complete ok');
