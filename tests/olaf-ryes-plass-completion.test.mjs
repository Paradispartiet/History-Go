...
test("four PlaceCard collections have honest own-place content", () => {
  ...
  assert.ok(place.objects.every(item => item.physicalObject && item.placeSpecific && item.source_urls.length >= 2));
  assert.deepEqual(place.related_place_ids, ["birkelunden", "sofienbergparken", "markveien", "daelenenga_idrettspark"]);
  assert.equal(Object.hasOwn(brands, "olaf_ryes_plass"), false);
...
test("place-open exposes every phase 8–19 surface", () => {
  ...
  assert.deepEqual(runtime.brands, []);
