...
const stream=read('data/Civication/narratives/leisure/scenekunst_improentusiast.json');
const world=read('data/Civication/roleWorlds/scenekunst/scenekunst_improentusiast.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'scenekunst',id:'improentusiast',label:'Improentusiast'});
...
const row=audit.positions.find(x=>x.key==='scenekunst/improentusiast');
...
assert.match(world.sociological_core.description,/ingen automatisk myndighet som skuespiller/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('sceneangst'));
assert.ok(world.sociological_core.description.includes('privat materiale'));
console.log('Improentusiast Role World gate ok');
