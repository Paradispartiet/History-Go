...
test('stedskuratering lagrer bare id-er og stedsspesifikt innhold',()=>{const link=registry.placeLinks.regjeringskvartalet;assert.ok(link.emneIds.length>=6);assert.ok(link.lenses.length>=4);assert.ok(link.guidingQuestions.length>=4);assert.equal(Object.hasOwn(link,'concepts'),false);assert.equal(Object.hasOwn(link,'chapters'),false);});
