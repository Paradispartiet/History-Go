...
const streamPath = 'data/Civication/narratives/leisure/media_bidragsyter.json';
const worldPath = 'data/Civication/roleWorlds/media/media_bidragsyter.json';
const stream = read(streamPath);

const tier = badge.tiers.find((entry) => entry.life_position?.id === 'bidragsyter');
assert.ok(tier);
assert.equal(tier.threshold, 25);
assert.equal(tier.life_position.kind, 'contributor_practice');
assert.equal(tier.life_position.employment_independent, true);

assert.equal(stream.schema, 'civication_narrative_stream_v1');
assert.equal(stream.id, 'media_bidragsyter_stream');
...
const row = audit.positions.find((entry) => entry.key === 'media/bidragsyter');
assert.ok(row);
...
if (row.role_world_status === 'role_world_not_started') {
  ...
  assert.equal(audit.first_ready?.key, 'media/bidragsyter');
  ...
  const indexed = index.roles.find((entry) => entry.life_position_key === 'media/bidragsyter');
  assert.ok(indexed);
  assert.equal(indexed.role_scope, 'media_bidragsyter');
}

const serialized = JSON.stringify(stream);
for (const forbidden of ['career_offer', 'fixed_salary', 'editorial_authority']) {
  assert.ok(!serialized.includes(forbidden), `Bidragsyter stream must not claim ${forbidden}`);
}

console.log(`Bidragsyter readiness gate ok: ready + ${row.role_world_status}`);
