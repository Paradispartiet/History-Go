...
const artist=audit.positions.find(x=>x.key==='musikk/artist');assert.ok(artist);assert.equal(artist.classification,'ready');assert.ok(!(artist.evidence.thematic_source_refs||[]).includes(streamPath),streamPath+' leaked thematic into musikk/artist');
