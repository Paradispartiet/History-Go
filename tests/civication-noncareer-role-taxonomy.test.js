...
assert.equal(roleWorldIndex.roles.length, 105, 'Role World-indeksen skal ha 85 karriereverdener + nitten life-position worlds');
...
assert.equal(lifePositionWorlds.length, 20, 'nitten canonical life-position worlds skal være materialisert, inkludert Uteligger');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 105, career_role_worlds: 85, life_position_role_worlds: 20 });
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 20 life-position worlds / layers remain separate');
