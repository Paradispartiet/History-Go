...
assert.equal(roleWorldIndex.roles.length, 88, 'Role World-indeksen skal ha 85 karriereverdener + tre life-position worlds');
...
assert.equal(lifePositionWorlds.length, 3, 'Supporter, Nabolagskjenner og Filmklubbmenneske skal være materialisert som life-position worlds');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 88, career_role_worlds: 85, life_position_role_worlds: 3 });
...
console.log('civication non-career role taxonomy ok: 200 selectable life positions / 85 career Role Worlds + 3 life-position worlds / layers remain separate');
