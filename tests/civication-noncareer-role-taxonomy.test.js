...
assert.equal(roleWorldIndex.roles.length, 123, 'Role World-indeksen skal ha 85 karriereverdener + trettiåtte life-position worlds');
...
assert.equal(lifePositionWorlds.length, 38, 'trettiåtte canonical life-position worlds skal være materialisert, inkludert Arkivrotte');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 123, career_role_worlds: 85, life_position_role_worlds: 38 });
...
const expectedCounts = {
  ...
  life_position_role_worlds: 38,
  total_role_worlds: 123,
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 38 life-position worlds / layers remain separate');
