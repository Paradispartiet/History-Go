...
assert.equal(roleWorldIndex.roles.length, 104, 'Role World-indeksen skal ha 85 karriereverdener + nitten life-position worlds');
...
assert.equal(lifePositionWorlds.length, 19, 'nitten canonical life-position worlds skal være materialisert, inkludert Uteligger');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 104, career_role_worlds: 85, life_position_role_worlds: 19 });
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 19 life-position worlds / layers remain separate');
