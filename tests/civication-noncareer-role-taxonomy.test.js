...
assert.equal(roleWorldIndex.roles.length, 179, 'Role World-indeksen skal ha 85 karriereverdener + 94 life-position worlds');
...
assert.equal(lifePositionWorlds.length, 94, '94 canonical life-position worlds skal være materialisert, inkludert Aktiv leser');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 179, career_role_worlds: 85, life_position_role_worlds: 94 });
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 94 life-position worlds / layers remain separate');
