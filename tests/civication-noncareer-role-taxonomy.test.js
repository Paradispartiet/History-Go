...
assert.equal(roleWorldIndex.roles.length, 106, 'Role World-indeksen skal ha 85 karriereverdener + tjueén life-position worlds');
...
assert.equal(lifePositionWorlds.length, 21, 'tjueén canonical life-position worlds skal være materialisert, inkludert Frilanser');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 106, career_role_worlds: 85, life_position_role_worlds: 21 });
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 21 life-position worlds / layers remain separate');
