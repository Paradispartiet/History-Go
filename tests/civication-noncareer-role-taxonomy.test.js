...
assert.equal(roleWorldIndex.roles.length, 109, 'Role World-indeksen skal ha 85 karriereverdener + tjuefire life-position worlds');
...
assert.equal(lifePositionWorlds.length, 24, 'tjuefire canonical life-position worlds skal være materialisert, inkludert Festivalgjenger');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 109, career_role_worlds: 85, life_position_role_worlds: 24 });
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 24 life-position worlds / layers remain separate');
