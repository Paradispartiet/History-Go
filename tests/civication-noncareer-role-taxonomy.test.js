...
assert.equal(roleWorldIndex.roles.length, 237, 'Role World-indeksen skal ha 85 karriereverdener + 152 life-position worlds');
...
assert.equal(lifePositionWorlds.length, 152, '152 canonical life-position worlds skal være materialisert, nå også Næringsliv Gründer som egen employment-independent entrepreneurial practice and self-employment');
...
assert.deepEqual(lifeWorldByKey.get('subkultur/dandy').life_position_ref, { badge_id: 'subkultur', id: null, label: 'Dandy' });
assert.equal(lifeWorldByKey.get('subkultur/dandy').role_scope, 'subkultur_dandy');
assert.deepEqual(lifeWorldByKey.get('subkultur/deltaker').life_position_ref, { badge_id: 'subkultur', id: null, label: 'Deltaker' });
assert.equal(lifeWorldByKey.get('subkultur/deltaker').role_scope, 'subkultur_deltaker');
assert.deepEqual(lifeWorldByKey.get('subkultur/gatesmart').life_position_ref, { badge_id: 'subkultur', id: null, label: 'Gatesmart' });
assert.equal(lifeWorldByKey.get('subkultur/gatesmart').role_scope, 'subkultur_gatesmart');
assert.deepEqual(lifeWorldByKey.get('subkultur/hakkekylling').life_position_ref, { badge_id: 'subkultur', id: null, label: 'Hakkekylling' });
assert.equal(lifeWorldByKey.get('subkultur/hakkekylling').role_scope, 'subkultur_hakkekylling');
assert.deepEqual(lifeWorldByKey.get('subkultur/observ_r').life_position_ref, { badge_id: 'subkultur', id: null, label: 'Observør' });
assert.equal(lifeWorldByKey.get('subkultur/observ_r').role_scope, 'subkultur_observor');
...
assert.deepEqual(lifeWorldByKey.get('sport/konkurranseutover').life_position_ref, { badge_id: 'sport', id: 'konkurranseutover', label: 'Konkurranseutøver' });
assert.equal(lifeWorldByKey.get('sport/konkurranseutover').role_scope, 'sport_konkurranseutover');
assert.deepEqual(lifeWorldByKey.get('sport/mosjonist').life_position_ref, { badge_id: 'sport', id: 'mosjonist', label: 'Mosjonist' });
assert.equal(lifeWorldByKey.get('sport/mosjonist').role_scope, 'sport_mosjonist');
assert.deepEqual(lifeWorldByKey.get('sport/profesjonell_utover').life_position_ref, { badge_id: 'sport', id: 'profesjonell_utover', label: 'Profesjonell utøver' });
assert.equal(lifeWorldByKey.get('sport/profesjonell_utover').role_scope, 'sport_profesjonell_utover');
...
assert.deepEqual(lifeWorldByKey.get('filosofi/filosofistudent').life_position_ref, { badge_id: 'filosofi', id: 'filosofistudent', label: 'Filosofistudent' });
assert.equal(lifeWorldByKey.get('filosofi/filosofistudent').role_scope, 'filosofi_filosofistudent');
...
assert.deepEqual(lifeWorldByKey.get('historie/student_historie').life_position_ref, { badge_id: 'historie', id: 'student_historie', label: 'Student' });
assert.equal(lifeWorldByKey.get('historie/student_historie').role_scope, 'historie_student_historie');
...
assert.deepEqual(lifeWorldByKey.get('kunst/kuratorpraksis').life_position_ref, { badge_id: 'kunst', id: 'kuratorpraksis', label: 'Kurator' });
...
assert.deepEqual(lifeWorldByKey.get('naeringsliv/bedriftseier').life_position_ref, { badge_id: 'naeringsliv', id: 'bedriftseier', label: 'Bedriftseier' });
assert.equal(lifeWorldByKey.get('naeringsliv/bedriftseier').role_scope, 'naeringsliv_bedriftseier');
assert.deepEqual(lifeWorldByKey.get('naeringsliv/grunder').life_position_ref, { badge_id: 'naeringsliv', id: 'grunder', label: 'Gründer' });
assert.equal(lifeWorldByKey.get('naeringsliv/grunder').role_scope, 'naeringsliv_grunder');
...
assert.deepEqual(lifeWorldByKey.get('naeringsliv/sideprosjektbygger').life_position_ref, { badge_id: 'naeringsliv', id: 'sideprosjektbygger', label: 'Sideprosjektbygger' });
assert.equal(lifeWorldByKey.get('naeringsliv/sideprosjektbygger').role_scope, 'naeringsliv_sideprosjektbygger');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 237, career_role_worlds: 85, life_position_role_worlds: 152 });
...
const expectedCounts = {
  ...
  life_position_role_worlds: 152,
  total_role_worlds: 237,
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 152 life-position worlds / layers remain separate');
