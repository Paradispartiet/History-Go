...
assert.equal(roleWorldIndex.roles.length, 96, 'Role World-indeksen skal ha 85 karriereverdener + elleve life-position worlds');
...
assert.equal(lifePositionWorlds.length, 11, 'elleve canonical life-position worlds skal være materialisert, inkludert Flanør');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 96, career_role_worlds: 85, life_position_role_worlds: 11 });
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 11 life-position worlds / layers remain separate');
