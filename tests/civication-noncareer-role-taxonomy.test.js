...
assert.equal(roleWorldIndex.roles.length, 97, 'Role World-indeksen skal ha 85 karriereverdener + tolv life-position worlds');
...
assert.equal(lifePositionWorlds.length, 12, 'tolv canonical life-position worlds skal være materialisert, inkludert Flanør og Scenekunst-Scenehenger');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 97, career_role_worlds: 85, life_position_role_worlds: 12 });
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 12 life-position worlds / layers remain separate');
