...
assert.equal(roleWorldIndex.roles.length, 108, 'Role World-indeksen skal ha 85 karriereverdener + tjuetre life-position worlds');
...
assert.equal(lifePositionWorlds.length, 23, 'tjuetre canonical life-position worlds skal være materialisert, inkludert Urbanist');
...
assert.deepEqual(roleWorldIndex.summary, { role_worlds_total: 108, career_role_worlds: 85, life_position_role_worlds: 23 });
...
console.log('civication non-career role taxonomy ok: 199 selectable life positions / 85 career Role Worlds + 23 life-position worlds / layers remain separate');
