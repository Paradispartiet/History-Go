...
const streamPath='data/Civication/narratives/leisure/politikk_organisasjonsmenneske.json';
const stream=read(streamPath),row=audit.positions.find(x=>x.key==='politikk/organisasjonsmenneske');
...
assert.equal(stream.applies_when.any_tags[0],'politikk:organisasjonsmenneske');
assert.equal(row.classification,'ready');assert.equal(row.role_world_status,'role_world_complete');
assert.equal(row.role_world_path,'data/Civication/roleWorlds/politikk/politikk_organisasjonsmenneske.json');assert.equal(row.priority_score,405);
...
console.log('Organisasjonsmenneske readiness and lifecycle complete ok');
