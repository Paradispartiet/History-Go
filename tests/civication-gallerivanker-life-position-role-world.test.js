...
const worldPath = 'data/Civication/roleWorlds/kunst/kunst_gallerivanker.json';
const narrativePath = 'data/Civication/narratives/leisure/gallerivanker.json';
...
assert.equal(world.role_scope, 'kunst_gallerivanker');
...
assert.equal(world.title, 'Gallerivanker');
assert.equal(world.status, 'role_world_complete');
assert.equal(world.materialization?.no_new_runtime, true);
assert.match(world.sociological_core.description, /ikke.*kurator|kurator/i);
assert.match(world.sociological_core.description, /gallerist|kunsthandler|formidler|konservator/i);
...
function verifyNarrativeRef(reference) {
  ...
  assert.ok(storyIds.has(reference.slice(prefix.length)), 'missing Gallerivanker anchor ' + reference);
}
...
assert.deepEqual(
  themeBank.reference_profiles['kunst/kunst_gallerivanker'],
  world.theme_ids
);
...
const coordinator = world.recurring_people_archetypes.find((row) => row.id === 'praktisk_apningskoordinator');
assert.ok(coordinator);
assert.match(coordinator.class_position, /uten myndighet.*kurator|uten myndighet.*gallerist/i);
assert.match(coordinator.teaches_player, /ikke skaper.*salgs|ikke skaper.*proveniens|ikke skaper.*kurator/i);
...
const entry = index.roles.find((row) => row.life_position_key === 'kunst/gallerivanker');
assert.ok(entry);
assert.equal(entry.category, 'kunst');
assert.equal(entry.role_scope, 'kunst_gallerivanker');
...
const readiness = audit.positions.find((row) => row.key === 'kunst/gallerivanker');
assert.ok(readiness);
...
assert.deepEqual(readiness.evidence.livelihood_templates, ['gallerivanker_apningshjelp']);
...
assert.match(
  ...
  /ikke.*kurator|ikke.*gallerist|ikke.*kunsthandler|formidler|konservator|salgs-|proveniens-|kunstnerisk beslutningsmyndighet/i
);
...
execFileSync(process.execPath, ['tests/civication-gallerivanker-life-position-readiness.test.js'], { cwd: ROOT, stdio: 'pipe' });

console.log('civication Gallerivanker Role World ok: 56/56 coverage / 14 governed anchors / 91 total worlds / no new runtime');
