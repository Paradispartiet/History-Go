...
const streamPath='data/Civication/narratives/leisure/politikk_motesliter.json';
const stream=read(streamPath),row=audit.positions.find(x=>x.key==='politikk/motesliter');
...
assert.equal(stream.applies_when.any_tags[0],'politikk:motesliter');
assert.equal(row.classification,'ready');assert.equal(row.role_world_status,'role_world_complete');
assert.equal(row.role_world_path,'data/Civication/roleWorlds/politikk/politikk_motesliter.json');assert.equal(row.priority_score,405);
...
console.log('Møtesliter readiness and lifecycle complete ok');
