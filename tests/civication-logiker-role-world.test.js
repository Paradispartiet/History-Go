...
const worldPath='data/Civication/roleWorlds/filosofi/filosofi_logiker.json';
const narrativePath='data/Civication/narratives/leisure/filosofi_logiker.json';
...
assert.equal(world.role_scope,'filosofi_logiker');
assert.equal(world.subject_type,'life_position');
assert.deepEqual(world.life_position_ref,{badge_id:'filosofi',id:'logiker',label:'Logiker'});
assert.equal(world.status,'role_world_complete');
assert.equal(world.materialization?.no_new_runtime,true);
assert.match(world.sociological_core.description,/logic_specialization/i);
assert.match(world.sociological_core.description,/ingen automatisk jobb|ingen automatisk.*lønn/i);
assert.match(world.sociological_core.description,/juridisk mandat|medisinsk mandat|forsker.*mandat|institusjonell beslutningsmyndighet/i);
...
assert.deepEqual(themeBank.reference_profiles['filosofi/filosofi_logiker'],world.theme_ids);
const entry=index.roles.find(x=>x.life_position_key==='filosofi/logiker');
assert.ok(entry);
assert.equal(entry.role_scope,'filosofi_logiker');
...
console.log('Logiker Role World gate ok: 14 storylets / 56 beats / 14 threads / 6 people / 5 aftermath / 6 delayed');
