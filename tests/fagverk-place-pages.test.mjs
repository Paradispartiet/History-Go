...
test('stedssiden viser forbindelsen fra undermerke til canonicalt Politikk-fag når stedet faktisk er Politikk',()=>{assert.match(page,/fagverkPlaceBadgePath/);assert.match(page,/politikk-fag-model\.js/);assert.match(page,/fagverk-place-canonical-integration\.js/);assert.match(canonical,/HGPolitikkFagModel\.resolvePlace/);assert.match(canonical,/model\.underbadges/);assert.match(canonical,/model\.domains/);assert.match(canonical,/coverageById/);});
test('Torggata beholder By-identitet og canonicale By-emner på stedets fagverkside',()=>{assert.equal(torggata.category,'by');assert.deepEqual(torggata.emne_ids,['em_by_gentrifisering_eiendom','em_by_styring_forvaltning_planmakt']);assert.ok(torggata.rounds.includes('badges'));for(const id of torggata.emne_ids){assert.equal(registry.emners[id]?.subject,'by');assert.ok(registry.emners[id]?.title);assert.ok(registry.emners[id]?.chapter);}assert.equal(registry.placePage.fallbackSubjectByCategory.by,'by');const byPortal=portal.categories.find((row)=>row.id==='by');assert.equal(byPortal?.subjectStatus,'materialized');assert.equal(byPortal?.subjectPage,'fagverk.html?subject=by');assert.equal(byPortal?.badgePage,'data/fag/by/merke_by.html');});
