...
assert.deepEqual(grammar.work_loops, EXPECTED_LOOPS, 'canonical research loops drifted');
assert.deepEqual(grammar.authority_boundary?.may, EXPECTED_MAY, 'canonical research may-boundary drifted');
assert.deepEqual(grammar.authority_boundary?.may_not, EXPECTED_MAY_NOT, 'canonical research may_not-boundary drifted');
assert.equal(grammar.practice_stories?.length, 5);
assert.equal(grammar.quality_axes?.length, 6);

assert.equal(grammar.day_one_contract?.entry, 'qualification_required');
assert.equal(grammar.day_one_contract?.first_object, 'forskningslogg_og_manusspor');
assert.equal(grammar.persistent_work_object_contract?.id, 'forskningslogg_og_manusspor');
assert.ok(grammar.persistent_work_object_contract?.states?.length >= 10);
assert.match(grammar.persistent_work_object_contract?.handoff_rule || '', /versjon|kilde|motbevis|usikker/i);
assert.ok(grammar.rhythm_contract?.waiting_states?.length >= 5);
assert.match(grammar.rhythm_contract?.loop || '', /waiting|venting/i);
assert.match(grammar.rhythm_contract?.rework_rule || '', /ny kilde|motbevis|fagfelle|etikk|siter/i);
...
assert.deepEqual(new Set(model.related_places.map((place) => place.id)), new Set([
  'forskningsdesign_og_sporsmalsbord',
  'arkiv_og_kildespor',
  'analyse_og_motprovingsflate',
  'fagfelle_og_publiseringsflate'
]));
...
assert.ok(model.required_knowledge?.people_connections?.length === 4);
assert.ok(model.required_knowledge?.place_connections?.length === 4);

const gateText = JSON.stringify({model, grammar}).toLowerCase();
for (const term of ['qualification_required','history go','badge','kildekritikk','historiografi','fagfelle','forskningsetikk','motbevis']) assert.ok(gateText.includes(term), `missing boundary term ${term}`);
for (const term of ['ikke en grad','ikke kvalifikasjon','kan ikke autentisere','kan ikke gi etikk','kan ikke avgjøre']) assert.ok(gateText.includes(term), `missing knowledge boundary ${term}`);
...
for (const term of ['history go','kildekritikk','historiografi','qualification_required']) assert.ok(knowledgeText.includes(term));
for (const term of ['grad','ansett','etikk','publiser','autentiser','funn']) assert.ok(knowledgeText.includes(term));

const manifest = read('data/Civication/roleModels/manifest.json');
assert.ok(manifest.files.includes(MODEL), 'aggregate research role model must be registered');
...
assert.equal(row.audit.components.entry.level, 'complete');
assert.equal(row.audit.components.authority.level, 'complete');
assert.equal(row.audit.components.economy.level, 'complete');
for (const name of ['day_one','workday_loop','people','places','mail','knowledge','consequences','performance','progression','exit']) assert.equal(row.audit.components[name].level, 'complete', name);
assert.equal(row.audit.salary.rows.length, 3);
for (const salary of row.audit.salary.rows) assert.equal(salary.offer_policy, 'qualification_required');
assert.ok(career.summary.runtime_gate_pass >= 57);
assert.ok(career.summary.statuses.playable >= 52);
assert.ok(career.summary.statuses.architecture_only <= 28);
...
const source = fs.readFileSync(path.join(ROOT, 'reports/CIVICATION_HISTORIE_FORSKNING_OG_AKADEMIA_PREREQUISITES_SOURCE_FIRST.md'), 'utf8');
assert.match(source, /Nine runtime gaps/i);
assert.match(source, /qualification_required/);
assert.match(source, /15 source mails/i);
assert.match(source, /not Role World completion/i);
console.log('Civication Historie Forskning og akademia prerequisites: OK');
