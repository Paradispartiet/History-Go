...
const streamPath='data/Civication/narratives/leisure/kunst_vernissagegjenger.json';
...
assert.equal(row.kind,'social_identity');
assert.equal(stream.storylets.length,14);
assert.equal(new Set(stream.storylets.map(x=>x.id)).size,14);
assert.equal(stream.applies_when.any_tags[0],'kunst:vernissagegjenger');
...
console.log('Vernissagegjenger readiness and lifecycle complete ok');
