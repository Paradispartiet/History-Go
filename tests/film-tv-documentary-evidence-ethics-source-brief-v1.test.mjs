...
test('kapitlet forblir uregistrert til fulltekst- og evidensaudit', () => {
  ...
  assert.equal(result.registry.subjects.film_tv.chapters.some((chapter) => chapter.id === 'dokumentar-evidens-og-etikk'), false);
  assert.equal(result.status.subjects.find((row) => row.id === 'film_tv').nextGate, 'documentary_evidence_ethics_source_brief_complete_full_chapter_production');
});
