...
const lifeId='troslivskjenner';
const lifeKey='religion/troslivskjenner';
const lifeScope='religion_troslivskjenner';
const streamPath='data/Civication/narratives/leisure/religion_troslivskjenner.json';
const worldPath='data/Civication/roleWorlds/religion/religion_troslivskjenner.json';

const stream=read(streamPath);
const tier=badge.tiers.find(x=>x.life_position?.id===lifeId);
assert.ok(tier,'Religion Troslivskjenner tier must exist');
assert.equal(tier.life_position.label,'Troslivskjenner');
assert.equal(tier.life_position.kind,'lived_religion_literacy');
...
assert.ok(evidence.canonical_decision.pure_life_or_practice_tiers.includes('Troslivskjenner'));
assert.ok(evidence.salary_mapping.not_salary_jobs.includes('Troslivskjenner'));
...
assert.ok(row,'Religion Troslivskjenner must remain in life-position readiness');
assert.equal(row.classification,'ready','The authored Religion Troslivskjenner stream must make the position ready before Role World production');
...
for(const key of ['religion/nysgjerrig','religion/besokende','religion/dialogbygger','religion/feltarbeider','religion/livssynsutforsker','religion/pilegrim','religion/trosstedsvandrer','religion/ritualkjenner','religion/symboltolker','religion/tradisjonskjenner']){
...
if(!worldExists){
  ...
  assert.equal(indexed,undefined,'Source-depth phase must not pre-register an unbuilt Troslivskjenner Role World');
  const queued=audit.queue.find(x=>x.key===lifeKey);
  assert.ok(queued,'Ready but unbuilt Troslivskjenner must remain in queue');
  assert.equal(queued.rank,1,'Troslivskjenner should become the next canonical ready Role World after authored-depth repair');
  ...
  assert.ok(!audit.queue.some(x=>x.key===lifeKey),'Completed Troslivskjenner must leave the readiness queue');
  assert.ok(indexed,'Completed Troslivskjenner must be indexed');
  assert.equal(indexed.role_scope,lifeScope);
  assert.equal(indexed.status,'role_world_complete');
  assert.equal(world.schema,'civication_role_world_v1');
  assert.equal(world.subject_type,'life_position');
  assert.equal(world.status,'role_world_complete');
  assert.deepEqual(world.life_position_ref,{badge_id:'religion',id:lifeId,label:'Troslivskjenner'});
  ...
  assert.match(world.sociological_core.description,/representasjonsrett|representasjonsmyndighet/);
}
console.log(worldExists
  ? 'Religion Troslivskjenner authored-depth and completed Role World gate ok'
  : 'Religion Troslivskjenner authored-depth gate ok: exact 14-storylet source makes lived-religion literacy ready without employment or representation authority');
