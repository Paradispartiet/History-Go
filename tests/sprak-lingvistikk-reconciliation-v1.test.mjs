...
test('Språk & lingvistikk har felt 1 materialisert og felt 2 Fonetikk source-first', () => {
  const report = audit();
  assert.equal(report.status, 'pass');
  assert.equal(report.domains, 12);
  assert.equal(report.materialized, 1);
  assert.equal(report.sourceFirstReady, 2);
  ...
  assert.equal(report.nextDomain, 'fonetikk_taleproduksjon_akustikk_persepsjon');
});
