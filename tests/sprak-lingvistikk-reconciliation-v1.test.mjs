...
test('Språk & lingvistikk har felt 1-4 materialisert og felt 5 Syntaks source-first', () => {
  const report = audit();
  assert.equal(report.status, 'pass');
  assert.equal(report.domains, 12);
  assert.equal(report.materialized, 4);
  assert.equal(report.sourceFirstReady, 5);
  ...
  assert.equal(report.nextDomain, 'syntaks_setningsstruktur_grammatikk');
});
