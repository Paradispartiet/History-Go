...
const worldPath = 'data/Civication/roleWorlds/scenekunst/scenekunst_scenehenger.json';
const narrativePath = 'data/Civication/narratives/leisure/scenekunst_scenehenger.json';
...
assert.equal(world.category, 'scenekunst');
assert.equal(world.role_scope, 'scenekunst_scenehenger');
assert.equal(world.subject_type, 'life_position');
assert.deepEqual(world.life_position_ref, { badge_id: 'scenekunst', id: 'scenehenger', label: 'Scenehenger' });
assert.equal(world.status, 'role_world_complete');
assert.equal(world.materialization?.no_new_runtime, true);
assert.match(
  world.sociological_core.description,
  /scenetekniker|produsent|regissør|sikkerhetsansvarlig|kunstnerisk beslutningstaker/i
);
...
function verifyNarrativeRef(reference) {
  ...
  assert.ok(storyIds.has(reference.slice(prefix.length)), 'missing Scenekunst Scenehenger anchor ' + reference);
}
...
assert.deepEqual(themeBank.reference_profiles['scenekunst/scenekunst_scenehenger'], world.theme_ids);
...
assert.ok(world.recurring_people_archetypes.some((row) => row.id === 'scenetekniker'));
assert.ok(world.recurring_people_archetypes.some((row) => row.id === 'kunstnerisk_ansvarlig'));
assert.ok(world.recurring_people_archetypes.some((row) => row.id === 'lokal_sceneleder'));
...
const entry = index.roles.find((row) => row.life_position_key === 'scenekunst/scenehenger');
assert.ok(entry);
assert.equal(entry.category, 'scenekunst');
assert.equal(entry.role_scope, 'scenekunst_scenehenger');
...
assert.equal(index.roles.filter((row) => row.subject_type === 'life_position').length, 12);
assert.deepEqual(index.summary, {
  role_worlds_total: 97,
  career_role_worlds: 85,
  life_position_role_worlds: 12
});
assert.equal(index.career_role_world_count, 85);
assert.equal(index.life_position_role_world_count, 12);
assert.equal(index.status, '97_role_worlds_materialized');

assert.ok(taxonomy.role_world_rollout_boundary.completed_life_position_role_worlds.includes('scenekunst/scenehenger'));
assert.equal(taxonomy.role_world_rollout_boundary.completed_life_position_role_worlds.length, 12);
assert.equal(taxonomy.role_world_rollout_boundary.next_source_backed_candidate, null);
assert.equal(taxonomy.canonical_counts.career_role_worlds, 85);
assert.equal(taxonomy.canonical_counts.life_position_role_worlds, 12);
assert.equal(taxonomy.canonical_counts.total_role_worlds, 97);
assert.equal(policy.noncareer_subject_boundary.life_position_readiness.first_source_backed_candidate, null);
assert.equal(policy.noncareer_subject_boundary.life_position_readiness.completed_life_position_role_worlds, 12);
...
assert.deepEqual(readiness.evidence.livelihood_templates, ['scenehenger_riggehjelp']);
...
assert.match(
  ...
  /avgrenset fra lys|lyd|elektriske koblinger|flysystemer|sikkerhetskontroll|sceneledelse/i
);
...
execFileSync(process.execPath, ['tests/civication-scenekunst-scenehenger-life-position-readiness.test.js'], { cwd: ROOT, stdio: 'pipe' });

console.log('civication Scenekunst Scenehenger Role World ok: 56/56 coverage / 14 governed anchors / 97 total worlds / 12 completed life-position worlds');
