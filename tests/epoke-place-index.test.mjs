...
test("generated epoch-place index is deterministic and current", () => {
  ...
  assert.equal(index.stats.canonical_story_milestone_count, 218);
  assert.equal(index.stats.verified_place_production_milestone_count, 392);
});
...
test("Oslo coverage classifies every canonical place exactly once without overstating completeness", () => {
  ...
  assert.equal(coverage.dated_evidence_place_count, 230);
  assert.equal(coverage.documented_case_place_count, 2);
  assert.equal(coverage.awaiting_source_backed_history_count, 355);
