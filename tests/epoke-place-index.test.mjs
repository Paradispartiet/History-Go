...
test("generated epoch-place index is deterministic and current", () => {
  ...
  assert.equal(index.stats.canonical_story_milestone_count, 209);
  assert.equal(index.stats.verified_place_production_milestone_count, 300);
});
...
test("Oslo coverage classifies every canonical place exactly once without overstating completeness", () => {
  ...
  assert.equal(coverage.canonical_place_count, 585);
  assert.equal(coverage.dated_evidence_place_count, 221);
