...
test("generated epoch-place index is deterministic and current", () => {
  ...
  assert.equal(index.stats.canonical_story_milestone_count, 212);
  assert.equal(index.stats.verified_place_production_milestone_count, 324);
});
...
test("Oslo coverage classifies every canonical place exactly once without overstating completeness", () => {
  ...
  assert.equal(coverage.awaiting_source_backed_history_count, 361);
