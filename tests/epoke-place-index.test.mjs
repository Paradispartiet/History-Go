...
test("generated epoch-place index is deterministic and current", () => {
  ...
  assert.equal(index.stats.canonical_story_milestone_count, 177);
  assert.equal(index.stats.verified_place_production_milestone_count, 167);
});
...
test("Oslo coverage classifies every canonical place exactly once without overstating completeness", () => {
  ...
  assert.equal(coverage.dated_evidence_place_count, 211);
  assert.equal(coverage.documented_case_place_count, 2);
  assert.equal(coverage.awaiting_source_backed_history_count, 371);
  for (const placeId of ["markveien", "paulus_kirke", "freia_fabrikken", "arbeidermuseet", "clemenskirken_ruin_oslo", "minneparken_gamlebyen", "saxegarden", "gamlebyen_gravlund", "gamlebyen_kirke", "galgeberg", "kampen_kirke", "kampen_park", "klosterenga_skulpturpark", "sagene", "torshov", "torshovparken", "grorud", "grorudparken"]) {
    assert.equal(
