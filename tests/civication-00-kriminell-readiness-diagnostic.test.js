...
try {
  ...
  const changedPositions = (after.positions || []).filter((row) => JSON.stringify(beforeByKey.get(row.key)) !== JSON.stringify(row)).map((row) => ({ key: row.key, before: beforeByKey.get(row.key) || null, after: row }));
  throw new Error('KRIMINELL_READINESS_DIAGNOSTIC=' + JSON.stringify({
