...
const place = JSON.parse(fs.readFileSync("data/places/by/oslo/places/bankplassen.json", "utf8"));
...
const server = http.createServer((request, response) => {
  ...
  if (pathname === "/__audit__/bankplassen.html") { response.writeHead(200, { "content-type": mime[".html"] }); response.end(fixture); return; }
...
try {
  ...
  await page.goto(`http://127.0.0.1:${port}/__audit__/bankplassen.html`, { waitUntil: "networkidle" });
  ...
  assert.ok((await page.locator("#fagverkPlaceArticle").innerText()).includes("festningsvollene"));
  assert.ok(await page.locator("#fagverkPlaceLenses > *").count() >= 2);
  assert.ok(await page.locator("#fagverkPlaceQuestions li").count() >= 3);
