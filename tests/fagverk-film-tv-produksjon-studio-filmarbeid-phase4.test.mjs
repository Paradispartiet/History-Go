...
test('Film & TV-fagets tekniske baseline er bevart etter kapittel 2', () => {
  const { report } = auditFilmTvProduksjonStudioFilmarbeidPhase4();
  const committed = JSON.parse(fs.readFileSync(path.join(root, 'reports/fagverk/film-tv-produksjon-studio-filmarbeid-phase4-audit.json'), 'utf8'));
  assert.ok(['remaining_domain_chapter_production', 'curriculum_completeness_refactor', 'canonical_inventory_migration', 'canonical_inventory_migrated_existing_chapter_reaudit', 'canonical_chapter_reaudit_complete_learning_order_plan', 'learning_order_plan_complete_first_chapter_source_brief'].includes(report.subject.nextGate)
    || /(?:source_brief_complete_full_chapter_production|full_chapter_complete_next_unit_source_brief)$/.test(report.subject.nextGate));
