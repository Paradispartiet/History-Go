...
const stream=read('data/Civication/narratives/leisure/helse_helseutforsker.json');
const world=read('data/Civication/roleWorlds/helse/helse_helseutforsker.json');
const index=read('data/Civication/roleWorlds/index.json');
const row=audit.positions.find(x=>x.key==='helse/helseutforsker');
assert.ok(row);
assert.equal(world.status,'role_world_complete');
assert.equal(world.subject_type,'life_position');
assert.deepEqual(world.life_position_ref,{badge_id:'helse',id:'helseutforsker',label:'Helseutforsker'});
...
for(const id of ['anatomy_vs_diagnosis','variation_vs_pathology','measurement_vs_meaning','learning_vs_treatment_authority','screening_vs_diagnosis','health_learning_vs_authority'])assert.ok(world.primary_threads.some(x=>x.id===id),id);
for(const id of ['spørrelysten_venn','fagtekstformidleren','måleentusiasten','erfaringsstemmen','autoritetssikre_debattant','grensebevisst_fagperson'])assert.ok(world.recurring_people_archetypes.some(x=>x.id===id),id);
assert.match(world.sociological_core.description,/ingen diagnosemyndighet/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
console.log('Helseutforsker Role World gate ok');
