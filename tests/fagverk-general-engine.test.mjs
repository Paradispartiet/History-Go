...
test('materialisert fagside og committed fase-1-rapport passerer full audit', () => {
  ...
  assert.equal(media.chapterCount, 6);
  ...
  assert.equal(religion.methodCount, 26);
  ...
  assert.equal(filmTv.domainCount, 10);
  assert.equal(filmTv.emneCount, 192);
  assert.equal(filmTv.methodCount, 119);
  assert.equal(filmTv.mappingCount, 192);
  assert.equal(filmTv.hookCount, 192);
  ...
  assert.ok(filmTv.chapterCount >= 1);
