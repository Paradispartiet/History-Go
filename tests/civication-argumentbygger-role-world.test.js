...
const stream=read('data/Civication/narratives/leisure/filosofi_argumentbygger.json');
const world=read('data/Civication/roleWorlds/filosofi/filosofi_argumentbygger.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'filosofi',id:'argumentbygger',label:'Argumentbygger'});
...
const row=audit.positions.find(x=>x.key==='filosofi/argumentbygger');
...
assert.match(world.sociological_core.description,/argumentation_practice/i);
assert.match(world.sociological_core.description,/ingen automatisk ekspertstatus/i);
...
assert.ok(world.sociological_core.description.includes('bevisbyrde'));
console.log('Argumentbygger Role World gate ok');
