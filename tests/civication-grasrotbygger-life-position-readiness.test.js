...
const streamPath='data/Civication/narratives/leisure/politikk_grasrotbygger.json';
const stream=read(streamPath),row=audit.positions.find(x=>x.key==='politikk/grasrotbygger');
...
assert.equal(stream.applies_when.any_tags[0],'politikk:grasrotbygger');
assert.equal(row.classification,'ready');assert.equal(row.role_world_status,'role_world_complete');
assert.equal(row.role_world_path,'data/Civication/roleWorlds/politikk/politikk_grasrotbygger.json');assert.equal(row.priority_score,405);
...
console.log('Grasrotbygger readiness and lifecycle complete ok');
