...
const coverage = new Set(world.season.coverage.map((row) => String(row.day) + '/' + row.phase));
assert.equal(coverage.size, 56);
for (let day = 1; day <= 14; day += 1) {
  for (const phase of phases) {
    assert.ok(coverage.has(String(day) + '/' + phase), 'missing Role World coverage ' + day + '/' + phase);
  }
}
...
function verifyNarrativeRef(reference) {
  ...
  assert.ok(storyIds.has(anchor), 'missing Filmklubbmenneske narrative anchor ' + anchor);
}
...
assert.deepEqual(themeBank.reference_profiles['film_tv/film_tv_filmklubbmenneske'], world.theme_ids,
  'Filmklubbmenneske theme profile must match completed Role World');
assert.ok(checklist.reference_worlds.includes(worldPath),
  'Filmklubbmenneske Role World must be registered in authoring checklist');
...
for (const thread of world.primary_threads) {
  ...
  for (const beatRef of thread.beat_refs) assert.ok(coverage.has(beatRef), thread.id + ' missing coverage ref ' + beatRef);
}
for (const beat of world.season.coverage) {
  for (const threadId of beat.thread_ids) assert.ok(threadIds.has(threadId), 'unknown thread id ' + threadId);
}
...
assert.ok(entry, 'Filmklubbmenneske Role World must be indexed');
