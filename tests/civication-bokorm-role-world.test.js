...
const stream=read('data/Civication/narratives/leisure/litteratur_bokorm.json');
const world=read('data/Civication/roleWorlds/litteratur/litteratur_bokorm.json');
const index=read('data/Civication/roleWorlds/index.json');
const row=audit.positions.find(x=>x.key==='litteratur/bokorm');
assert.ok(row);
assert.equal(world.status,'role_world_complete');
assert.equal(world.subject_type,'life_position');
assert.deepEqual(world.life_position_ref,{badge_id:'litteratur',id:'bokorm',label:'Bokorm'});
...
assert.equal(row.role_world_path,'data/Civication/roleWorlds/litteratur/litteratur_bokorm.json');
assert.ok(!audit.queue.some(x=>x.key==='litteratur/bokorm'));
...
for(const id of ['reading_pleasure_vs_completion_debt','attention_vs_identity_test','ownership_vs_book_culture','book_buying_vs_economic_room','reader_identity_vs_authority','reading_metrics_vs_status','bookworm_practice_vs_professional_authority'])assert.ok(world.primary_threads.some(x=>x.id===id),id);
for(const id of ['lesevennen','bokhandleren','bibliotekaren','den_hurtige_leseren','den_langsomme_leseren','den_kanonbevisste_vennen'])assert.ok(world.recurring_people_archetypes.some(x=>x.id===id),id);
...
console.log('Bokorm Role World gate ok');
