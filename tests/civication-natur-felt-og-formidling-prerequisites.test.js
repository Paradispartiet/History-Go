...
const KEY = 'natur/natur_felt_og_formidling';
const ROLE = 'natur_felt_og_formidling';
const MODEL = 'data/Civication/roleModels/natur/natur_felt_og_formidling.json';
const GRAMMAR = 'data/Civication/workGrammars/natur/natur_felt_og_formidling.json';
const PLAN = 'data/Civication/mailPlans/natur/natur_felt_og_formidling_plan.json';
...
const ACTORS = [
  'solveig_feltleder_natur_felt_og_formidling',
  'amir_artsfaglig_kvalitetssikrer_natur_felt_og_formidling',
  'nora_naturveileder_besoksansvarlig_natur_felt_og_formidling',
  'jonas_hms_logistikk_natur_felt_og_formidling'
];
const PLACES = [
  'feltbrief_og_utstyrsbase_natur',
  'observasjonsflate_og_provetakingspunkt_natur',
  'besokssenter_og_formidlingspunkt_natur',
  'kvalitetssjekk_og_handoffbord_natur'
];
...
assert.match(grammar.rhythm_contract.rework_rule, /observasjon|metadata|HMS|vær|formidling/i);
...
assert.deepEqual(policy.Feltassistent, {policy:'direct'});
assert.deepEqual(policy.Naturveileder, {policy:'qualification_required',qualification_ids:['relevant_education_or_employer_qualification']});
...
assert.equal(offers.Feltassistent.policy, 'direct');
assert.equal(offers.Naturveileder.policy, 'qualification_required');
assert.deepEqual(offers.Naturveileder.qualification_ids, ['relevant_education_or_employer_qualification']);

assert.equal(plan.schema, 'civication_mail_plan_v1');
assert.equal(plan.id, 'natur_felt_og_formidling_foundation_v1');
...
for (const term of ['relevant_education_or_employer_qualification','history go','natur-badge','hms','observasjon','usikkerhet','sårbar','forvaltningsvedtak','politisk']) {
  assert.ok(boundary.includes(term), term);
}
const source = fs.readFileSync(path.join(ROOT, SOURCE), 'utf8');
for (const term of [/not Role World completion/i,/Feltassistent.*direct/i,/Naturveileder.*qualification_required/i,/15 source mails/i,/candidate_when_shared_work_is_real/i,/History Go/i,/No new runtime/i]) {
...
console.log('PASS: Natur Felt og formidling foundation is playable and rollout-ready while bounded audience standing remains reserved for Role World authoring.');
