...
const stream=read('data/Civication/narratives/leisure/religion_livssynsutforsker.json');
const world=read('data/Civication/roleWorlds/religion/religion_livssynsutforsker.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'religion',id:'livssynsutforsker',label:'Livssynsutforsker'});
...
const row=audit.positions.find(x=>x.key==='religion/livssynsutforsker');
...
assert.match(world.sociological_core.description,/knowledge_identity/i);
assert.match(world.sociological_core.description,/ingen automatisk myndighet som trosrepresentant/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('lære mot levd praksis'));
assert.ok(world.sociological_core.description.includes('sammenligning uten rangering'));
console.log('Livssynsutforsker Role World gate ok');
