...
assert.deepEqual(audit.summary.classifications, {
  ready: 85,
  needs_authored_depth: 74,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 85);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 14);
assert.equal(audit.summary.positions_with_exact_governed_sources, 85);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 85);
...
console.log('civication life-position Role World readiness v2 ok: 85 ready / 74 authored-depth / 40 not-standalone; 85 complete / no pending-ready');
