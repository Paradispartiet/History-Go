...
assert.deepEqual(audit.summary.classifications, {
  ready: 125,
  needs_authored_depth: 34,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 125);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 15);
assert.equal(audit.summary.positions_with_exact_governed_sources, 125);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 125);
...
console.log('civication life-position Role World readiness v2 ok: 125 ready / 34 authored-depth / 40 not-standalone; 125 complete / no pending-ready');
