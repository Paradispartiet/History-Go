...
assert.deepEqual(audit.summary.classifications, {
  ready: 90,
  needs_authored_depth: 69,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 90);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 14);
assert.equal(audit.summary.positions_with_exact_governed_sources, 90);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 90);
...
console.log('civication life-position Role World readiness v2 ok: 90 ready / 69 authored-depth / 40 not-standalone; 90 complete / no pending-ready');
