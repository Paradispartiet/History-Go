...
assert.deepEqual(audit.summary.classifications, {
  ready: 129,
  needs_authored_depth: 30,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 129);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 15);
assert.equal(audit.summary.positions_with_exact_governed_sources, 129);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 129);
...
const klasseromsobservator = audit.positions.find((row) => row.key === 'utdanning/klasseromsobservator');
assert.ok(klasseromsobservator);
assert.equal(klasseromsobservator.classification, 'ready');
assert.equal(klasseromsobservator.role_world_status, 'role_world_complete');
assert.equal(klasseromsobservator.role_world_path, 'data/Civication/roleWorlds/utdanning/utdanning_klasseromsobservator.json');
assert.equal(klasseromsobservator.priority_score, 405);
assert.equal(klasseromsobservator.authored_depth.exact_source_ref_count, 1);
assert.equal(klasseromsobservator.authored_depth.max_narrative_depth, 14);
assert.deepEqual(klasseromsobservator.evidence.exact_source_refs, ['data/Civication/narratives/leisure/utdanning_klasseromsobservator.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'utdanning/klasseromsobservator'));
const laeringsutforsker = audit.positions.find((row) => row.key === 'utdanning/laeringsutforsker');
assert.ok(laeringsutforsker);
assert.equal(laeringsutforsker.classification, 'ready');
assert.equal(laeringsutforsker.role_world_status, 'role_world_complete');
assert.equal(laeringsutforsker.role_world_path, 'data/Civication/roleWorlds/utdanning/utdanning_laeringsutforsker.json');
assert.equal(laeringsutforsker.priority_score, 405);
assert.equal(laeringsutforsker.authored_depth.exact_source_ref_count, 1);
assert.equal(laeringsutforsker.authored_depth.max_narrative_depth, 14);
assert.deepEqual(laeringsutforsker.evidence.exact_source_refs, ['data/Civication/narratives/leisure/utdanning_laeringsutforsker.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'utdanning/laeringsutforsker'));
const utdanningskritiker = audit.positions.find((row) => row.key === 'utdanning/utdanningskritiker');
assert.ok(utdanningskritiker);
assert.equal(utdanningskritiker.classification, 'ready');
assert.equal(utdanningskritiker.role_world_status, 'role_world_complete');
assert.equal(utdanningskritiker.role_world_path, 'data/Civication/roleWorlds/utdanning/utdanning_utdanningskritiker.json');
assert.equal(utdanningskritiker.priority_score, 405);
assert.equal(utdanningskritiker.authored_depth.exact_source_ref_count, 1);
assert.equal(utdanningskritiker.authored_depth.max_narrative_depth, 14);
assert.deepEqual(utdanningskritiker.evidence.exact_source_refs, ['data/Civication/narratives/leisure/utdanning_utdanningskritiker.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'utdanning/utdanningskritiker'));
const folkeforsker = audit.positions.find((row) => row.key === 'vitenskap/folkeforsker');
assert.ok(folkeforsker);
assert.equal(folkeforsker.classification, 'ready');
assert.equal(folkeforsker.role_world_status, 'role_world_complete');
assert.equal(folkeforsker.role_world_path, 'data/Civication/roleWorlds/vitenskap/vitenskap_folkeforsker.json');
assert.equal(folkeforsker.priority_score, 405);
assert.equal(folkeforsker.authored_depth.exact_source_ref_count, 1);
assert.equal(folkeforsker.authored_depth.max_narrative_depth, 14);
assert.deepEqual(folkeforsker.evidence.exact_source_refs, ['data/Civication/narratives/leisure/vitenskap_folkeforsker.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'vitenskap/folkeforsker'));
const labnerd = audit.positions.find((row) => row.key === 'vitenskap/labnerd');
assert.ok(labnerd);
assert.equal(labnerd.classification, 'ready');
assert.equal(labnerd.role_world_status, 'role_world_complete');
assert.equal(labnerd.role_world_path, 'data/Civication/roleWorlds/vitenskap/vitenskap_labnerd.json');
assert.equal(labnerd.priority_score, 405);
assert.equal(labnerd.authored_depth.exact_source_ref_count, 1);
assert.equal(labnerd.authored_depth.max_narrative_depth, 14);
assert.deepEqual(labnerd.evidence.exact_source_refs, ['data/Civication/narratives/leisure/vitenskap_labnerd.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'vitenskap/labnerd'));
const stjernekikker = audit.positions.find((row) => row.key === 'vitenskap/stjernekikker');
assert.ok(stjernekikker);
assert.equal(stjernekikker.classification, 'ready');
assert.equal(stjernekikker.role_world_status, 'role_world_complete');
assert.equal(stjernekikker.role_world_path, 'data/Civication/roleWorlds/vitenskap/vitenskap_stjernekikker.json');
assert.equal(stjernekikker.priority_score, 405);
assert.equal(stjernekikker.authored_depth.exact_source_ref_count, 1);
assert.equal(stjernekikker.authored_depth.max_narrative_depth, 14);
assert.deepEqual(stjernekikker.evidence.exact_source_refs, ['data/Civication/narratives/leisure/vitenskap_stjernekikker.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'vitenskap/stjernekikker'));
const teknologibygger = audit.positions.find((row) => row.key === 'vitenskap/teknologibygger');
assert.ok(teknologibygger);
assert.equal(teknologibygger.classification, 'ready');
assert.equal(teknologibygger.role_world_status, 'role_world_complete');
assert.equal(teknologibygger.role_world_path, 'data/Civication/roleWorlds/vitenskap/vitenskap_teknologibygger.json');
assert.equal(teknologibygger.priority_score, 405);
assert.equal(teknologibygger.authored_depth.exact_source_ref_count, 1);
assert.equal(teknologibygger.authored_depth.max_narrative_depth, 14);
assert.deepEqual(teknologibygger.evidence.exact_source_refs, ['data/Civication/narratives/leisure/vitenskap_teknologibygger.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'vitenskap/teknologibygger'));
const argumentbygger = audit.positions.find((row) => row.key === 'filosofi/argumentbygger');
assert.ok(argumentbygger);
...
assert.ok(!(audit.queue || []).some((row) => row.key === 'filosofi/argumentbygger'));
const etiker = audit.positions.find((row) => row.key === 'filosofi/etiker');
assert.ok(etiker);
assert.equal(etiker.classification, 'ready');
assert.equal(etiker.role_world_status, 'role_world_complete');
assert.equal(etiker.role_world_path, 'data/Civication/roleWorlds/filosofi/filosofi_etiker.json');
assert.equal(etiker.priority_score, 390);
assert.equal(etiker.authored_depth.exact_source_ref_count, 1);
assert.equal(etiker.authored_depth.max_narrative_depth, 14);
assert.deepEqual(etiker.evidence.exact_source_refs, ['data/Civication/narratives/leisure/filosofi_etiker.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'filosofi/etiker'));
const fagfilosof = audit.positions.find((row) => row.key === 'filosofi/fagfilosof');
assert.ok(fagfilosof);
assert.equal(fagfilosof.classification, 'ready');
assert.equal(fagfilosof.role_world_status, 'role_world_complete');
assert.equal(fagfilosof.role_world_path, 'data/Civication/roleWorlds/filosofi/filosofi_fagfilosof.json');
assert.equal(fagfilosof.priority_score, 390);
assert.equal(fagfilosof.authored_depth.exact_source_ref_count, 1);
assert.equal(fagfilosof.authored_depth.max_narrative_depth, 14);
assert.deepEqual(fagfilosof.evidence.exact_source_refs, ['data/Civication/narratives/leisure/filosofi_fagfilosof.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'filosofi/fagfilosof'));
const filosofiskVeileder = audit.positions.find((row) => row.key === 'filosofi/filosofisk_veileder');
assert.ok(filosofiskVeileder);
assert.equal(filosofiskVeileder.classification, 'ready');
assert.equal(filosofiskVeileder.role_world_status, 'role_world_complete');
assert.equal(filosofiskVeileder.role_world_path, 'data/Civication/roleWorlds/filosofi/filosofi_filosofisk_veileder.json');
assert.equal(filosofiskVeileder.priority_score, 390);
assert.equal(filosofiskVeileder.authored_depth.exact_source_ref_count, 1);
assert.equal(filosofiskVeileder.authored_depth.max_narrative_depth, 14);
assert.deepEqual(filosofiskVeileder.evidence.exact_source_refs, ['data/Civication/narratives/leisure/filosofi_filosofisk_veileder.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'filosofi/filosofisk_veileder'));
const logiker = audit.positions.find((row) => row.key === 'filosofi/logiker');
assert.ok(logiker);
assert.equal(logiker.classification, 'ready');
assert.equal(logiker.role_world_status, 'role_world_complete');
assert.equal(logiker.role_world_path, 'data/Civication/roleWorlds/filosofi/filosofi_logiker.json');
assert.equal(logiker.priority_score, 390);
assert.equal(logiker.authored_depth.exact_source_ref_count, 1);
assert.equal(logiker.authored_depth.max_narrative_depth, 14);
assert.deepEqual(logiker.evidence.exact_source_refs, ['data/Civication/narratives/leisure/filosofi_logiker.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'filosofi/logiker'));
const nysgjerrig = audit.positions.find((row) => row.key === 'filosofi/nysgjerrig');
assert.ok(nysgjerrig);
assert.equal(nysgjerrig.classification, 'ready');
assert.equal(nysgjerrig.role_world_status, 'role_world_complete');
assert.equal(nysgjerrig.role_world_path, 'data/Civication/roleWorlds/filosofi/filosofi_nysgjerrig.json');
assert.equal(nysgjerrig.priority_score, 390);
assert.equal(nysgjerrig.authored_depth.exact_source_ref_count, 1);
assert.equal(nysgjerrig.authored_depth.max_narrative_depth, 14);
assert.deepEqual(nysgjerrig.evidence.exact_source_refs, ['data/Civication/narratives/leisure/filosofi_nysgjerrig.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'filosofi/nysgjerrig'));
const samtalepartner = audit.positions.find((row) => row.key === 'filosofi/samtalepartner');
assert.ok(samtalepartner);
assert.equal(samtalepartner.classification, 'ready');
assert.equal(samtalepartner.role_world_status, 'role_world_complete');
assert.equal(samtalepartner.role_world_path, 'data/Civication/roleWorlds/filosofi/filosofi_samtalepartner.json');
assert.equal(samtalepartner.priority_score, 390);
assert.equal(samtalepartner.authored_depth.exact_source_ref_count, 1);
assert.equal(samtalepartner.authored_depth.max_narrative_depth, 14);
assert.deepEqual(samtalepartner.evidence.exact_source_refs, ['data/Civication/narratives/leisure/filosofi_samtalepartner.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'filosofi/samtalepartner'));
const sporrer = audit.positions.find((row) => row.key === 'filosofi/sporrer');
assert.ok(sporrer);
assert.equal(sporrer.classification, 'ready');
assert.equal(sporrer.role_world_status, 'role_world_complete');
assert.equal(sporrer.role_world_path, 'data/Civication/roleWorlds/filosofi/filosofi_sporrer.json');
assert.equal(sporrer.priority_score, 390);
assert.equal(sporrer.authored_depth.exact_source_ref_count, 1);
assert.equal(sporrer.authored_depth.max_narrative_depth, 14);
assert.deepEqual(sporrer.evidence.exact_source_refs, ['data/Civication/narratives/leisure/filosofi_sporrer.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'filosofi/sporrer'));
const gallerist = audit.positions.find((row) => row.key === 'kunst/gallerist');
assert.ok(gallerist);
assert.equal(gallerist.classification, 'ready');
assert.equal(gallerist.role_world_status, 'role_world_complete');
assert.equal(gallerist.role_world_path, 'data/Civication/roleWorlds/kunst/kunst_gallerist.json');
assert.equal(gallerist.priority_score, 390);
assert.equal(gallerist.authored_depth.exact_source_ref_count, 1);
assert.equal(gallerist.authored_depth.max_narrative_depth, 14);
assert.deepEqual(gallerist.evidence.exact_source_refs, ['data/Civication/narratives/leisure/kunst_gallerist.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'kunst/gallerist'));
...
assert.ok(!(audit.queue || []).some((row) => row.key === 'litteratur/anmelder'));
...
assert.equal(essayist.role_world_status, 'role_world_complete');
assert.equal(essayist.role_world_path, 'data/Civication/roleWorlds/litteratur/litteratur_essayist.json');
assert.equal(essayist.authored_depth.exact_source_ref_count, 1);
assert.equal(essayist.authored_depth.max_narrative_depth, 14);
assert.deepEqual(essayist.evidence.exact_source_refs, ['data/Civication/narratives/leisure/litteratur_essayist.json']);
...
const litteraturinteressert = audit.positions.find((row) => row.key === 'litteratur/litteraturinteressert');
...
assert.equal(litteraturinteressert.role_world_path, 'data/Civication/roleWorlds/litteratur/litteratur_litteraturinteressert.json');
...
assert.deepEqual(litteraturinteressert.evidence.exact_source_refs, ['data/Civication/narratives/leisure/litteratur_litteraturinteressert.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'litteratur/litteraturinteressert'));
const litteraturkritiker = audit.positions.find((row) => row.key === 'litteratur/litteraturkritiker');
...
assert.equal(litteraturkritiker.role_world_path, 'data/Civication/roleWorlds/litteratur/litteratur_litteraturkritiker.json');
...
assert.deepEqual(litteraturkritiker.evidence.exact_source_refs, ['data/Civication/narratives/leisure/litteratur_litteraturkritiker.json']);
assert.ok(!(audit.queue || []).some((row) => row.key === 'litteratur/litteraturkritiker'));
...
console.log('civication life-position Role World readiness v2 ok: 128 ready / 31 authored-depth / 40 not-standalone; 128 complete / no pending-ready');
