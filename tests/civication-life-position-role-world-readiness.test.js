...
assert.deepEqual(audit.summary.classifications, {
  ready: 48,
  needs_authored_depth: 111,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 48);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 14);
assert.equal(audit.summary.positions_with_exact_governed_sources, 48);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 48);
...
console.log('civication life-position Role World readiness v2 ok: 48 ready / 111 authored-depth / 40 not-standalone; 48 complete / no pending-ready');
