...
assert.deepEqual(audit.summary.classifications, {
  ready: 124,
  needs_authored_depth: 35,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 124);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 15);
assert.equal(audit.summary.positions_with_exact_governed_sources, 124);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 124);
...
console.log('civication life-position Role World readiness v2 ok: 124 ready / 35 authored-depth / 40 not-standalone; 124 complete / no pending-ready');
