...
assert.deepEqual(audit.summary.classifications, {
  ready: 102,
  needs_authored_depth: 57,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 102);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 14);
assert.equal(audit.summary.positions_with_exact_governed_sources, 102);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 102);
...
console.log('civication life-position Role World readiness v2 ok: 102 ready / 57 authored-depth / 40 not-standalone; 102 complete / no pending-ready');
