...
assert.deepEqual(audit.summary.classifications, {
  ready: 115,
  needs_authored_depth: 44,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 115);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 15);
assert.equal(audit.summary.positions_with_exact_governed_sources, 115);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 115);
...
console.log('civication life-position Role World readiness v2 ok: 104 ready / 55 authored-depth / 40 not-standalone; 104 complete / no pending-ready');
