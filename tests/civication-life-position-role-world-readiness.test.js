...
assert.deepEqual(audit.summary.classifications, {
  ready: 74,
  needs_authored_depth: 85,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 74);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 14);
assert.equal(audit.summary.positions_with_exact_governed_sources, 74);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 74);
...
console.log('civication life-position Role World readiness v2 ok: 74 ready / 85 authored-depth / 40 not-standalone; 74 complete / no pending-ready');
