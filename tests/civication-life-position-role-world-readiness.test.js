...
assert.deepEqual(audit.summary.classifications, {
  ready: 59,
  needs_authored_depth: 100,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 59);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 14);
assert.equal(audit.summary.positions_with_exact_governed_sources, 59);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 59);
...
console.log('civication life-position Role World readiness v2 ok: 59 ready / 100 authored-depth / 40 not-standalone; 59 complete / no pending-ready');
