...
assert.deepEqual(audit.summary.classifications, {
  ready: 126,
  needs_authored_depth: 33,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 126);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 15);
assert.equal(audit.summary.positions_with_exact_governed_sources, 126);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 126);
...
console.log('civication life-position Role World readiness v2 ok: 126 ready / 33 authored-depth / 40 not-standalone; 126 complete / no pending-ready');
