...
assert.deepEqual(audit.summary.classifications, {
  ready: 52,
  needs_authored_depth: 107,
  not_a_standalone_world: 40
});
assert.equal(audit.summary.completed_life_position_role_worlds, 52);
assert.equal(audit.summary.pending_ready_positions, 0);
assert.equal(audit.summary.livelihood_backed_positions, 14);
assert.equal(audit.summary.positions_with_exact_governed_sources, 52);
assert.equal(audit.summary.positions_with_multi_scene_narrative_foundation, 52);
...
console.log('civication life-position Role World readiness v2 ok: 52 ready / 107 authored-depth / 40 not-standalone; 52 complete / no pending-ready');
