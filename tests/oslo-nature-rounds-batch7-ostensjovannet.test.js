...
const routeDir = 'data/places/natur/oslo/places_oslo_natur_ostensjovannet';
const routePlaces = [
  ['ostensjovannet_nord', `${routeDir}/ostensjovannet_nord.json`],
  ['ostensjovannet_fugletarn', `${routeDir}/ostensjovannet_fugletarn.json`],
  ['ostensjovannet_sivbelte', `${routeDir}/ostensjovannet_sivbelte.json`],
  ['ostensjovannet_sor', `${routeDir}/ostensjovannet_sor.json`],
  ['bogerudmyra', `${routeDir}/bogerudmyra.json`]
];
...
for (const [expectedId, file] of routePlaces) {
  ...
  assert.strictEqual(place.routeId, 'ostensjovannet_vatmark', `${place.id}: feil rute`);
...
console.log('Østensjøvannet Nature profiles batch 7 OK');
