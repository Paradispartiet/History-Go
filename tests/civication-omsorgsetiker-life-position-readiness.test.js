...
const streamPath='data/Civication/narratives/leisure/helse_omsorgsetiker.json';
...
assert.equal(row.kind,'ethics_identity');
assert.equal(stream.storylets.length,14);
assert.equal(new Set(stream.storylets.map(x=>x.id)).size,14);
assert.equal(stream.applies_when.any_tags[0],'helse:omsorgsetiker');
...
console.log('Omsorgsetiker readiness and lifecycle complete ok');
