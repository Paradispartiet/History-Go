...
const stream=read('data/Civication/narratives/leisure/natur_turgaer.json');
const world=read('data/Civication/roleWorlds/natur/natur_turgaer.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'natur',id:'turgaer',label:'Turgåer'});
...
const row=audit.positions.find(x=>x.key==='natur/turgaer');
...
assert.match(world.sociological_core.description,/ingen automatisk myndighet som profesjonell guide/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
console.log('Turgåer Role World gate ok');
