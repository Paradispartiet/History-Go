...
assert.deepEqual(world.life_position_ref,{badge_id:'naeringsliv',id:'industrieier',label:'Industrieier'});
assert.equal(world.materialization.no_new_runtime,true);
assert.match(world.sociological_core.description,/industrial_ownership|produksjonsmidler|kapitalbinding/i);
assert.match(world.sociological_core.description,/Bedriftseier|Investor|Industribygger|Daglig leder|Produksjonsleder/i);
assert.match(world.sociological_core.description,/ingen automatisk operativ instruksjonsmyndighet|fagansvar|styrelederstatus/i);
...
assert.equal(new Set(world.season.coverage.map(x=>x.summary)).size,56);
