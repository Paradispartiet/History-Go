...
const stream=read('data/Civication/narratives/leisure/vitenskap_teknologibygger.json');
const world=read('data/Civication/roleWorlds/vitenskap/vitenskap_teknologibygger.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'vitenskap',id:'teknologibygger',label:'Teknologibygger'});
...
const row=audit.positions.find(x=>x.key==='vitenskap/teknologibygger');
...
assert.match(world.sociological_core.description,/ingen automatisk ingeniørrolle/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('teknisk gjeld'));
assert.ok(world.sociological_core.description.includes('rollback'));
console.log('Teknologibygger Role World gate ok');
