...
function walk(a,b,key){
  ...
  if(Array.isArray(a)&&Array.isArray(b)){
    ...
    const n=Math.max(a.length,b.length); for(let i=0;i<n&&diffs.length<80;i++) walk(a[i],b[i],key+'['+i+']');
    return;
  }
  if(a&&b&&typeof a==='object'&&typeof b==='object'){
    const ks=[...new Set([...Object.keys(a),...Object.keys(b)])].sort();
    for(const k of ks){ if(diffs.length>=80)break; walk(a[k],b[k],key?key+'.'+k:k); }
    return;
  }
  if(JSON.stringify(a)!==JSON.stringify(b)) diffs.push({path:key,before:a,after:b});
}
walk(before,after,'');
throw new Error('SMASPARER_GENERATOR_DIFF\n'+JSON.stringify(diffs,null,2));
