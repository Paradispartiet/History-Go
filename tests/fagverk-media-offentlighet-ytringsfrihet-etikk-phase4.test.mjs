...
import { auditMediaOffentlighetYtringsfrihetEtikkPhase4 } from '../scripts/audit-fagverk-media-offentlighet-ytringsfrihet-etikk-phase4.mjs';
...
test('Offentlighet, ytringsfrihet og medieetikk er canonicalt materialisert 21/21', () => {
  ...
  assert.equal(report.canonicalCoverage.coveredSubjectEmneCount, 42);
...
test('Offentlighetskapittelet har full pedagogisk og evidensbasert pakke', () => {
  const { report, chapter, claimsDoc, modules } = auditMediaOffentlighetYtringsfrihetEtikkPhase4();
  ...
  assert.deepEqual(chapter.relatedPlaces.map((place) => place.id), ['stortinget', 'tinghuset', 'litteraturhuset', 'aftenposten_akersgata']);
...
test('Offentlighetskapittelet hydrerer alle synlige rendererfelt', async () => {
  const registry = JSON.parse(fs.readFileSync(path.join(root, 'data/fagverk/fagverk_registry.json'), 'utf8'));
  const chapterMeta = registry.subjects.media.chapters.find((chapter) => chapter.id === 'offentlighet-ytringsfrihet-og-medieetikk');
