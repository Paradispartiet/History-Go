...
const cards = JSON.parse(fs.readFileSync("data/natur/fauna/marine_akrafjorden_batch_7.json", "utf8"));
...
assert.ok(place.published_species_batch >= 7);
assert.ok(place.fauna.length >= 151);
assert.ok(place.published_species_count >= 151);
assert.ok(place.remaining_species_level_taxa_count <= 110);
assert.equal(place.excluded_higher_taxa_count, 99);
assert.ok(["Motacilla cinerea", "Turdus torquatus", "Xanthoria aureola"].every(taxon => place.excluded_species_level_edge_taxa.includes(taxon)));
