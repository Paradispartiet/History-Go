...
const streamPath = 'data/Civication/narratives/leisure/vitenskap_maker.json';
const stream = readJson(streamPath);
const row = audit.positions.find((item) => item.key === 'vitenskap/maker');

assert.ok(row);
assert.equal(stream.storylets.length, 14);
assert.equal(row.classification, 'ready');
assert.equal(row.role_world_status, 'role_world_complete');
assert.equal(row.role_world_path, 'data/Civication/roleWorlds/vitenskap/vitenskap_maker.json');
