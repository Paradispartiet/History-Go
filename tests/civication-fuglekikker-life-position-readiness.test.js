...
const streamPath='data/Civication/narratives/leisure/natur_fuglekikker.json';
const stream=read(streamPath),row=audit.positions.find(x=>x.key==='natur/fuglekikker');
...
assert.equal(stream.applies_when.any_tags[0],'natur:fuglekikker');
assert.equal(row.classification,'ready');assert.equal(row.role_world_status,'role_world_complete');
assert.equal(row.role_world_path,'data/Civication/roleWorlds/natur/natur_fuglekikker.json');assert.equal(row.priority_score,405);
...
console.log('Fuglekikker readiness and lifecycle complete ok');
