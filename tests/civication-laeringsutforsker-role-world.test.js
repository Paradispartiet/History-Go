...
const stream=read('data/Civication/narratives/leisure/utdanning_laeringsutforsker.json');
const world=read('data/Civication/roleWorlds/utdanning/utdanning_laeringsutforsker.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'utdanning',id:'laeringsutforsker',label:'Læringsutforsker'});
...
const row=audit.positions.find(x=>x.key==='utdanning/laeringsutforsker');
...
assert.ok(world.sociological_core.description.includes('forklaringsvariasjon'));
assert.ok(world.sociological_core.description.includes('aldersantakelser'));
console.log('Læringsutforsker Role World gate ok');
