...
const LOOPS = ['bestilling -> kunnskapsgrunnlag -> analyse -> alternativer -> råd -> dokumentasjon','tiltak -> naturverdi -> regelverk -> avbøting -> restusikkerhet -> anbefaling'];
...
const EXPECTED_THEMES = ['professional_culture','bureaucratic_power','status_anxiety','shame_reputation','precarity','care_vs_efficiency','invisible_work','public_private_leakage','class_power'];
const EXPECTED_AUDIENCES = ['faglig_kvalitet_og_naturforvaltning','plan_regelverk_og_mandat','kart_data_og_metode','prosjekt_kvalitet_og_bestiller','beslutningseier_og_forvaltningsledelse','tiltakshaver_og_oppdragsgiver','tverrfaglige_kolleger_og_horingsmiljo','private_relations'];
...
assert.equal(grammar.persistent_work_object_contract.states.length,19);
...
for (const audience of rep.audiences) {
  ...
  assert.ok(audience.cannot_grant.length>=700,`${audience.id}: ${audience.cannot_grant.length}`);
  for (const term of [/kan ikke/i,/evidens/i,/relevant_education_or_employer_qualification/i,/History Go/i,/Badge/i,/hjemmel|delegasjon/i,/vedtak/i]) assert.match(audience.cannot_grant,term,`${audience.id}/${term}`);
}
assert.ok(rep.divergence_examples.length>=8);
for (const term of [/global/i,/evidens/i,/Naturforvalter/i,/Rådgiver/i,/Seniorrådgiver/i,/qualification_required/i,/relevant_education_or_employer_qualification/i,/regelverk|lovverk/i,/delegasjon/i,/vedtak/i,/History Go/i,/Badge/i]) assert.match(rep.authority_separation,term);
...
assert.ok(world.history_go_affordance.better_question.length>=1200,world.history_go_affordance.better_question.length);
for (const term of [/kan ikke/i,/relevant_education_or_employer_qualification/i,/feltdata/i,/hjemmel/i,/delegasjon/i,/vedtak|beslutningsmyndighet/i,/Badge/i]) assert.match(world.history_go_affordance.authority_boundary,term);

assert.equal(world.cross_role_proof.status,'not_required_for_rollout_no_shared_object');
...
assert.match(world.cross_role_proof.rule,/shared|delt|felles/i);
assert.match(world.cross_role_proof.rule,/uten cross-role-link|uten.*link/i);
assert.deepEqual(world.editorial_uniqueness.not_copy_of,['natur/natur_felt_og_formidling','natur/natur_biologi_og_forskning','historie/historie_institusjonsledelse']);
for (const term of [/beslutningsgrunnlag/i,/datagap/i,/naturverdi/i,/regelverk/i,/delegasjon/i,/alternativ/i,/avbøting/i,/restkonsekvens/i,/restusikkerhet/i,/bestiller/i,/vedtak/i,/privat/i]) assert.match(world.editorial_uniqueness.rule,term);
...
for (const beat of world.season.coverage) {
  ...
  assert.ok(beat.summary.length>=2000,`${beat.day}/${beat.phase} summary=${beat.summary.length}`);
  assert.ok(beat.standing_consequence.length>=950,`${beat.day}/${beat.phase} standing=${beat.standing_consequence.length}`);
...
for (const thread of world.primary_threads) {
  assert.ok(thread.relationship.length>=380,`${thread.id}: ${thread.relationship.length}`);
  ...
  for (const ref of thread.beat_refs) {
    assert.ok(beatKeys.has(ref),`${thread.id}/${ref}`);
    const beat = world.season.coverage.find((row)=>`${row.day}/${row.phase}`===ref);
    assert.ok((beat.thread_ids||[]).includes(thread.id),`${thread.id} missing from ${ref}`);
  }
}
...
assert.ok(readiness.summary.role_world_complete_or_pilot>=67);
assert.ok(readiness.summary.rollout_queue_roles<=18);
...
const policies = Object.fromEntries(career.audit.salary.rows.map((row)=>[row.title,row.offer_policy]));
assert.equal(policies.Naturforvalter,'qualification_required');
assert.equal(policies['Rådgiver (miljø/natur)'],'qualification_required');
assert.equal(policies['Seniorrådgiver (miljø/natur)'],'qualification_required');
...
for (const term of [/Scope lock/i,/Naturforvalter.*qualification_required/i,/Rådgiver.*qualification_required/i,/Seniorrådgiver.*qualification_required/i,/relevant_education_or_employer_qualification/i,/situated_reputation/i,/no global reputation score/i,/not_required_for_rollout/i,/no cross-role link/i,/History Go/i,/no new runtime/i,/Editorial uniqueness/i,/15 canonical/i,/14.*4.*56/i]) assert.match(source,term);

console.log('Civication Natur Forvaltning og rådgivning Role World rollout: OK');
