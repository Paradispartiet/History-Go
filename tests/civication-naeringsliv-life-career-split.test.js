...
const jobs = [
  ['Ekspeditør / butikkmedarbeider',1,'direct',[],'naeringsliv_handel_og_kundeservice'],
  ['Renholder',1,'direct',[],'naeringsliv_renhold_og_hygiene'],
  ['Lager- og driftsmedarbeider',1,'direct',[],'naeringsliv_logistikk_og_drift'],
  ['Økonomi- og administrasjonsmedarbeider',1,'direct',[],'naeringsliv_administrasjon_og_okonomistyring'],
  ['Fagarbeider',1,'qualification_required',['relevant_education_or_employer_qualification'],'naeringsliv_fag_og_produksjon'],
  ['Skiftleder',1,'appointment_required',['employer_appointment'],'naeringsliv_operativ_ledelse'],
  ['Formann / arbeidsleder',2,'appointment_required',['employer_appointment'],'naeringsliv_operativ_ledelse'],
  ['Controller',2,'qualification_required',['relevant_education_or_employer_qualification'],'naeringsliv_administrasjon_og_okonomistyring'],
  ['Avdelingsleder',2,'appointment_required',['employer_appointment'],'naeringsliv_operativ_ledelse'],
  ['Driftsleder',2,'appointment_required',['employer_appointment'],'naeringsliv_operativ_ledelse'],
  ['Finansanalytiker',2,'qualification_required',['relevant_education_or_employer_qualification'],'naeringsliv_finans_og_kapitalforvaltning'],
  ['Produksjonsleder',2,'appointment_required',['employer_appointment'],'naeringsliv_operativ_ledelse'],
  ['Butikksjef / enhetsleder',2,'appointment_required',['employer_appointment'],'naeringsliv_operativ_ledelse'],
  ['Økonomi- og finanssjef',3,'appointment_required',['employer_appointment'],'naeringsliv_finansiell_ledelse'],
  ['Daglig leder',3,'appointment_required',['employer_appointment'],'naeringsliv_virksomhetsledelse'],
  ['Finansdirektør',3,'appointment_required',['employer_appointment'],'naeringsliv_finansiell_ledelse'],
  ['Konserndirektør',3,'appointment_required',['employer_appointment'],'naeringsliv_virksomhetsledelse'],
  ['Konsernsjef',3,'appointment_required',['employer_appointment'],'naeringsliv_virksomhetsledelse'],
  ['Kapitalforvalter',3,'qualification_required',['relevant_education_or_employer_qualification'],'naeringsliv_finans_og_kapitalforvaltning']
];
...
assert.deepStrictEqual(businessAudit.find((row) => row[0] === 'Finansanalytiker'), ['Finansanalytiker','qualified_job','qualification_required','keep_with_gate',['relevant_education_or_employer_qualification']]);
...
assert.ok(meritsSource.includes('new Set(["natur", "naeringsliv"])'), 'Næringsliv-overlay skal være obligatorisk fail-closed');
const sandbox = {
  console,
  setTimeout: () => 0,
  clearTimeout: () => {},
...
for (const label of pureLife) {
  ...
  assert.strictEqual(Resolver.resolveCareerRoleScope({career_id:'naeringsliv', title:label}), 'unknown', `${label}: skal ikke ha title-basert jobbscope`);
}
...
for (const [title,salaryTier,policy,qualificationIds,scope] of jobs) {
  ...
  assert.strictEqual(Resolver.resolveCareerRoleScope({career_id:'naeringsliv', title}), scope, `${title}: feil role_scope`);
}
...
for (const title of ['Skiftleder','Daglig leder','Konsernsjef']) {
  result = sandbox.window.CivicationJobs.pushOffer({career_id:'naeringsliv', title});
  assert.strictEqual(result.ok, false, `${title}: arbeidsgiverutnevnelse må kreves`);
}
qualifications = new Set(['employer_appointment']);
for (const title of ['Skiftleder','Daglig leder','Konsernsjef']) {
  result = sandbox.window.CivicationJobs.pushOffer({career_id:'naeringsliv', title});
  assert.strictEqual(result.ok, true, `${title}: eksplisitt arbeidsgiverutnevnelse skal åpne rollen`);
}

assert.throws(() => sandbox.window.applyBadgeCareerContractOverlay([JSON.parse(JSON.stringify(rawBadge))], {
  badge_id:'naeringsliv', allowed_tier_patch_fields:['life_position'], tiers:[{label:'Ikke en tier', life_position:{kind:'x'}}]
}), /unknown_tier/, 'overlay skal fail-closed på ukjent tittel');
assert.throws(() => sandbox.window.applyBadgeCareerContractOverlay([JSON.parse(JSON.stringify(rawBadge))], {
  badge_id:'naeringsliv', allowed_tier_patch_fields:['life_position'], tiers:[{label:'Gründer', threshold:999}]
...
console.log('civication Næringsliv life-career split ok: 5 ownership/economic positions / 19 formal jobs / 9 high-quality work worlds / overlay fail-closed');
