...
const stream=read('data/Civication/narratives/leisure/sport_sondagsutover.json');
const world=read('data/Civication/roleWorlds/sport/sport_sondagsutover.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'sport',id:'sondagsutover',label:'Søndagsutøver'});
...
const row=audit.positions.find(x=>x.key==='sport/sondagsutover');
...
assert.match(world.sociological_core.description,/ingen automatisk myndighet som trener/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('personlige rekorder'));
assert.ok(world.sociological_core.description.includes('kroppssignaler'));
console.log('Søndagsutøver Role World gate ok');
