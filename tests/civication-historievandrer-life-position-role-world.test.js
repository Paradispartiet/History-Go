...
const worldPath = 'data/Civication/roleWorlds/historie/historie_historievandrer.json';
const narrativePath = 'data/Civication/narratives/leisure/historievandrer.json';
...
assert.equal(world.role_scope, 'historie_historievandrer');
...
assert.equal(world.title, 'Historievandrer');
assert.equal(world.status, 'role_world_complete');
assert.equal(world.materialization?.no_new_runtime, true);
assert.match(world.sociological_core.description, /ikke.*historiker|historiker/i);
assert.match(world.sociological_core.description, /arkivar|guide|kulturminnemyndighet/i);
...
function verifyNarrativeRef(reference) {
  ...
  assert.ok(storyIds.has(reference.slice(prefix.length)), 'missing Historievandrer anchor ' + reference);
}
...
assert.deepEqual(
  themeBank.reference_profiles['historie/historie_historievandrer'],
  world.theme_ids
);
...
const organizer = world.recurring_people_archetypes.find((row) => row.id === 'lokal_arrangor');
assert.ok(organizer);
assert.match(organizer.class_position, /uten myndighet|ingen myndighet|uten.*autorisere/i);
assert.match(organizer.teaches_player, /ikke.*historiker|ikke.*arkivar|ikke.*kulturminne|ikke.*offentlig/i);
...
const entry = index.roles.find((row) => row.life_position_key === 'historie/historievandrer');
assert.ok(entry);
assert.equal(entry.category, 'historie');
assert.equal(entry.role_scope, 'historie_historievandrer');
...
assert.match(
  ...
  /ikke.*autorisert guide|ikke.*historiker|ikke.*arkivar|kulturminneforvalter|offentlige myndigheter/i
);
...
execFileSync(process.execPath, ['tests/civication-historievandrer-life-position-readiness.test.js'], { cwd: ROOT, stdio: 'pipe' });

console.log('civication Historievandrer Role World ok: 56/56 coverage / 14 governed anchors / 91 total worlds / no new runtime');
