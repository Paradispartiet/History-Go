...
const worldPath='data/Civication/roleWorlds/filosofi/filosofi_filosofisk_veileder.json';
const narrativePath='data/Civication/narratives/leisure/filosofi_filosofisk_veileder.json';
...
assert.equal(world.role_scope,'filosofi_filosofisk_veileder');
assert.equal(world.subject_type,'life_position');
assert.deepEqual(world.life_position_ref,{badge_id:'filosofi',id:'filosofisk_veileder',label:'Filosofisk veileder'});
assert.equal(world.status,'role_world_complete');
assert.equal(world.materialization?.no_new_runtime,true);
assert.match(world.sociological_core.description,/philosophical_practice/i);
assert.match(world.sociological_core.description,/ingen automatisk fast lønn|ingen automatisk.*arbeidsforhold/i);
assert.match(world.sociological_core.description,/terapeutrolle|psykologrolle|juridisk rådgiverrolle|behandlingsmandat/i);
...
assert.deepEqual(themeBank.reference_profiles['filosofi/filosofi_filosofisk_veileder'],world.theme_ids);
const entry=index.roles.find(x=>x.life_position_key==='filosofi/filosofisk_veileder');
assert.ok(entry);
