...
const workSurfaces = [
  'psykologi_forsker_psykologi_analyserom',
  'psykologi_forsker_psykologi_prosjektmote',
  'psykologi_forsker_psykologi_manusverksted'
];

const model = readJson('data/Civication/roleModels/psykologi/forsker_psykologi.json');
const grammar = readJson('data/Civication/workGrammars/psykologi/forsker_psykologi.json');
const plan = readJson('data/Civication/mailPlans/psykologi/forsker_psykologi_plan.json');
...
assert.equal(model.version, 1, 'existing Psychology researcher roleModel stays v1');
assert.equal(model.category, 'psykologi');
assert.equal(model.role_scope, 'forsker_psykologi');
assert.equal(model.title, 'Forsker (psykologi)');
assert.ok(model.scope_boundary?.can?.length >= 3);
assert.ok(model.scope_boundary?.cannot?.some(line => /psykologautorisasjon/.test(line)), 'research employment never implies psychologist authorization');
...
assert.ok(model.scope_boundary?.cannot?.some(line => /persondata/.test(line)), 'personal-data boundary remains explicit');
assert.ok(model.career_path?.progression_to?.some(line => /Professor/.test(line)), 'academic progression remains explicit');
assert.ok(model.career_path?.possible_exits?.length >= 1);

assert.equal(grammar.schema, 'civication_work_grammar_v1');
assert.equal(grammar.version, 1, 'existing Psychology researcher FWG stays v1');
assert.equal(grammar.category, 'psykologi');
assert.equal(grammar.role_scope, 'forsker_psykologi');
assert.deepEqual(grammar.badge_binding.badge_titles, ['Forsker (psykologi)']);
...
assert.ok(grammar.actor_grammar.length >= 4, 'role has explicit research relationships');
assert.equal(grammar.place_grammar.length, 3, 'role has three concrete functional research work surfaces');
...
assert.equal(plan.role_scope, 'forsker_psykologi');
...
for (const type of mailTypes) {
  const rel = `data/Civication/mailFamilies/psykologi/${type}/forsker_psykologi_${type}.json`;
  ...
  assert.equal(catalog.role_scope, 'forsker_psykologi', `${type} scope`);
  ...
  for (const mail of mails) {
    ...
    mailIds.add(mail.id);
    assert.equal(mail.role_scope, 'forsker_psykologi');
...
assert.equal(followup.thread_key, conflict.thread_key, 'follow-up continues the analysis-integrity thread');
assert.equal(consequence.thread_key, conflict.thread_key, 'consequence continues the analysis-integrity thread');

const knowledge = catalogs.get('knowledge').families.flatMap(family => family.mails)[0];
assert.match(knowledge.summary, /valgt etter at mønsteret i dataene var kjent/, 'knowledge scene teaches timing-sensitive evidence status');
assert.ok(knowledge.choices.some(choice => choice.tags.includes('harking')), 'unsafe HARKing choice is explicit');

const event = catalogs.get('event').families.flatMap(family => family.mails)[0];
assert.ok(event.choices.some(choice => choice.tags.includes('privacy_risk')), 'unsafe premature data sharing is explicit');

const world = matrix.worlds.find(row => row.key === 'psykologi/forsker_psykologi');
assert.ok(world, 'Psychology researcher remains a canonical career world');
...
console.log('✓ Psykologi forsker_psykologi is canonical playable with 14 complete components and one intentional practice-story partial');
