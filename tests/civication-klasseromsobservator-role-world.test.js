...
const stream=read('data/Civication/narratives/leisure/utdanning_klasseromsobservator.json');
const world=read('data/Civication/roleWorlds/utdanning/utdanning_klasseromsobservator.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'utdanning',id:'klasseromsobservator',label:'Klasseromsobservatør'});
...
const row=audit.positions.find(x=>x.key==='utdanning/klasseromsobservator');
...
assert.match(world.sociological_core.description,/observation_identity/i);
assert.match(world.sociological_core.description,/ingen automatisk lærer-, inspektør-, sensor-, diagnose- eller spesialpedagogmyndighet/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('romorganisering'));
assert.ok(world.sociological_core.description.includes('personvern'));
console.log('Klasseromsobservatør Role World gate ok');
