...
const stream=read('data/Civication/narratives/leisure/psykologi_selvgransker.json');
const world=read('data/Civication/roleWorlds/psykologi/psykologi_selvgransker.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'psykologi',id:'selvgransker',label:'Selvgransker'});
...
const row=audit.positions.find(x=>x.key==='psykologi/selvgransker');
...
assert.ok(world.sociological_core.description.includes('intensjon mot effekt'));
assert.ok(world.sociological_core.description.includes('blindsoner'));
console.log('Selvgransker Role World gate ok');
