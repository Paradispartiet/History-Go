...
const stream=read('data/Civication/narratives/leisure/kunst_ateliermenneske.json');
const world=read('data/Civication/roleWorlds/kunst/kunst_ateliermenneske.json');
const index=read('data/Civication/roleWorlds/index.json');
const row=audit.positions.find(x=>x.key==='kunst/ateliermenneske');
assert.ok(row);
assert.equal(world.status,'role_world_complete');
assert.equal(world.subject_type,'life_position');
assert.deepEqual(world.life_position_ref,{badge_id:'kunst',id:'ateliermenneske',label:'Ateliermenneske'});
...
assert.equal(row.role_world_path,'data/Civication/roleWorlds/kunst/kunst_ateliermenneske.json');
assert.equal(row.authored_depth.max_narrative_depth,14);
assert.ok(!audit.queue.some(x=>x.key==='kunst/ateliermenneske'));
...
for(const id of ['space_vs_status','critique_vs_authority','cleanup_vs_invisible_work','open_studio_vs_gatekeeping','collaboration_vs_authorship','atelier_practice_vs_artist_authority'])assert.ok(world.primary_threads.some(x=>x.id===id),id);
for(const id of ['ateliernaboen','verkstedansvarlig','den_nye_medbrukeren','atelierforvalteren','den_kritiske_vennen','samarbeidspartneren'])assert.ok(world.recurring_people_archetypes.some(x=>x.id===id),id);
assert.match(world.sociological_core.description,/ingen profesjonell kunstnerstatus/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
console.log('Ateliermenneske Role World gate ok');
