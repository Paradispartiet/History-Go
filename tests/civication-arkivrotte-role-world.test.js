...
const stream=read('data/Civication/narratives/leisure/historie_arkivrotte.json');
const world=read('data/Civication/roleWorlds/historie/historie_arkivrotte.json');
const index=read('data/Civication/roleWorlds/index.json');
const row=audit.positions.find(x=>x.key==='historie/arkivrotte');
assert.ok(row);
assert.equal(world.status,'role_world_complete');
assert.equal(world.subject_type,'life_position');
assert.deepEqual(world.life_position_ref,{badge_id:'historie',id:'arkivrotte',label:'Arkivrotte'});
...
assert.equal(row.role_world_path,'data/Civication/roleWorlds/historie/historie_arkivrotte.json');
assert.equal(row.authored_depth.max_narrative_depth,14);
assert.ok(!audit.queue.some(x=>x.key==='historie/arkivrotte'));
...
for(const id of ['catalog_vs_fragment','provenance_vs_transparency','archival_silence_vs_reconstruction','rare_find_vs_sensationalism','source_conflict_vs_confirmation','archive_practice_vs_possession'])assert.ok(world.primary_threads.some(x=>x.id===id),id);
for(const id of ['lesesalsvert','funnjegeren','katalogkjenner','lokalhistorikeren','kildekritikeren','digitalisatoren'])assert.ok(world.recurring_people_archetypes.some(x=>x.id===id),id);
assert.match(world.sociological_core.description,/ingen profesjonell arkivmyndighet/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
console.log('Arkivrotte Role World gate ok');
