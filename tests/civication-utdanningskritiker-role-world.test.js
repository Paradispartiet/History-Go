...
const stream=read('data/Civication/narratives/leisure/utdanning_utdanningskritiker.json');
const world=read('data/Civication/roleWorlds/utdanning/utdanning_utdanningskritiker.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'utdanning',id:'utdanningskritiker',label:'Utdanningskritiker'});
...
const row=audit.positions.find(x=>x.key==='utdanning/utdanningskritiker');
...
assert.match(world.sociological_core.description,/public_education_analysis/i);
assert.match(world.sociological_core.description,/ingen automatisk lærerrolle/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('korrelasjon og årsak'));
assert.ok(world.sociological_core.description.includes('medvirkning'));
console.log('Utdanningskritiker Role World gate ok');
