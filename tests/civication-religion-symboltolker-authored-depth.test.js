...
const lifeId='symboltolker';
const lifeKey='religion/symboltolker';
const lifeScope='religion_symboltolker';
const streamPath='data/Civication/narratives/leisure/religion_symboltolker.json';
const worldPath='data/Civication/roleWorlds/religion/religion_symboltolker.json';

const stream=read(streamPath);
const tier=badge.tiers.find(x=>x.life_position?.id===lifeId);
assert.ok(tier,'Religion Symboltolker tier must exist');
assert.equal(tier.life_position.label,'Symboltolker');
assert.equal(tier.life_position.kind,'symbol_interpretation_practice');
...
assert.ok(evidence.canonical_decision.pure_life_or_practice_tiers.includes('Symboltolker'));
assert.ok(evidence.salary_mapping.not_salary_jobs.includes('Symboltolker'));
...
assert.ok(row,'Religion Symboltolker must remain in life-position readiness');
assert.equal(row.classification,'ready','The authored Religion Symboltolker stream must make the position ready before Role World production');
...
for(const key of ['religion/nysgjerrig','religion/besokende','religion/dialogbygger','religion/feltarbeider','religion/livssynsutforsker','religion/pilegrim','religion/trosstedsvandrer','religion/ritualkjenner','religion/tradisjonskjenner','religion/troslivskjenner']){
...
if(!worldExists){
  ...
  assert.equal(indexed,undefined,'Source-depth phase must not pre-register an unbuilt Symboltolker Role World');
  const queued=audit.queue.find(x=>x.key===lifeKey);
  assert.ok(queued,'Ready but unbuilt Symboltolker must remain in queue');
  assert.equal(queued.rank,1,'Symboltolker should become the next canonical ready Role World after authored-depth repair');
  ...
  assert.ok(!audit.queue.some(x=>x.key===lifeKey),'Completed Symboltolker must leave the readiness queue');
  assert.ok(indexed,'Completed Symboltolker must be indexed');
  assert.equal(indexed.role_scope,lifeScope);
  assert.equal(indexed.status,'role_world_complete');
  assert.equal(world.schema,'civication_role_world_v1');
  assert.equal(world.subject_type,'life_position');
  assert.equal(world.status,'role_world_complete');
  assert.deepEqual(world.life_position_ref,{badge_id:'religion',id:lifeId,label:'Symboltolker'});
...
console.log(worldExists
  ? 'Religion Symboltolker authored-depth and completed Role World gate ok'
  : 'Religion Symboltolker authored-depth gate ok: exact 14-storylet source makes the life position ready without employment or interpretive authority');
