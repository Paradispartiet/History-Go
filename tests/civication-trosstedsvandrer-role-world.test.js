...
const stream=read('data/Civication/narratives/leisure/religion_trosstedsvandrer.json');
const world=read('data/Civication/roleWorlds/religion/religion_trosstedsvandrer.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'religion',id:'trosstedsvandrer',label:'Trosstedsvandrer'});
...
const row=audit.positions.find(x=>x.key==='religion/trosstedsvandrer');
...
assert.match(world.sociological_core.description,/ingen automatisk myndighet som medlem/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('observasjon mot ritualdeltakelse'));
assert.ok(world.sociological_core.description.includes('tilgjengelighet'));
console.log('Trosstedsvandrer Role World gate ok');
