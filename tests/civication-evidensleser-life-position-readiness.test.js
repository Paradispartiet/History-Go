...
const streamPath='data/Civication/narratives/leisure/helse_evidensleser.json';
...
assert.equal(row.kind,'evidence_literacy');
assert.equal(stream.storylets.length,14);
assert.equal(new Set(stream.storylets.map(x=>x.id)).size,14);
assert.equal(stream.applies_when.any_tags[0],'helse:evidensleser');
...
console.log('Evidensleser readiness and lifecycle complete ok');
