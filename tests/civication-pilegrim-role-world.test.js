...
const stream=read('data/Civication/narratives/leisure/religion_pilegrim.json');
const world=read('data/Civication/roleWorlds/religion/religion_pilegrim.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'religion',id:'pilegrim',label:'Pilegrim'});
...
const row=audit.positions.find(x=>x.key==='religion/pilegrim');
...
assert.match(world.sociological_core.description,/self_selected_identity/i);
...
assert.ok(world.sociological_core.description.includes('deltakelse mot appropriasjon'));
assert.ok(world.sociological_core.description.includes('hjemkomst'));
console.log('Pilegrim Role World gate ok');
