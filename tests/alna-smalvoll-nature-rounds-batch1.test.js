...
const placePath='data/places/natur/oslo/places_oslo_natur_alnaelva_rute/alna_smalvoll.json', quizPath='data/quiz/natur/alna_smalvoll_sets.json', storyPath='data/stories/stories_alna_smalvoll.json', articlePath='data/leksikon/places/oslo/natur/leksikon_alna_smalvoll.json';
...
assert.strictEqual(place.id,'alna_smalvoll');
...
for(const file of mapFiles){const raw=readJson(file);const entry=(raw.places||raw).alna_smalvoll;if(!entry)continue;merged.flora.push(...(entry.flora||[]));merged.fauna.push(...(entry.fauna||[]));}
