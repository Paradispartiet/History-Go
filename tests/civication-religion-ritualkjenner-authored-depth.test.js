...
const lifeId='ritualkjenner';
const lifeKey='religion/ritualkjenner';
const lifeScope='religion_ritualkjenner';
const streamPath='data/Civication/narratives/leisure/religion_ritualkjenner.json';
const worldPath='data/Civication/roleWorlds/religion/religion_ritualkjenner.json';

const stream=read(streamPath);
const tier=badge.tiers.find(x=>x.life_position?.id===lifeId);
assert.ok(tier,'Religion Ritualkjenner tier must exist');
assert.equal(tier.life_position.label,'Ritualkjenner');
assert.equal(tier.life_position.kind,'ritual_literacy');
...
assert.ok(evidence.canonical_decision.pure_life_or_practice_tiers.includes('Ritualkjenner'));
assert.ok(evidence.salary_mapping.not_salary_jobs.includes('Ritualkjenner'));
...
assert.ok(row,'Religion Ritualkjenner must remain in life-position readiness');
assert.equal(row.classification,'ready','The authored Religion Ritualkjenner stream must make the position ready before Role World production');
...
for(const key of ['religion/nysgjerrig','religion/besokende','religion/dialogbygger','religion/feltarbeider','religion/livssynsutforsker','religion/pilegrim','religion/trosstedsvandrer','religion/symboltolker','religion/tradisjonskjenner','religion/troslivskjenner']){
...
if(!worldExists){
  ...
  assert.equal(indexed,undefined,'Source-depth phase must not pre-register an unbuilt Ritualkjenner Role World');
  const queued=audit.queue.find(x=>x.key===lifeKey);
  assert.ok(queued,'Ready but unbuilt Ritualkjenner must remain in queue');
  assert.equal(queued.rank,1,'Ritualkjenner should become the next canonical ready Role World after authored-depth repair');
  ...
  assert.ok(!audit.queue.some(x=>x.key===lifeKey),'Completed Ritualkjenner must leave the readiness queue');
  assert.ok(indexed,'Completed Ritualkjenner must be indexed');
  assert.equal(indexed.role_scope,lifeScope);
  assert.equal(indexed.status,'role_world_complete');
  assert.equal(world.schema,'civication_role_world_v1');
  assert.equal(world.subject_type,'life_position');
  assert.equal(world.status,'role_world_complete');
  assert.deepEqual(world.life_position_ref,{badge_id:'religion',id:lifeId,label:'Ritualkjenner'});
...
console.log(worldExists
  ? 'Religion Ritualkjenner authored-depth and completed Role World gate ok'
  : 'Religion Ritualkjenner authored-depth gate ok: exact 14-storylet source makes the life position ready without employment or ritual authority');
