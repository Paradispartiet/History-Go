...
const career=read('data/Civication/careerGameplayMatrix.json'),row=career.worlds.find(x=>x.key===K);assert.ok(row,'career row missing');assert.equal(row.status,'playable');assert.equal(row.audit.runtime_gate,true);assert.deepEqual(row.audit.missing_components,[]);assert.equal(row.audit.salary.rows.length,3);
const salaryPolicies=Object.fromEntries(row.audit.salary.rows.map(x=>[x.title,x.offer_policy]));assert.deepEqual(salaryPolicies,{Kurator:'appointment_required',Kuratorassistent:'qualification_required','Senior kurator':'appointment_required'});
