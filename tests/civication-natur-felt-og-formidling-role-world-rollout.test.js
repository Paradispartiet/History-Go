...
const LOOPS = ['brief -> feltplan -> observasjon -> dokumentasjon -> kvalitetssjekk -> rapportering','målgruppe -> faggrunnlag -> formidling -> spørsmål -> korrigering -> læring'];
const WAITING = ['vaervindu','artsavklaring','prove_eller_fotometadata','hms_avklaring','saarbarhetsvurdering','faglig_kvalitetssjekk','malgruppe_eller_tilgjengelighet'];
const POLICY = {'Feltassistent':{policy:'direct'},'Naturveileder':{policy:'qualification_required',qualification_ids:['relevant_education_or_employer_qualification']}};
...
const EXPECTED_AUDIENCES = ['field_leadership_and_hms','species_quality_peers','field_team_and_local_observers','visitors_and_learning_groups','site_stewards_and_sensitive_nature_owners','metadata_and_handoff_peers','employer_commissioners_and_program_leads','private_relations'];
...
for (const audience of rep.audiences) {
  ...
  for (const term of [/kan ikke/i,/feltdata/i,/relevant_education_or_employer_qualification/i,/History Go/i,/Badge/i,/forvaltningsvedtak/i,/politisk myndighet/i]) assert.match(audience.cannot_grant,term,`${audience.id}/${term}`);
}
assert.ok(rep.divergence_examples.length>=8);
for (const term of [/global/i,/evidens/i,/feltdata/i,/Feltassistent/i,/Naturveileder/i,/qualification_required/i,/relevant_education_or_employer_qualification/i,/HMS/i,/forvaltningsvedtak/i,/politisk myndighet/i,/History Go/i,/Badge/i]) assert.match(rep.authority_separation,term);
...
for (const term of [/kan ikke/i,/relevant_education_or_employer_qualification/i,/feltdata/i,/artsbekreftelse/i,/HMS/i,/forvaltningsvedtak/i,/politisk/i,/Badge/i]) assert.match(world.history_go_affordance.authority_boundary,term);

assert.equal(world.cross_role_proof.status,'candidate_when_shared_work_is_real_no_shared_object');
...
assert.ok(readiness.summary.role_world_complete_or_pilot>=66);
assert.ok(readiness.summary.rollout_queue_roles<=19);
...
assert.equal(policies.Feltassistent,'direct');
assert.equal(policies.Naturveileder,'qualification_required');
...
for (const term of [/Scope lock/i,/Feltassistent.*direct/i,/Naturveileder.*qualification_required/i,/relevant_education_or_employer_qualification/i,/situated_reputation/i,/no global reputation score/i,/candidate_when_shared_work_is_real/i,/no cross-role link/i,/History Go/i,/no new runtime/i,/Editorial uniqueness/i,/15 canonical/i,/14.*4.*56/i]) assert.match(source,term);

console.log('Civication Natur Felt og formidling Role World rollout: OK');