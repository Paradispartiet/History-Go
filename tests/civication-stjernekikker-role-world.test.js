...
const stream=read('data/Civication/narratives/leisure/vitenskap_stjernekikker.json');
const world=read('data/Civication/roleWorlds/vitenskap/vitenskap_stjernekikker.json');
const index=read('data/Civication/roleWorlds/index.json');
assert.equal(world.status,'role_world_complete');
assert.deepEqual(world.life_position_ref,{badge_id:'vitenskap',id:'stjernekikker',label:'Stjernekikker'});
...
const row=audit.positions.find(x=>x.key==='vitenskap/stjernekikker');
...
assert.match(world.sociological_core.description,/ingen automatisk astronomstilling/i);
assert.match(world.sociological_core.description,/ingen ny runtime/i);
assert.ok(world.sociological_core.description.includes('mørketilpasning'));
assert.ok(world.sociological_core.description.includes('prognoseusikkerhet'));
console.log('Stjernekikker Role World gate ok');
