...
for (const world of matrix.worlds) {
  ...
  assert(
    !world.artifacts.role_tests.some((rel) => rel.endsWith('-authored-depth.test.js')),
    `${world.key} must not use a life-position authored-depth gate as career gameplay evidence`
  );
}
