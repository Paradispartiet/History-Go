...
test('Auditerte fag har dokumentert og statusriktig fremdrift gjennom den generelle motoren', () => {
  ...
  assert.equal(utdanning.nextGate, 'school_leadership_organization_source_brief_complete_full_chapter_production');
...
test('Helse er strict-complete og Utdanning låser konsistent 11/14-fremdrift', () => {
  const manifest = readJson('data/fag/fag_manifest.json');
  const status = readJson('data/fagverk/subject_status.json');
  ...
  for (const id of ['helse', 'utdanning']) {
    ...
    if (id === 'helse') {
      ...
      assert.equal(subjectStatus.nextGate, 'school_leadership_organization_source_brief_complete_full_chapter_production');
      assert.equal(registry.subjects.utdanning.editorialPlan.registeredChapterCount, 11);
      assert.equal(registry.subjects.utdanning.editorialPlan.completedSourceBriefCount, 12);
      assert.equal(pensum.domains.filter((domain) => domain.status === 'materialized').length, 11);
      ...
      assert.equal(pensum.domains[3].domain_id, 'grunnskole');
      assert.equal(pensum.domains[4].domain_id, 'videregaende_yrkesfag');
      ...
      assert.equal(pensum.domains[7].domain_id, 'spesialpedagogikk');
      assert.equal(pensum.domains[8].domain_id, 'inkludering_tilpasset_opplaering');
      assert.equal(pensum.domains[9].domain_id, 'utdanningshistorie');
