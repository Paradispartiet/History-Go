...
assert.equal(world.existing_work_continuity.persistent_work_object, 'dramaturgisk_arbeidsbok');
...
const audienceIds = [
  ...
  'director_and_ensemble',
  'research_and_archives',
  'production_and_technical',
  'institutional_leadership',
  'communication_and_publics',
  'private_relations'
];
...
assert.match(world.situated_reputation_model.authority_separation, /opphavsrett/);
assert.match(world.situated_reputation_model.authority_separation, /regimandat/);
assert.match(world.situated_reputation_model.authority_separation, /programmerings-/);
...
assert.match(world.history_go_affordance.better_question, /Johanne Dybwad/);
...
for (const beat of world.season.coverage) {
  assert.ok(beat.summary.length >= 500, `${beat.day}/${beat.phase} summary`);
  assert.ok(beat.standing_consequence.length >= 420, `${beat.day}/${beat.phase} consequence`);
...
for (const thread of world.primary_threads) {
  assert.ok(thread.relationship.length >= 140);
...
assert.ok(readiness.summary.role_world_complete_or_pilot >= 46);
...
const source = fs.readFileSync(path.join(ROOT, 'reports/CIVICATION_SCENEKUNST_DRAMATURGI_OG_UTVIKLING_ROLE_WORLD_ROLLOUT_SOURCE_FIRST.md'), 'utf8');
...
console.log('Civication Scenekunst dramaturgi og utvikling Role World rollout: OK');
