...
const world = read('data/Civication/roleWorlds/litteratur/litteratur_skribent.json');
const stream = read('data/Civication/narratives/leisure/litteratur_skribent.json');

assert.equal(world.schema, 'civication_role_world_v1');
assert.equal(world.category, 'litteratur');
assert.equal(world.role_scope, 'litteratur_skribent');
assert.equal(world.subject_type, 'life_position');
assert.deepEqual(world.life_position_ref, { badge_id: 'litteratur', id: 'skribent', label: 'Skribent' });
...
const prefix = 'data/Civication/narratives/leisure/litteratur_skribent.json#';
...
for (const term of ['brief', 'leser', 'sjanger', 'kilde', 'struktur', 'klarhet', 'revisjon', 'redigering', 'frist', 'publisering', 'oppdrag']) {
  assert.ok(normalized.includes(term), `Skribent description missing practice marker: ${term}`);
}
for (const boundary of ['forfatter', 'poet', 'jobb', 'lønn', 'honorar', 'kontrakt']) {
  assert.ok(normalized.includes(boundary), `Skribent description missing boundary marker: ${boundary}`);
}
assert.match(description, /ingen ny runtime/i);

console.log('Skribent Role World gate ok: 14 storylets / 56 beats / 14 threads / 6 people / 5 aftermath / 6 delayed');
