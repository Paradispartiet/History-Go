...
const WORLD = 'data/Civication/roleWorlds/historie/historie_fagledelse.json';
...
assert.ok(exists(WORLD), 'Historie Fagledelse Role World must exist before strict rollout proof runs');
...
assert.equal(world.existing_work_continuity.persistent_work_object, 'faglig_prioriterings_og_kvalitetslogg');
...
assert.deepEqual(world.materialization.source_refs, canonicalRefs, 'Role World must reuse the complete 15-mail prerequisite provenance');
...
const audienceIds = [
  'senior_historians_and_method_peers',
  'staff_and_capacity_bearers',
  'department_leadership_and_resource_line',
  'quality_archive_and_documentation',
  'commissioners_and_institutional_clients',
  'public_communication_and_affected_groups',
  'profession_peers_and_future_employers',
  'private_relations'
];
...
for (const term of [/global/i,/evidens|kilde/i,/diktere|konklusjon/i,/delegasjon|linje/i,/budsjett|personal/i,/publiser/i,/History Go|Badge/i]) assert.match(rep.authority_separation, term);
...
assert.match(world.history_go_affordance.better_question, /kilde|historiografi|perspektiv/i);
assert.match(world.history_go_affordance.authority_boundary, /kan ikke|ikke gi/i);
assert.match(world.history_go_affordance.authority_boundary, /konklusjon|myndighet|delegasjon|budsjett/i);
...
assert.ok(readiness.summary.role_world_complete_or_pilot >= 56);
assert.ok(readiness.summary.rollout_queue_roles <= 29);
...
const source = fs.readFileSync(path.join(ROOT, 'reports/CIVICATION_HISTORIE_FAGLEDELSE_ROLE_WORLD_ROLLOUT_SOURCE_FIRST.md'), 'utf8');
...
console.log('Civication Historie Fagledelse Role World rollout: OK');
