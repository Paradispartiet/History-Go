...
const replaceAllCount=(file,from,to,expected)=>{const s=fs.readFileSync(file,'utf8');const count=s.split(from).length-1;assert(count===expected,`Forventet ${expected} treff i ${file}, fant ${count}: ${from.slice(0,100)}`);const n=s.split(from).join(to);assert(n!==s,`Ingen endring i ${file}`);fs.writeFileSync(file,n)};
...
replaceAllCount(workflow,scriptBrief,`${scriptBrief}\n${scriptFull}`,2);
...
replaceAllCount(workflow,testBrief,`${testBrief}\n${testFull}`,2);
