...
export async function auditByBylivSosialOffentlighetPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters.length === 16, 'By skal nå ha ni registrerte Fase 4-kapitler totalt');
  ...
  assert(model.chapters.length === 16, 'Normalisert By-modell skal vise ni kapitler etter første Arkitektur-batch');
  ...
  const report = {
    ...
    gates: {
      ...
      secondChapterPreservedAcrossTwelveChapterRegistry: true,
