// Split Musikk Oslo places aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/musikk/oslo');
const sourceFile = 'places_musikk.json';
const sourceStem = 'places_musikk';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/musikk/oslo/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/musikk/oslo/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/musikk/oslo/${sourceFile}`,
