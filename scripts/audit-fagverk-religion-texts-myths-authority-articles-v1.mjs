...
export function auditReligionTextsMythsAuthorityArticles({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(isDeepStrictEqual(readiness.production_progress.completed_area_ids, ['theory_method', 'history_comparison', 'west_asian_abrahamic', 'south_asian_religions', 'east_asian_religions', 'indigenous_sami', 'ritual_materiality_space', AREA_ID, 'society_politics_law']), 'Eksakt ni Religion-områder skal være komplette');
  assert(isDeepStrictEqual(readiness.production_progress.materialized_topic_ids.slice(42, 48), requiredIds), 'Readiness har feil tekster, myter og autoritet-emner');
