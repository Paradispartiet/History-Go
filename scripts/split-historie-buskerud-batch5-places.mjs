// Split Historie Buskerud batch 5 place aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/historie/buskerud');
const sourceFile = 'places_historie_buskerud_batch5.json';
const sourceStem = 'places_historie_buskerud_batch5';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/historie/buskerud/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/historie/buskerud/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/historie/buskerud/${sourceFile}`,
