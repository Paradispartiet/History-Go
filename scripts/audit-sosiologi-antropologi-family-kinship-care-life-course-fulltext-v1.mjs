...
export function audit() {
  ...
  assert(chapter.subject_id === 'politikk' && chapter.canonical_subcategory_id === 'sosiologi_antropologi' && chapter.domain_id === 'familie_slektskap_omsorg_livslop', 'Kapittelet har feil canonicalt eierskap');
  ...
  assert(production.progress.materializedDomains === 6 && production.progress.totalDomains === 12 && production.progress.strictCompletionProven === false, 'Produksjonsregister skal vise nøyaktig 6/12 og strict=false');
  assert(production.materialized.length === 6 && production.materialized.filter((row) => row.ordinal === 6 && row.chapter === P.chapter).length === 1, 'Felt 6 skal være registrert nøyaktig én gang');
  const subcategory = category.canonicalSubcategories.politikk.find((row) => row.id === 'sosiologi_antropologi');
  assert(subcategory?.status === 'expansion_planned', 'Underkategorien må forbli expansion_planned ved 6/12');
...
try {
  const report = audit();
  console.log(`Familie, slektskap, omsorg og livsløp fulltekstaudit OK: ${report.counts.paragraphs} avsnitt, ${report.counts.verifiedClaims} claims, ${report.six_part_quality_review.total}/30.`);
} catch (error) {
