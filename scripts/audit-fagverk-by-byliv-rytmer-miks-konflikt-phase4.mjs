...
export async function auditByBylivRytmerMiksKonfliktPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters.length === 10, 'By skal ha fem Byliv-kapitler og ett Arkitektur-kapittel etter Arkitektur-start');
  ...
  assert(model.chapters.length === 10, 'Normalisert By-modell skal vise ni kapitler etter første Arkitektur-batch');
  ...
  const report = {
    ...
    gates: {
      ...
      fiveBylivChaptersPreservedAcrossTenChapterRegistry: true,
