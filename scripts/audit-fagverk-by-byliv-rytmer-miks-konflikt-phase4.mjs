...
export async function auditByBylivRytmerMiksKonfliktPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters.length === 11, 'By skal ha fem Byliv-kapitler og ett Arkitektur-kapittel etter Arkitektur-start');
  ...
  assert(model.chapters.length === 11, 'Normalisert By-modell skal vise ni kapitler etter første Arkitektur-batch');
  ...
  const report = {
    ...
    gates: {
      ...
      fiveBylivChaptersPreservedAcrossElevenChapterRegistry: true,
