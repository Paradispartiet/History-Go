...
const ID='helsetjenester-helseokonomi-organisering-kvalitet-prioritering-og-ressursbruk',DIR='data/fagverk/helse/'+ID,INPUT='rehabilitation_function_full_chapter_complete_next_domain_source_brief',OUTPUT='health_services_economics_full_chapter_complete_strict_proof_next';
...
export function build(){
 const source=read(P.source),safety=read(P.safety),manifest=read(P.manifest),inventory=read(P.inventory),registry=read(P.registry),status=read(P.status),health=status.subjects.find(x=>x.id==='helse');assert([INPUT,OUTPUT].includes(health.nextGate),'Feil gate '+health.nextGate);assert(safety.status==='blocking','Klinisk sikkerhetskontrakt må være blocking');validateSourceBoundOutputs(source);for(const [file,value] of Object.entries(OUTPUTS))write(file,value);
