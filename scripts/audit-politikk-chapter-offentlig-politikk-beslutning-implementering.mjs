...
const P = Object.freeze({
  chapter: 'data/fagverk/politikk/offentlig-politikk-beslutning-implementering.json',
  brief: 'data/fagverk/politikk/offentlig-politikk-beslutning-implementering/brief.json',
  claims: 'data/fagverk/politikk/offentlig-politikk-beslutning-implementering/claims.json',
  module1: 'data/fagverk/politikk/offentlig-politikk-beslutning-implementering/01-grunnlag.json',
  module2: 'data/fagverk/politikk/offentlig-politikk-beslutning-implementering/02-fordypning.json',
  module3: 'data/fagverk/politikk/offentlig-politikk-beslutning-implementering/03-anvendelse.json',
  ...
  report: 'reports/fagverk/politikk-offentlig-politikk-beslutning-implementering-audit.json'
});
...
export function auditPolitikkOffentligPolitikkChapter({ writeReport = false, checkReport = true } = {}) {
  ...
  const domain = (pensum.domains || []).find((row) => row.domain_id === 'offentlig_politikk_beslutning_implementering');
  assert(domain, 'Pensum mangler fagområdet offentlig politikk, beslutning og implementering');
  ...
  assert(chapter.id === 'offentlig-politikk-beslutning-implementering' && chapter.chapter_id === chapter.id, 'Kapittel-ID er usynkron');
  ...
  const report = {
    schema: 'history_go_politikk_chapter_offentlig_politikk_beslutning_implementering_audit_v1',
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = auditPolitikkOffentligPolitikkChapter({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') });
