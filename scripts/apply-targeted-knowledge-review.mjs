...
let quizzes = read("js/quizzes.js");
quizzes = replaceExact(
  quizzes,
...
quizzes = replaceExact(
  ...
  "            const meta = { correctAnswers, conceptsCorrect, emnersTouched: emnerTouched, emnerTouched, answers };",
  "quiz completion meta"
);
const reviewEngine = String.raw`
...
let memory = read("js/knowledgeQuizMemory.ts");
memory = replaceExact(
  memory,
...
const popupFunction = String.raw`  function knowledgePopupHtml(bundle: JsonObject): string {
...
memory = replaceRegex(
  memory,
  /  function knowledgePopupHtml\(bundle: JsonObject\): string \{[\s\S]*?\n  \}\n\n  function closeKnowledgePopup/,
  popupFunction,
  "review popup"
);
memory = replaceExact(
...
const summaryFunction = String.raw`  function attachBundleToSummary(bundleValue: unknown): boolean {
...
memory = replaceRegex(
  memory,
  /  function attachBundleToSummary\(bundleValue: unknown\): boolean \{[\s\S]*?\n  \}\n\n  function watchForSummary/,
  summaryFunction,
  "summary review action"
);
const reviewCapture = String.raw`
...
const overviewFunction = String.raw`  function renderOverview(profileValue: unknown): void {
...
memory = replaceRegex(
  memory,
  /  function renderOverview\(profileValue: unknown\): void \{[\s\S]*?\n  \}\n\n  function initBrowserIntegration/,
  overviewFunction,
  "Knowledge overview review action"
);
const integrationFunction = String.raw`  function initBrowserIntegration(): void {
...
memory = replaceRegex(
  memory,
  /  function initBrowserIntegration\(\): void \{[\s\S]*?\n  \}\n\n  return \{/,
  integrationFunction,
...
let integrationTest = read("tests/knowledge-profile-memory-integration.test.js");
integrationTest += String.raw`
...
`;
write("tests/knowledge-profile-memory-integration.test.js", integrationTest);

let browserTest = read("tests/knowledge-browser-e2e.test.mjs");
browserTest = replaceExact(
  browserTest,
...
write("tests/knowledge-browser-e2e.test.mjs", browserTest);

const manifestPath = "data/knowledge/knowledge_manifest.json";
...
manifest.runtime.nextRuntimeRequirements = manifest.runtime.nextRuntimeRequirements.filter((item) => !String(item).includes("targeted repetition"));
manifest.runtime.nextRuntimeRequirements.push("Add spaced repetition scheduling after targeted review has stable usage telemetry.");
write(manifestPath, JSON.stringify(manifest, null, 2) + "\n");

console.log("Targeted Knowledge review applied");
