...
const projection=(r)=>({schema:r.schema,version:r.version,status:r.status,generatedFrom:r.generatedFrom,subject:r.subject,summary:r.summary,domainCoverage:r.domainCoverage,chapterIds:r.chapterIds,gates:r.gates});
...
export function auditPsykologiComplete({writeReport=false,checkReport=true}={}){
  ...
  assert(subject.chapters.length===6,'Psykologi må ha 6/6 kapitler før complete-audit');
  ...
  assert(statusEntry.editorialStatus==='complete','Psykologi må stå complete');
  assert(statusEntry.nextGate==='maintenance_source_refresh_and_place_case_expansion','Psykologi har feil vedlikeholdsport');
  ...
  for(const [index,row] of subject.chapters.entries()){
    ...
    assert(modules.length>=3&&sections.length>=9,`${row.id} har for lite redaksjonelt innhold`);
  ...
  const report={schema:'history_go_fagverk_psykologi_complete_audit_v1',version:'1.0.0',status:'psykologi_complete_and_audited',generatedFrom:P,subject:{id:'psykologi',editorialStatus:statusEntry.editorialStatus,nextGate:statusEntry.nextGate,registeredChapterCount:subject.chapters.length,canonicalDomainCount:pensum.summary.domain_count,canonicalEmneCount:pensum.summary.emne_count,canonicalMethodCount:pensum.summary.method_count},summary:{chapterCount:6,domainCount:6,emneCount:58,uniqueEmneCount:58,canonicalMethodCount:58,moduleCount,sectionCount,paragraphCount,claimCount,sourceCount,externalSourceCount},domainCoverage,chapterIds,gates:{sixCanonicalDomainsCovered:true,all58CanonicalEmnersCoveredExactlyOnce:true,allCanonicalChapterMethodSetsResolved:true,allChapterClaimsTraceToSources:true,minimumExternalSourceFloorMetForEveryChapter:true,allChapterBriefsCarryDiagnosisGuard:true,registryAndStatusComplete:true,maintenanceGateSet:true}};
...
function main(){const args=new Set(process.argv.slice(2));try{const r=auditPsykologiComplete({writeReport:args.has('--write-report'),checkReport:!args.has('--no-check-report')&&!args.has('--write-report')});console.log(`Psykologi complete-audit OK: ${r.report.summary.chapterCount}/6 kapitler, ${r.report.summary.emneCount}/58 emner, ${r.report.summary.paragraphCount} avsnitt, ${r.report.summary.claimCount} claims og ${r.report.summary.externalSourceCount} eksterne kilder.`);}catch(e){console.error(`Psykologi complete-audit FEIL: ${e.message}`);process.exitCode=1;}}
