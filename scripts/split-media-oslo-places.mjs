// Split Media Oslo places aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/media/oslo');
const sourceFile = 'places_oslo_media.json';
const sourceStem = 'places_oslo_media';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/media/oslo/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/media/oslo/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/media/oslo/${sourceFile}`,
