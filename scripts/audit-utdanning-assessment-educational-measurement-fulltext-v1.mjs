...
const P = {
  ...
  report: 'reports/fagverk/utdanning-assessment-educational-measurement-fulltext-v1-audit.json',
};
...
export function audit({ writeReport = false } = {}) {
  ...
  assert(claimFile.sources.every((source) => source.url.startsWith('https://') && source.source_location && source.retrieval_status === 'verified_2026-08-26'), 'Kilder må være inspiserbare');
  ...
  assert(!brief.safety.individualDiagnosis && !brief.safety.fixedLearnerTyping && !brief.safety.scoreFatalism && !brief.safety.universalMethodClaim && !brief.safety.groupDifferenceAsIndividualCause, 'Etiske og målingsfaglige grenser mangler');
  ...
  assert(registryRow?.chapters?.filter((row) => row.id === CHAPTER_ID).length === 1 && registryRow.editorialPlan.registeredChapterCount === 7 && registryRow.editorialPlan.completedSourceBriefCount === 8, 'Registry må være 7/14 med åtte source briefs');
  ...
  assert(pensum.domains.filter((domain) => domain.status === 'materialized').length === 7 && pensum.domains[6].domain_id === 'vurdering_pedagogiske_malinger' && !pensum.complete_ready, 'Pensum skal være monotont 7/14');
  assert(nextBrief.scope.primary_domain_id === 'spesialpedagogikk' && !nextBrief.runtime_registration.registered, 'Spesialpedagogikk må være neste uregistrerte source-first-domene');

  const report = {
    schema: 'history_go_utdanning_assessment_educational_measurement_fulltext_audit_v1',
    version: '1.0.0',
    ...
    gates: {
      ...
      purposeBeforeInstrumentBoundary: true,
      scoreNonIdentityBoundary: true,
      validityInterpretationUseBoundary: true,
      reliabilityNonSufficiencyBoundary: true,
      alphaNonDimensionalityBoundary: true,
      modelFitNonProofBoundary: true,
      formativeUseBoundary: true,
      fairnessAccessBoundary: true,
      noIndividualPredictionFromGroupData: true,
      standardSettingNonNaturalBoundary: true,
      chapterRegisteredExactlyOnce: true,
      educationMaterializedAndAudited: true,
      prematureSubjectCompletionBlocked: true,
      specialEducationSourceFirstNext: true,
    },
    six_part_quality_review: {
      source_authority_and_provenance: 5,
      claim_trace_and_verifiability: 5,
      measurement_theory_and_boundary_quality: 5,
      fairness_and_learner_ethics: 5,
      pedagogy_and_assessment: 4,
      architecture_and_reproducibility: 5,
      total: 29,
      maximum: 30,
      note: 'Vurderingspakken er intern; 7 domener og strict subject-proof gjenstår før Utdanning kan bli complete.',
    },
  };
...
try {
  ...
  console.log(`Utdanning Vurdering og pedagogiske målinger fulltekstaudit OK: ${report.counts.paragraphs} avsnitt, ${report.counts.verifiedClaims} claims, ${report.counts.inspectableSources} kilder; ${report.six_part_quality_review.total}/30.`);
} catch (error) {
  console.error(`Utdanning Vurdering og pedagogiske målinger fulltekstaudit FEIL: ${error.message}`);
  process.exitCode = 1;
}
