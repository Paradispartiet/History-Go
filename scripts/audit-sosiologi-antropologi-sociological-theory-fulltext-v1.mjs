...
function expectedReport(sourceBrief) {
  return {
    schema: 'history_go_sosiologi_antropologi_sociological_theory_fulltext_audit_v1', version: '1.0.0', updated_at: '2026-08-28', status: 'pass', conclusion: 'sociological_theory_fulltext_materialized_subcategory_still_expansion_planned', subject_id: 'politikk', canonical_subcategory_id: 'sosiologi_antropologi', chapter_id: CHAPTER_ID,
...
export function audit() {
  ...
  projectSociologyMilestone(production, category, 1);
  ...
  assert(chapter.subject_id === 'politikk' && chapter.canonical_subcategory_id === 'sosiologi_antropologi' && chapter.domain_id === 'sosiologisk_teori', 'Kapittelet har feil canonicalt eierskap');
  ...
  assert(production.progress.materializedDomains === 1 && production.progress.totalDomains === 12 && production.progress.strictCompletionProven === false && production.materialized.filter((row) => row.ordinal === 1 && row.chapter === P.chapter).length === 1, 'Produksjonsregister skal vise nøyaktig 1/12');
  const subcategory = category.canonicalSubcategories.politikk.find((row) => row.id === 'sosiologi_antropologi');
  assert(subcategory?.status === 'expansion_planned', 'Underkategorien må forbli expansion_planned ved 1/12');
...
try { const report = audit(); console.log(`Sosiologisk teori fulltekstaudit OK: ${report.counts.paragraphs} avsnitt, ${report.counts.verifiedClaims} claims, ${report.six_part_quality_review.total}/30.`); }
catch (error) { console.error(`Sosiologisk teori fulltekstaudit FEIL: ${error.message}`); process.exitCode = 1; }
