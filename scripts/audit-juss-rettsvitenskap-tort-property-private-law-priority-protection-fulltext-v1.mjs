...
export function audit() {
  ...
  for (const token of ['skadeserstatningsloven', 'bilansvarslova', 'yrkesskadeforsikringsloven', 'produktansvar', 'forurensningsloven', 'sameigelova', 'grannelova', 'servituttlova', 'hevdslova', 'tinglysingsloven', 'panteloven', 'dekningsloven', 'godtroerverv']) {
    assert(joined.includes(token), `Mangler privatrettslig gate: ${token}`);
  }
  ...
  const report = {
    ...
    gates: {
      ...
      liability_basis_causation_loss_and_remedy_distinguished: true,
      personal_and_proprietary_rights_distinguished: true,
      ownership_coownership_neighbour_law_distinguished: true,
      servitude_and_prescription_timeline: true,
      validity_priority_and_legal_protection_distinguished: true,
      asset_type_and_publicity_act_required: true,
      security_creditor_seizure_and_avoidance_distinguished: true,
      assessment: true,
