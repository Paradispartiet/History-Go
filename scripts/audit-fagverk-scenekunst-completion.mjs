...
export function auditScenekunstCompletion({writeReport=false,checkReport=true,contentOnly=false}={}){
 ...
 const allText=allParagraphs.map(x=>x.text).join('\n'); const allTextNorm=allText.toLowerCase();
 for(const phrase of ['opptak erstatter ikke','arkiv','besøkstall måler oppmøte','kunstnerisk kvalitet','samtykke','universell utforming','representasjon','film & tv','musikk']) assert(allTextNorm.includes(phrase),`Mangler eksplisitt completion-begrensning/grense: ${phrase}`);
