...
source = source
  .replace("const BATCH = 177;", "const BATCH = 178;")
  .replace("if (maxBatch !== 176) throw new Error(`Expected current coordinate max batch 176, got ${maxBatch}. Rebase before batch 177.`);", "if (maxBatch !== 177) throw new Error(`Expected current coordinate max batch 177, got ${maxBatch}. Rebase before batch 178.`);");
if (!source.includes("const BATCH = 178;")) throw new Error('Could not renumber validated Ring 3 runner to batch 178');
if (!source.includes("if (maxBatch !== 177)")) throw new Error('Could not update Ring 3 sequence guard to max batch 177');

const tempScript = path.join('/tmp', `history-go-batch-178-ring3-${Date.now()}.mjs`);
...
console.log(JSON.stringify({ replayedBatch: 178, placeId, activePlaceFile: active.file, routeSegmentCount: active.place?.routeSegments?.length ?? 0 }, null, 2));
