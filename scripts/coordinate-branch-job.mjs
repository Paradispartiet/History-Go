...
function boundaryDistances(line, rings) {
  const projection = projector([...line, ...rings.flat()]);
  const distances = line.map((point) => {
    let min = Infinity;
    for (const ring of rings) {
      for (let i = 0; i < ring.length - 1; i += 1) min = Math.min(min, pointSegmentDistanceMeters(point, ring[i], ring[i + 1], projection));
    }
    return min;
  });
  return {
...
const place = {
  ...
  desc: "Oppdemmet vannflate i øvre Akerselva ved Frysja, kartlagt som Frysja med Brekkedammen som alternativt navn og brukt som bade- og rekreasjonsområde.",
  ...
  anchors: [
    { id: "brekkedammen_demning", name: "Brekkedammen – demningen", type: "boundary_point", lat: weirAnchor.lat, lon: weirAnchor.lon, r: 160 }
  ],
  ...
  nature_profile: {
    ...
    summary: "Brekkedammen er en oppdemmet del av Akerselva der et roligere vannspeil, elvekant og demningsanlegg danner ett sammenhengende natur- og rekreasjonsmiljø.",
