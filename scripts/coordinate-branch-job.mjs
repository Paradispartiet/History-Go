...
for (const file of [
  'tools/audit-oslo-people-coverage.mts',
  'tools/audit-oslo-latent-people-coverage.mts',
]) {
  const before = fs.readFileSync(file, 'utf8');
  const needle = '  return [];\n}\n';
  const replacement = "  if (data && typeof data === 'object' && typeof data.id === 'string' && data.id.trim()) return [data];\n  return [];\n}\n";
  if (!before.includes(needle)) throw new Error(`Expected toArray tail not found in ${file}`);
...
const lines = [
  ...
  '',
  ...(queue.length ? queue.map((entry) => `- \`${entry.placeId}\` — ${entry.name}`) : ['- None.']),
  '',
  '## Audit correction',
  '',
...
run('git', ['add', '-A']);
run('git', ['commit', '-m', 'Count single-record files in Oslo People audits']);
