...
const allowed = new Set([".json", ".jsonl", ".js", ".mjs", ".cjs", ".ts", ".tsx", ".md", ".html", ".yml", ".yaml"]);
const excludedDirs = new Set([".git", "node_modules", "dist", "coverage", "reports"]);
const files = [];
const walk = (path) => {
...
for (const path of files) {
  ...
  for (const id of [OLD_ID, NEW_ID]) {
    ...
    textHits[id].push({ file, occurrenceCount: text.split(id).length - 1, matchingLines: matchingLines.slice(0, 100) });
  }
...
const summary = {
  version: "2026-07-23",
  purpose: "Determine whether unresolved sagene_kvernhus can be retired in favor of canonical glads_molle without breaking references.",
  canonicalCollision: {
    oldId: OLD_ID,
    newId: NEW_ID,
    relationship: "same exact physical object and exact verified Geonorge address coordinate"
  },
