...
const PLACE_ID = "bygdoy_dronningberget";
const PLACE_FILE = "data/places/natur/oslo/places_oslo_natur_bygdoy/bygdoy_dronningberget.json";
const SPLIT_MANIFEST = "data/places/natur/oslo/places_oslo_natur_bygdoy_manifest.json";
const TEST_FILE = "tests/bygdoy-dronningberget-mapped-species-inventory.test.js";
const REPORT_FILE = "reports/bygdoy-dronningberget-mapped-species-inventory.md";
const MAP_FILES = [
...
assert.ok(total > 0, "Dronningberget mangler aktive kartarter");
...
place.quiz_profile = {
  place_type: "kystberg i naturreservat",
  subtype: "kalkrik_torrbakke_og_skogkant",
  signature_features: [
    "vernet bergknaus på østsiden av Bygdøy",
    "overgang mellom åpne bergflater og løvskog",
    "sol- og vindeksponert tørrbakke over indre fjord",
    "aktivt kartinventar av dokumenterte plantearter"
  ],
  primary_angles: [
    "berggrunn_og_jordsmonn",
    "torrbakkevegetasjon",
    "skogkant_og_busksjikt",
    "vern_og_terrengform"
  ],
  question_families: [
    "stedsspesifikk_observasjon",
    "geologi_og_vegetasjon",
    "arter_og_habitat",
    "sammenligning_med_kystsonen"
  ],
  avoid_angles: [
    "generisk_utsiktsquiz",
    "ren_turguide",
    "arter_utenfor_species_inventory",
    "påstander_uten_kildestotte"
  ],
  must_include: [
    "den offisielle vernepolygonen",
    "overgangen mellom berg og skog",
...
assert.ok(splitEntry, "Mangler Dronningberget i split-manifest");
...
for (const file of mapFiles) {
  const entry = read(file).places?.bygdoy_dronningberget;
...
test("Dronningberget inventory matches active nature maps", () => {
  assert.equal(place.coordStatus, "verified_geometry");
  assert.ok(inventory);
...
test("Dronningberget inventory and quiz profile are complete", () => {
...
const report = [
  "# Bygdøy Dronningberget – aktivt artsinventar",
  "",
  "- Flora: **" + flora.length + "**",
  "- Fauna: **" + fauna.length + "**",
  "- Totalt: **" + total + "**",
  ...
  "- Geometri: `verified_geometry` – Miljødirektoratet VV00003059",
  "",
  "Alle arts-ID-er finnes allerede som kanoniske kort. Det er ikke opprettet eller gjettet nye arter. Inventaret er eksakt union av de aktive Dronningberget-koblingene i Oslo-naturkartene."
].join("\n");
...
console.log("Dronningberget species_inventory OK:", flora.length, "flora +", fauna.length, "fauna =", total, "arter");
