...
if (!decision || decision.coordinateStatus !== "ready") throw new Error("Kunstnerforbundet is not coordinate-ready in merged scope.");
...
if (places.some((p) => p.id === placeId || String(p.name ?? "").toLowerCase() === "kunstnerforbundet")) throw new Error("Kunstnerforbundet already exists on current main.");

const c = decision.coordinate;
if (c.sourceObjectId !== "geonorge-adresser-v1:0301:13743:3" || Number(c.lat) !== 59.91286247033279 || Number(c.lon) !== 10.735585135946035) throw new Error("Locked Kunstnerforbundet coordinate changed unexpectedly.");
...
const placeFile = `data/${placeManifestEntry}`;
const evidenceManifestEntry = "oslo/kunst/kunstnerforbundet.json";
const evidenceFile = `data/coordinate-evidence/${evidenceManifestEntry}`;
...
writeJson(placeFile, {
  ...
  quiz_profile: {
    ...
    signature_features: [
      ...
      "eier bygården med visningsrom, lager, kontorer og atelierer",
...
if (!countMatch) throw new Error("Could not parse current Oslo active verified count.");
...
writeJson("reports/visitoslo-galleries-audit-20260723/priority-tranche/kunstnerforbundet-production.json", { version: DATE, placeId, batch: nextBatch, placeManifestEntry, evidenceManifestEntry, coordinateSourceObjectId: c.sourceObjectId, sourceScopeReport: scopePath });
console.log(`Produced ${placeId} as Oslo coordinate batch ${nextBatch}; count ${countMatch[1]} -> ${newCount}.`);
