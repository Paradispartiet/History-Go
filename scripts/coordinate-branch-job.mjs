...
const conceptRows = concepts.filter((item) => issueIds.has(item.concept_id)).map((item) => ({
  concept_id:item.concept_id,label:item.label,definition:item.definition,concept_type:item.concept_type,
...
const theoryRows = theories.filter((item)=>item.explanatory_scope?.includes(domainId)).map((item)=>({
  theory_id:item.theory_id,label:item.label,definition:item.definition,limitations:item.limitations??[],object_type:item.object_type,method_links:item.method_links??[],thinker_ids:item.thinker_ids??[],source_hook_id:item.source_hook_id,evidence_ready:item.evidence_ready
}));
...
fs.writeFileSync(path.join(reportDir,'migrasjon-minoritet-tilhorighet-target-inventory.json'),`${JSON.stringify(report,null,2)}\n`);
