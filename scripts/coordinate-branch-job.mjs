...
function copyValidatedFile(sourcePath, targetPath) { const content = execFileSync('git', ['show', `FETCH_HEAD:${sourcePath}`], { encoding: 'utf8' }); fs.mkdirSync(path.dirname(abs(targetPath)), { recursive: true }); fs.writeFileSync(abs(targetPath), content); }

assertNoActivePlaceId('brannmuseet_oslo');
if (fs.existsSync(abs(PLACE_FILE)) || fs.existsSync(abs(EVIDENCE_FILE))) throw new Error('Brannmuseet source/evidence already exists on current main');
...
if (place.id !== 'brannmuseet_oslo' || place.sourceObjectId !== 'geonorge-adresser-v1:0301:12450:32') throw new Error('Unexpected validated Brannmuseet place payload');
if (evidence.placeId !== place.id || evidence.coordinateDecision !== 'do_not_change_coordinates_yet') throw new Error('Unexpected validated Brannmuseet evidence payload');
...
const row60 = '| 60 | `vestre_gravlund` | Vestre gravlund | verified | `osm-way:4740772` |';
if (protocol.includes(row56)) throw new Error('Batch 56 row already exists');
const lines = protocol.split('\n');
const existingRows = [row54, row55, row57, row58, row59, row60];
const positions = existingRows.map((row) => { const hits = lines.map((line, index) => line === row ? index : -1).filter((index) => index >= 0); if (hits.length !== 1) throw new Error(`Expected exactly one protocol row: ${row}; found ${hits.length}`); return hits[0]; });
...
if (unexpected.length) throw new Error(`Unexpected content between batch rows 54-60: ${unexpected.join(' | ')}`);
lines.splice(start, end - start + 1, row54, row55, row56, row57, row58, row59, row60);
protocol = lines.join('\n');
...
console.log('Copied fully validated Brannmuseet payload onto Oslo 206 baseline as batch 56 and normalized rows 54-60.');
