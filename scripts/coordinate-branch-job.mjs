...
const appendUnique = (values, additions) => [...new Set([...(Array.isArray(values) ? values : []), ...additions])];
...
const pruneSourceButPreserveRetiredRecord = (value) => {
  if (Array.isArray(value)) return value.map((item) => hasRetiredIdentity(item) ? item : pruneRetiredReference(item));
  if (isObject(value) && Array.isArray(value.places)) {
    return { ...value, places: value.places.map((item) => hasRetiredIdentity(item) ? item : pruneRetiredReference(item)) };
  }
  return value;
};
  ...
  if (Array.isArray(value)) {
    return {
      value: value.map((item) => {
        if (isObject(item) && item.id === placeId) { replacements += 1; return replacement; }
        return item;
      }),
      replacements,
    };
...
assert(!/^\|\s*196\s*\|/m.test(protocolBefore), 'Batch 196 already exists; rerun this retirement from the new state.');

const audit = await readJson('reports/oslo-coordinate-ostensjovannet-sivbelte-model-audit-post-195/summary.json');
...
assert(audit.decision === 'model_as_parent_habitat_retire_separate_marker', `Unexpected model decision: ${audit.decision}`);
assert(audit.canonicalChanged === false, 'Research audit unexpectedly changed canonical data.');
assert(audit.findings?.officialReedFeatureCount === 0, 'An official reed geometry candidate now exists; automatic retirement must stop.');
assert(audit.findings?.officialExactFeatureCount === 0, 'An exact official habitat feature now exists; automatic retirement must stop.');
...
assert(audit.findings?.overpassExactNamedElements === 0, 'An exact named contextual object now exists; automatic retirement must stop.');

const runtimeIndexBefore = await readJson(globalIndexRel);
const runtimePlacesBefore = placesFrom(runtimeIndexBefore);
assert(runtimePlacesBefore.some((place) => place.id === retiredId), 'Separate sivbelte marker is already absent from runtime.');
const runtimeParentBefore = runtimePlacesBefore.find((place) => place.id === parentId);
assert(runtimeParentBefore, 'Verified Østensjøvannet parent is absent from runtime.');
...
for (const sourceRel of manifestFiles) {
  ...
  let data;
  try { data = await readJson(sourceRel); } catch { continue; }
...
assert(activeTargetSources.length >= 1, 'No manifest-loaded source contains the sivbelte marker.');
assert(activeParentSources.length >= 1, 'No manifest-loaded source contains Østensjøvannet.');

const parentRecords = [];
for (const sourceRel of activeParentSources) {
  const data = await readJson(sourceRel);
  parentRecords.push(...placesFrom(data).filter((place) => place.id === parentId).map((place) => ({ sourceRel, place })));
}
assert(parentRecords.length >= 1, 'Could not load the active parent record.');
const parentBefore = parentRecords[0].place;
assert(parentBefore.coordStatus === 'verified_geometry', 'Parent is not verified geometry.');
assert(JSON.stringify(parentBefore.nature_profile ?? {}).includes('sivbelter'), 'Parent no longer preserves sivbelte knowledge.');
const lockedParentCoordinate = { lat: parentBefore.lat, lon: parentBefore.lon, r: parentBefore.r };
...
const migratedParent = {
  ...
  rounds: appendUnique(parentBefore.rounds, ['nature', 'leksikon', 'routes']),
  routeId: 'ostensjovannet_vatmark',
};
assert(migratedParent.coordStatus === 'verified_geometry', 'Parent verification status changed.');
assert(migratedParent.lat === lockedParentCoordinate.lat && migratedParent.lon === lockedParentCoordinate.lon && migratedParent.r === lockedParentCoordinate.r,
...
if (await exists(knownParentSplitRel)) {
  const before = await readJson(knownParentSplitRel);
  const replaced = replacePlace(before, parentId, migratedParent);
  assert(replaced.replacements === 1, 'Known Østensjøvannet split file has the wrong shape.');
...
assert(!exclusions.disabledPlaceIds.includes(retiredId), 'Separate sivbelte marker is already disabled.');
...
for (const fullPath of allDataJson) {
  ...
  if (typeof row.file === 'string') targetSourcePaths.add(rel(path.join(path.dirname(fullPath), row.file)));
...
for (const fullPath of allDataJson) {
  const relativePath = rel(fullPath);
  if (relativePath.startsWith('data/places/') && relativePath.endsWith('_manifest.json')) await refreshManifest(relativePath);
}
...
assert(!protocolBefore.includes(protocolRow), 'Protocol already contains the sivbelte retirement row.');
await fs.writeFile(path.join(root, protocolRel), `${protocolBefore.trimEnd()}\n${protocolRow}\n`, 'utf8');
...
assert(parentAfterRecords.length >= 1, 'Migrated parent disappeared from active sources.');
for (const parentAfter of parentAfterRecords) {
  assert(parentAfter.coordStatus === 'verified_geometry', 'Parent lost verified status.');
  assert(parentAfter.lat === lockedParentCoordinate.lat && parentAfter.lon === lockedParentCoordinate.lon && parentAfter.r === lockedParentCoordinate.r,
    'Parent coordinate changed.');
  assert(parentAfter.routeId === 'ostensjovannet_vatmark', 'Wetland route was not attached to the parent.');
  assert(!JSON.stringify(parentAfter).includes(retiredId), 'Parent still references the disabled marker ID.');
}
assert((await readJson(exclusionsRel)).disabledPlaceIds.includes(retiredId), 'Disabled marker ID was not persisted.');
assert(await exists(evidenceRel), 'Coordinate evidence was deleted; source history must be preserved.');
assert(await exists(knownTargetSplitRel), 'Source-led sivbelte record was deleted; it must remain as disabled history.');
