...
const IDS = ['stortinget','oslo_radhus','hoyesteretts_hus','politihuset_gronland','folkets_hus_oslo'];
const EXPECTED = {
...
for (const id of IDS) {
  ...
  if (validated.coordStatus !== 'verified' || validated.sourceProvider !== 'official_address') throw new Error(`${id} is not validated official_address`);
  if (validated.sourceObjectId !== EXPECTED[id]) throw new Error(`${id} source mismatch`);
...
for (const row of manifest.places || []) if (IDS.includes(row.id)) row.sha256 = sha256(`data/places/politikk/oslo/${row.file}`);
...
for (const id of IDS) {
  ...
  if (!sourceLine || targetIndex < 0) throw new Error(`Missing protocol row ${id}`);
...
if (!sourceNote || targetNoteIndex < 0) throw new Error('Missing batch 118 note');
...
console.log(JSON.stringify({ ok: true, replayedBatch: 118, verifiedOfficialAddressIds: IDS.map((id) => ({ id, sourceObjectId: EXPECTED[id] })) }, null, 2));
