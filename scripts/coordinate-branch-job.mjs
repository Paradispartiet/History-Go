...
const emneRows = emner
  .filter((item) => item.domain_id === domainId || item.domain_ids?.includes(domainId) || String(item.emne_id).includes('offentlighet_mobilisering'))
...
fs.writeFileSync(path.join(reportDir, 'offentlighet-mobilisering-bevegelser-inventory.json'), `${JSON.stringify(report, null, 2)}\n`);
