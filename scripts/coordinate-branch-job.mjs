...
const conceptRows = concepts
  .filter((item) => issueIds.has(item.concept_id))
  .map((item) => ({
...
fs.writeFileSync(path.join(reportDir, 'offentlighet-mobilisering-bevegelser-target-inventory.json'), `${JSON.stringify(report, null, 2)}\n`);
fs.rmSync(path.join(reportDir, 'offentlighet-mobilisering-bevegelser-inventory.json'), { force: true });
