...
const lines = [
  ...
  '', '# theories',
  ...theories.map((item) => `${item.theory_id}\t${item.label}\t${item.object_type}`),
  '', '# emner',
  ...emner.map((item) => `${item.emne_id}\t${item.title}\tCORE=${A(item.core_concepts).join('|')}\tSUB=${A(item.sub_concepts).join('|')}`)
];
fs.mkdirSync(reportDir, { recursive: true });
fs.writeFileSync(path.join(reportDir, 'kilder-arkiv-spor-curation-labels.txt'), lines.join('\n') + '\n');
...
run('git', ['add', '-A']);
run('git', ['commit', '-m', 'List source archive and trace curation objects']);
