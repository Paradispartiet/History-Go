...
if (!source.includes(testMarker)) throw new Error('Could not find quiz-production test marker in phase 8 builder');
...
const migration = String.raw`
{
  ...
  for (const item of migrations) {
    ...
    write(item.quiz, quiz);

    const brief = read(item.brief);
