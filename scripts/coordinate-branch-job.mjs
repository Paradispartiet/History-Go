...
const sourceValidator = String.raw`#!/usr/bin/env node
import fs from 'node:fs';
import { spawnSync } from 'node:child_process';
...
`;
fs.writeFileSync('tools/validate-historie-quality-kilder-arkiv-spor.mjs', sourceValidator);
...
run('git', ['add', '-A']);
run('git', ['commit', '-m', 'Add permanent source-domain quality validator']);
