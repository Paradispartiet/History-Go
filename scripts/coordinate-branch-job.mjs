...
const reportDir = 'reports/oslo-attractions-completeness-20260720/galleri-map';
const selfPath = 'scripts/coordinate-branch-job.mjs';
const address = 'Tøyengata 32 Oslo';
...
fs.writeFileSync(path.join(reportDir, 'README.md'), `# Galleri MAP – address-first coordinate intake\n\n- Candidate: \`galleri_map\`\n- Query: \`${address}\`\n- Method: normative \`places:coords:find:address\` against Geonorge Adresser API.\n- Result status: \`${result.status}\`\n- Source object: \`${result.sourceObjectId ?? 'none'}\`\n\nNo canonical place data is changed in this intake.\n`);
if (fs.existsSync(selfPath)) fs.unlinkSync(selfPath);
console.log(JSON.stringify({ candidateId: 'galleri_map', query: address, status: result.status, sourceObjectId: result.sourceObjectId, coordinate: result.coordinate ?? null }, null, 2));
