...
if (!countMatch || Number(countMatch[1]) !== 331) throw new Error(`Expected Oslo total 331, found ${countMatch?.[1] ?? 'missing'}`);
if (!protocol.includes('| 115 | `bogstadvannet` |')) throw new Error('Batch 115 rows missing');
if (protocol.includes('| 116 | `oslo_golfklubb_bogstad` |')) throw new Error('Batch 116 golf row already exists');

protocol = protocol.replace(
  /Oslo-tabellen inneholder nå 331 dokumenterte verifiserte eller kildekontrollerte canonical steder\.[^\n]*/,
...
if (batch115Index < 0) throw new Error('Batch 115 narrative missing');
...
console.log('Synced validated Oslo Golfklubb payload as coordinate batch 116 on 331-count baseline.');
