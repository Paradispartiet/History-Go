...
const domainId = 'his_1814_statsdannelse';
...
const conceptRows = concepts.filter((item) => issueIds.has(item.concept_id)).map((item) => ({
  ...
  issues: issueRows.find((row) => row.concept_id === item.concept_id)?.issues ?? []
}));
...
const report = {
  ...
  domain_id: domainId,
  counts: { concepts_with_validator_issues: conceptRows.length, theories: theoryRows.length },
  concept_ids: conceptRows.map((item) => item.concept_id),
  theory_ids: theoryRows.map((item) => item.theory_id),
  concepts: conceptRows,
  theories: theoryRows
};
fs.mkdirSync(reportDir, { recursive: true });
fs.writeFileSync(path.join(reportDir, '1814-statsdannelse-target-inventory.json'), `${JSON.stringify(report, null, 2)}\n`);
