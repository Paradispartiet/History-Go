...
if (anchorIndex < 0) throw new Error('Could not find Oslo table end anchor.');
const osloTableRegion = protocol.slice(0, anchorIndex);
const osloRows = [...osloTableRegion.matchAll(/^\|\s*(\d+)\s*\|.*$/gm)];
if (!osloRows.length) throw new Error('No Oslo table rows found.');
...
protocol = `${protocol.trimEnd()}\n\nBatch ${nextBatch} (2026-07-20) retter protokollplasseringen for \`${id}\`. Den opprinnelige produksjonsmergen la ved en feil batch-74-raden nederst i Etne-tabellen; FRIGO tok senere korrekt Oslo-batch 74 og Galleri MAP batch 75. Denne reparasjonen fjerner den feilplasserte raden og den gamle batch-74-teksten, registrerer Ekeberg i riktig Oslo-tabell på neste ledige batch, og gjenoppretter \`${evidenceRel}\` i coordinate-evidence-manifestet. Canonical place, runtime-identitet, coordinate snapshot og den Riksantikvaren-verifiserte geometrien \`kulturminnesok:41907\` endres ikke, og Oslo-totalen forblir ${currentCount}.\n`;
