...
function copyFromValidatedBranch(sourcePath, targetPath) {
...
if (place.id !== 'oslo_reptilpark') throw new Error('Unexpected copied place payload');
if (place.sourceObjectId !== 'geonorge-adresser-v1:0301:16935:2') throw new Error('Unexpected Reptilpark coordinate source');
...
protocol = replaceOnce(
  protocol,
  'Oslo-tabellen inneholder nå 196 verifiserte eller kildekontrollerte canonical steder. Duplikatet `jernbanetorget_trafikknutepunkt` er migrert til `jernbanetorget` uten å opprette et nytt fysisk sted. Antallet fullførte kontroller uten godkjent Oslo-koordinat er 35.',
  'Oslo-tabellen inneholder nå 197 verifiserte eller kildekontrollerte canonical steder. Batch 51 legger til Oslo Reptilpark med det entydige Geonorge-punktet for dagens besøksadresse i St. Olavs gate 2, samtidig som åpningen i Storgata i 2002 bevares som et separat historisk lokaliseringslag. Antallet fullførte kontroller uten godkjent Oslo-koordinat er 35.',
  'Oslo summary'
);
protocol = replaceOnce(
  protocol,
...
console.log('Copied the validated Oslo Reptilpark payload onto fresh main and registered coordinate batch 51.');
