...
export function auditVitenskapChemistryMaterialScienceFulltext() {
  ...
  assert(readiness.current_inventory?.vitenskap?.registered_chapter_count===4, 'Readiness må registrere fire Vitenskap-kapitler');
  assert(isDeepStrictEqual(sorted(readiness.editorial_blockers||[]),sorted(EXPECTED_REMAINING_BLOCKERS)), 'Etter Unit 4 skal bare medisin blokkere breadth completion');
  ...
  assert(registrySubject?.chapters?.length===4 && registryChapter, 'Vitenskap-registry skal ha fire kapitler inkludert Unit 4');
  ...
  assert(releaseSubject?.chapter_status==='materialized' && releaseSubject?.chapter_count===4 && releaseSubject?.missing_chapter_files?.length===0, 'Release må materialisere fire Vitenskap-kapitler uten manglende filer');
  return {schema:'history_go_fagverk_vitenskap_chemistry_material_science_fulltext_audit_v1',version:'1.0.0',status:'pass',subject:'vitenskap',chapterId:CHAPTER_ID,summary:{emneCount:6,methodCount:8,moduleCount:3,sectionCount:9,paragraphCount:27,sourceCount:12,claimCount:20,misconceptionCount:4,workedExampleCount:2,applicationTaskCount:4,selfCheckCount:6,registeredChapterCount:4,remainingEditorialBlockerCount:1},gates:{structureReactionMeasurementBoundaryLocked:true,thermodynamicsKineticsBoundaryLocked:true,sampleSignalInferenceBoundaryLocked:true,claimTraceReciprocalAndComplete:true,sourceClaimIntegrityPreserved:true,chemistryChapterMaterializedAndRegistered:true,chemistryEditorialBlockerResolved:true,oneBreadthEditorialBlockerRemains:true,prematureCompleteBlocked:true,technologyRemainsNested:true}};
}
