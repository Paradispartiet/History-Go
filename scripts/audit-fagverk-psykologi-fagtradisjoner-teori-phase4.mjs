...
const P = Object.freeze({
  ...
  report: 'reports/fagverk/psykologi-fagtradisjoner-teori-phase4-audit.json'
});
const MODULES = [
...
const EXPECTED_THEORY_CASES = ['Wundts laboratorium i Leipzig', 'Freud og psykoanalysen', 'Skinner og behaviorismen', 'Den kognitive vendingen'];
const REQUIRED_SOURCE_IDS = [
  'src-leipzig-wundt', 'src-harvard-history', 'src-testing-standards', 'src-sep-consciousness',
  'src-apa-unconscious', 'src-who-icd11-cddr', 'src-freud-museum-about', 'src-freud-legacy',
  'src-harvard-skinner', 'src-apa-behaviorism', 'src-apa-cognitive', 'src-sep-cognitive-science',
  'src-apa-humanistic', 'src-apa-humanistic-handbook', 'src-apa-motivation', 'src-apa-neuropsych',
  'src-ncbi-emotions', 'src-pmc-ffm', 'src-pubmed-ffm', 'src-pmc-personality-clinical', 'src-hg-uio-place'
];
...
const committedProjection = (report) => ({
  ...
  runtimePlaceIds: report.runtimePlaceIds, theoryCaseNames: report.theoryCaseNames, gates: report.gates
});

export function auditPsykologiFagtradisjonerPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(canonicalEmneIds.length === 14, 'Canonicalt domene skal ha 14 emner');
  assert(isDeepStrictEqual(chapter.emne_ids, canonicalEmneIds), 'Kapittelet dekker ikke canonicale emner i eksakt rekkefølge');
  ...
  assert(brief.safety?.noPersonalityTypingFromCasualObservation === true && claimsDoc.source_policy?.noPersonalityTypingFromCasualObservation === true, 'Personlighetstypingsvernet mangler');
  ...
  assert(sectionEmneIds.size === 14 && canonicalEmneIds.every((id) => sectionEmneIds.has(id)), 'Seksjonene dekker ikke 14/14 emner');
  ...
  assert(sources.length === 21 && externalSources.length === 20 && claims.length === 27, 'Kapittelet skal ha 21 kilder, 20 eksterne og 27 claims');
  ...
  assert(REQUIRED_SOURCE_IDS.every((id) => sourceIds.has(id)), 'Et låst kildespor mangler');
  ...
  const theoryCases = chapter.theoryCases || [];
  const theoryCaseNames = theoryCases.map((item) => item.name);
  assert(isDeepStrictEqual(theoryCaseNames, EXPECTED_THEORY_CASES), 'Teoricasene avviker fra låst case-sett');
  assert(theoryCases.every((item) => item.caseStatus === 'documented_case_not_runtime_place'), 'Teoricase later som runtime-sted');

  const registrySubject = registry.subjects?.psykologi;
  assert(registrySubject?.chapters?.length === 2, 'Psykologi skal ha nøyaktig to registrerte kapitler');
  ...
  const forbiddenPatterns = [/du har (?:en|et) [a-zæøå-]+lidelse/i, /du er (?:deprimert|psykotisk|bipolar)/i, /testen viser at du har/i, /big five viser at du er/i];
  assert(forbiddenPatterns.every((pattern) => !pattern.test(JSON.stringify({ chapter, brief, modules }))), 'Kapittelet inneholder diagnostisk eller typestemplende språk');

  const report = {
    schema: 'history_go_fagverk_psykologi_fagtradisjoner_teori_phase4_audit_v1', version: '1.0.0',
    status: 'psykologi_fagtradisjoner_teori_chapter_ready', generatedFrom: P,
    subject: { id: 'psykologi', editorialStatus: statusEntry.editorialStatus, nextGate: statusEntry.nextGate, registeredChapterCount: registrySubject.chapters.length, targetChapterCount: registrySubject.editorialPlan.targetChapterCount },
    ...
    summary: { emneCount: 14, methodCount: 18, moduleCount: 3, sectionCount: 9, paragraphCount: 27, claimCount: 27, sourceCount: 21, externalSourceCount: 20, runtimePlaceCount: runtimePlaceIds.length, theoryCaseCount: theoryCases.length },
    canonicalEmneIds, methodIds: chapter.method_ids, runtimePlaceIds, theoryCaseNames,
    gates: { exactCanonicalEmneCoverage: true, exactCanonicalMethodCoverage: true, threeModulesNineSectionsTwentySevenParagraphs: true, paragraphClaimTraceComplete: true, allClaimsUsedAndSourceResolved: true, twentyExternalSourcesPresent: true, doNotDiagnosePeopleGuardPresent: true, noPersonalityTypingGuardPresent: true, noInventedRuntimePlaces: true, theoryCasesExplicitlyNonRuntime: true, registrySynchronizedAtTwoOfSix: true, statusChaptersInProgress: true }
  };
  if (writeReport) {
...
function main() {
  ...
  try {
    const result = auditPsykologiFagtradisjonerPhase4({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') && !args.has('--write-report') });
    console.log(`Psykologi Fagtradisjoner Phase 4 OK: ${result.report.summary.emneCount}/14 emner, ${result.report.summary.methodCount} metoder, ${result.report.summary.paragraphCount} avsnitt, ${result.report.summary.claimCount} claims og ${result.report.summary.externalSourceCount} eksterne kilder.`);
  } catch (error) {
    console.error(`Psykologi Fagtradisjoner Phase 4 FEIL: ${error.message}`);
    process.exitCode = 1;
  }
}
