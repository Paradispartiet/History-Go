// Split Norge før 1500 batch 4 aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/historie/norge');
const sourceFile = 'places_historie_norge_for_1500_batch4.json';
const sourceStem = 'places_historie_norge_for_1500_batch4';
...
function indexRow(place, file) {
  return {
    ...
    fylke: place.fylke ?? null,
    kommune: place.kommune ?? null,
    lat: place.lat ?? null,
    lon: place.lon ?? null,
    r: place.r ?? null,
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/historie/norge/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/historie/norge/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/historie/norge/${sourceFile}`,
