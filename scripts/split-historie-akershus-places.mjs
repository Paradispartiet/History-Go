// Split Historie Akershus aggregate place files into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { basename, join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/historie/akershus');

const sources = [
  'places_historie_akershus_batch1.json',
  'places_historie_akershus_batch2.json',
  'places_historie_akershus_batch3.json',
  'places_historie_akershus_batch4.json',
  'places_historie_akershus_batch5.json',
];
...
async function splitSource(sourceFile) {
  ...
  if (!Array.isArray(places)) {
    throw new Error(`Expected data/places/historie/akershus/${sourceFile} to be a JSON array.`);
  }
  ...
  const manifest = {
    ...
    source_path: `data/places/historie/akershus/${sourceFile}`,
  ...
  const report = [
    ...
    '',
    `Source: data/places/historie/akershus/${sourceFile}`,
