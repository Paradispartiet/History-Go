...
export function audit() {
  ...
  assert(registry.progress.materializedDomains === report.production_plan.materialized && registry.progress.materializedDomains === 1 && registry.progress.totalDomains === 12 && registry.progress.strictCompletionProven === false, 'Reconciliation og produksjonsregister må vise ærlig 1/12 uten strict proof');
  ...
  assert(report.production_plan.source_first_ready === 2 && report.production_plan.next_domain === 'antropologisk_teori', 'Neste source-first-port må være antropologisk teori');
  return { status: 'pass', domains: ci.domains.length, materialized: 1, findings: report.findings.length, reuseWithExpansion: 3 };
}
