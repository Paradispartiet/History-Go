...
export function audit() {
  ...
  assert(materialized === report.production_plan.materialized && materialized > 0 && materialized <= 12 && registry.progress.totalDomains === 12, 'Reconciliation og produksjonsregister må vise gyldig fremdrift');
  assert(subcategory?.status === (strictCompletionProven ? 'foundation_materialized' : 'expansion_planned'), 'Canonical status må følge strict completion-porten');
  ...
  if (strictCompletionProven) {
    assert(report.status === 'authority_audit_complete_strict_subcategory_completion_proven' && report.production_plan.source_first_ready === 12 && report.production_plan.next_domain === null && report.production_plan.strict_completion_proven === true, 'Strict completion må lukke produksjonsplanen uten neste domene');
    assert(ci.domains.every((row) => row.sourceBrief && row.sourceBriefScript && row.sourceBriefTest && row.fulltextMaterializer && row.fulltextAudit && row.fulltextTest), 'Alle tolv domener må ha source-first- og fulltekstkontrakt');
  } else {
  ...
  return { status: 'pass', domains: ci.domains.length, materialized, strictCompletionProven, findings: report.findings.length, reuseWithExpansion: 3, nextDomain: nextDomain?.domainId ?? null };
}
