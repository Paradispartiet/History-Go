...
const P = {
  ...
  report: 'reports/fagverk/utdanning-education-policy-fulltext-v1-audit.json',
};
...
export function audit({ writeReport = false } = {}) {
  ...
  assert(!brief.safety.partisanRecommendationAsFact && !brief.safety.lawAsImplementationProof && !brief.safety.indicatorAsCompleteQuality && !brief.safety.resultGapAsCause && !brief.safety.averageAsDistributionProof, 'Utdanningspolitiske og etiske grenser mangler');
  ...
  assert(registryRow?.chapters?.filter((row) => row.id === CHAPTER_ID).length === 1 && registryRow.editorialPlan.registeredChapterCount === 11 && registryRow.editorialPlan.completedSourceBriefCount === 12, 'Registry må være 11/14 med tolv source briefs');
  ...
  assert(pensum.domains.filter((domain) => domain.status === 'materialized').length === 11 && pensum.domains[10].domain_id === 'utdanningspolitikk' && !pensum.complete_ready, 'Pensum skal være monotont 11/14');
  assert(nextBrief.scope.primary_domain_id === 'skoleledelse_organisasjon' && !nextBrief.runtime_registration.registered, 'Skoleledelse og organisasjon må være neste uregistrerte source-first-domene');

  const report = {
    schema: 'history_go_utdanning_education_policy_fulltext_audit_v1',
    version: '1.0.0',
    ...
    gates: {
      ...
      rightCapacityBoundary: true,
      contestedValuesBoundary: true,
      instrumentMechanismBoundary: true,
      decentralizationCapacityBoundary: true,
      policyEnactmentBoundary: true,
      indicatorQualityBoundary: true,
      gapCausalityBoundary: true,
      distributionBoundary: true,
      democraticEvaluationBoundary: true,
      policyRevisionLearningLoop: true,
      chapterRegisteredExactlyOnce: true,
      educationMaterializedAndAudited: true,
      prematureSubjectCompletionBlocked: true,
      schoolLeadershipOrganizationSourceFirstNext: true,
    },
    six_part_quality_review: {
      source_authority_and_provenance: 5,
      claim_trace_and_verifiability: 5,
      policy_theory_governance_and_boundary_quality: 5,
      rights_equity_and_democratic_legitimacy: 5,
      pedagogy_and_assessment: 4,
      architecture_and_reproducibility: 5,
      total: 29,
      maximum: 30,
      note: 'Utdanningspolitikk-pakken er intern; 3 domener og strict subject-proof gjenstår før Utdanning kan bli complete.',
    },
  };
...
try {
  ...
  console.log(`Utdanning Utdanningspolitikk fulltekstaudit OK: ${report.counts.paragraphs} avsnitt, ${report.counts.verifiedClaims} claims, ${report.counts.inspectableSources} kilder; ${report.six_part_quality_review.total}/30.`);
} catch (error) {
  console.error(`Utdanning Utdanningspolitikk fulltekstaudit FEIL: ${error.message}`);
  process.exitCode = 1;
}
