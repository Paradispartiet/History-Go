...
const P = Object.freeze({
  chapter: 'data/fagverk/politikk/konflikt-makt-sivilsamfunn.json',
  brief: 'data/fagverk/politikk/konflikt-makt-sivilsamfunn/brief.json',
  claims: 'data/fagverk/politikk/konflikt-makt-sivilsamfunn/claims.json',
  module1: 'data/fagverk/politikk/konflikt-makt-sivilsamfunn/01-grunnlag.json',
  module2: 'data/fagverk/politikk/konflikt-makt-sivilsamfunn/02-fordypning.json',
  module3: 'data/fagverk/politikk/konflikt-makt-sivilsamfunn/03-anvendelse.json',
  ...
  report: 'reports/fagverk/politikk-konflikt-makt-sivilsamfunn-audit.json'
});
...
export function auditPolitikkKonfliktMaktSivilsamfunnChapter({ writeReport = false, checkReport = true } = {}) {
  ...
  const domain = (pensum.domains || []).find((row) => row.domain_id === 'konflikt_makt_sivilsamfunn');
  assert(domain, 'Pensum mangler fagområdet konflikt, makt og sivilsamfunn');
  ...
  assert(chapter.id === 'konflikt-makt-sivilsamfunn' && chapter.chapter_id === chapter.id, 'Kapittel-ID er usynkron');
  ...
  assert(equalSet(chapter.emne_ids || [], domain.emne_ids || []), 'Kapittelet dekker ikke nøyaktig fagområdets tretten emner');
  ...
  assert(equalSet(chapter.method_ids || [], domain.method_ids || []), 'Kapittelet dekker ikke nøyaktig fagområdets sytten metoder');
  ...
  assert(hasRenderableWorkedExampleAnalysis(workedExamples), 'Arbeidseksemplene mangler renderbare analysesteg');
  ...
  const report = {
    schema: 'history_go_politikk_chapter_konflikt_makt_sivilsamfunn_audit_v1',
    ...
    gates: {
      ...
      workedExamplesAndMisconceptions: true, renderableWorkedExampleAnalysis: true,
      tasksAndSelfCheck: true, inspectableSources: true, paragraphLevelClaimTrace: true,
      allClaimsUsed: true, registryAndRuntimeSynced: true, honestEditorialStatus: true
    }
  };
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = auditPolitikkKonfliktMaktSivilsamfunnChapter({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') });
