...
for(const f of [ADJ,REGISTRY,PORTAL,LEGACY_BADGE,COMPATIBILITY]) if(!exists(f)) throw new Error(`Mangler ${f}`);
...
const compatibilityRedirectPresent=compatibilityHtml.includes('location.replace')&&compatibilityHtml.includes('subject=naeringsliv#fagverkIaProgresjon')&&!/merke-blokk|<h2>1\. Felt<\/h2>|profesjonalitet|offshoring/i.test(compatibilityHtml);
...
if(!report.summary.portalRedirected) throw new Error(`Næringsliv badgePage må peke til ${TARGET} etter grønn adjudisering.`);
if(!report.summary.legacyBadgeSourcePreserved) throw new Error('Arkivert Næringsliv-teori må bevares som auditkilde.');
if(!report.summary.compatibilityRedirectPresent) throw new Error('Legacy Næringsliv-URL må være en compatibility-redirect til Progresjon.');
