...
export function auditPsykologiSosialpsykologiNormalitetStigmaPhase4({writeReport=false,checkReport=true}={}){
  ...
  const registrySubject=registry.subjects?.psykologi; assert(registrySubject?.chapters?.length===5,'Psykologi skal ha 5/6 registrerte kapitler');
  ...
  assert(registrySubject.editorialPlan?.targetChapterCount===6&&registrySubject.editorialPlan?.nextGate==='remaining_domain_chapter_production','Feil 5/6-plan');
