...
const P = Object.freeze({
  chapter: 'data/fagverk/politikk/rett-lov-rettssikkerhet.json',
  brief: 'data/fagverk/politikk/rett-lov-rettssikkerhet/brief.json',
  claims: 'data/fagverk/politikk/rett-lov-rettssikkerhet/claims.json',
  module1: 'data/fagverk/politikk/rett-lov-rettssikkerhet/01-grunnlag.json',
  module2: 'data/fagverk/politikk/rett-lov-rettssikkerhet/02-fordypning.json',
  module3: 'data/fagverk/politikk/rett-lov-rettssikkerhet/03-anvendelse.json',
  ...
  report: 'reports/fagverk/politikk-rett-lov-rettssikkerhet-audit.json'
});
...
export function auditPolitikkRettLovRettssikkerhetChapter({ writeReport = false, checkReport = true } = {}) {
  ...
  const domain = (pensum.domains || []).find((row) => row.domain_id === 'rett_lov_rettssikkerhet');
  assert(domain, 'Pensum mangler fagområdet rett, lov og rettssikkerhet');
  ...
  assert(chapter.id === 'rett-lov-rettssikkerhet' && chapter.chapter_id === chapter.id, 'Kapittel-ID er usynkron');
  ...
  assert(equalSet(chapter.emne_ids || [], domain.emne_ids || []), 'Kapittelet dekker ikke nøyaktig fagområdets elleve emner');
  ...
  assert(equalSet(chapter.method_ids || [], domain.method_ids || []), 'Kapittelet dekker ikke nøyaktig fagområdets femten metoder');
  ...
  const report = {
    schema: 'history_go_politikk_chapter_rett_lov_rettssikkerhet_audit_v1',
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = auditPolitikkRettLovRettssikkerhetChapter({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') });
