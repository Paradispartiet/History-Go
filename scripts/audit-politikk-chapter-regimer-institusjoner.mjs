...
const P = Object.freeze({
  ...
  brief: 'data/fagverk/politikk/regimer-og-institusjoner/brief.json',
  claims: 'data/fagverk/politikk/regimer-og-institusjoner/claims.json',
  module1: 'data/fagverk/politikk/regimer-og-institusjoner/01-grunnlag.json',
  module2: 'data/fagverk/politikk/regimer-og-institusjoner/02-fordypning.json',
  module3: 'data/fagverk/politikk/regimer-og-institusjoner/03-anvendelse.json',
  ...
  report: 'reports/fagverk/politikk-regimer-institusjoner-audit.json'
});
...
export function auditPolitikkRegimerChapter({ writeReport = false, checkReport = true } = {}) {
  ...
  const domain = (pensum.domains || []).find((row) => row.domain_id === 'komparativ_politikk_regimer_institusjoner');
  assert(domain, 'Pensum mangler komparativt politikk-fagområde');
  ...
  assert(chapter.id === 'regimer-og-institusjoner' && chapter.chapter_id === chapter.id, 'Kapittel-ID er usynkron');
  ...
  assert(equalSet(chapter.emne_ids || [], domain.emne_ids || []), 'Kapittelet dekker ikke nøyaktig fagområdets 15 emner');
  ...
  assert(equalSet(chapter.method_ids || [], domain.method_ids || []), 'Kapittelet dekker ikke nøyaktig fagområdets 16 metoder');
  ...
  for (const [moduleIndex, module] of modules.entries()) {
    assert((module.sections || []).length >= (moduleIndex === 1 ? 3 : 2), `Modul ${moduleIndex + 1} har for få redigerte seksjoner`);
  ...
  assert(sources.length >= 8, 'Kapittelet har for få inspectable kilder');
  assert(claims.length >= 25, 'Kapittelet har for få påstandsposter');
  ...
  const allText = [read(P.chapter), read(P.brief), ...modules.map((_, index) => read([P.module1, P.module2, P.module3][index]))].join('\n').toLowerCase();
  ...
  const report = {
    schema: 'history_go_politikk_chapter_regimer_institusjoner_audit_v1',
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = auditPolitikkRegimerChapter({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') });
