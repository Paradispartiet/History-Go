// Split Oslo nature salamander ponds aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/natur/oslo');
const sourceFile = 'places_oslo_natur_salamanderdammer.json';
const sourceStem = 'places_oslo_natur_salamanderdammer';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/natur/oslo/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/natur/oslo/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/natur/oslo/${sourceFile}`,
