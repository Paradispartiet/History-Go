// Split Lisbon subculture aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/subkultur/europe/portugal/lisbon');
const sourceFile = 'places_lisbon_subkultur.json';
const sourceStem = 'places_lisbon_subkultur';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/subkultur/europe/portugal/lisbon/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/subkultur/europe/portugal/lisbon/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/subkultur/europe/portugal/lisbon/${sourceFile}`,
