...
const DOMAINS = [
  {
    ...
    topics: [
      ...
      t('em_sub_postsubkultur_scener_nettverk', 'Postsubkultur, scener og nettverk', 'Prøver teorier om flytende, hybride og nettverksbaserte tilhørigheter mot mer sammenhengende subkulturmodeller.', 'Forklarer caset best varige grenser, skiftende scene-deltakelse eller et nettverk på tvers av steder?', 'Mobilitet og digitale koblinger kan gjøre medlemskap situasjonelt uten å fjerne sosial struktur.', 'stabil gruppe vs flytende tilhørighet', ['met_sub_begrepsavgrensning', 'met_sub_nettverksanalyse'], ['tidsserier', 'nettverksspor', 'flerstedlig dokumentasjon'], 'Flytende identitet må ikke brukes som påskudd for svak dokumentasjon.')
    ]
  },
  {
...
function main() {
  ...
  else console.log(`Subkultur canonical V2 OK: ${DOMAINS.length} domener, ${DOMAINS.flatMap((domain) => domain.topics).length} emner og ${METHODS.length} metoder.`);
}
