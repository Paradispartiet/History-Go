...
const REPORT_DIR = 'reports/oslo-coordinate-control-batch-43';
...
sourceQuiz.merge_notes = {
  ...sourceQuiz.merge_notes,
  existing_quiz_status: 'Existing 5×6 set retained and corrected in batch 43 after a gate-validated source pass.'
};
...
if (!protocol.includes(row43)) {
  ...
  if (!protocol.includes(anchorRow)) throw new Error('Current main batch-42 anchor row missing');
...
const report = {
  ...
  concurrentMainSafety: 'Only the oslo_kornmagasin row is replaced in current aggregate/index/manifest, preserving museum batch 42 and unrelated main changes.',
