...
const P = Object.freeze({
  chapter: 'data/fagverk/naeringsliv/handel-forbruk-marked.json',
  brief: 'data/fagverk/naeringsliv/handel-forbruk-marked/brief.json',
  claims: 'data/fagverk/naeringsliv/handel-forbruk-marked/claims.json',
  module1: 'data/fagverk/naeringsliv/handel-forbruk-marked/01-grunnlag.json',
  module2: 'data/fagverk/naeringsliv/handel-forbruk-marked/02-fordypning.json',
  module3: 'data/fagverk/naeringsliv/handel-forbruk-marked/03-anvendelse.json',
  ...
  report: 'reports/fagverk/naeringsliv-handel-forbruk-marked-audit.json'
});
...
const completeTrace = (values) => Array.isArray(values)
...
export function auditNaeringslivHandelForbrukMarkedChapter({ writeReport = false, checkReport = true } = {}) {
  ...
  const domain = (pensum.domains || []).find((row) => row.domain_id === 'handel_forbruk_marked');

  assert(domain, 'Pensum mangler fagområdet handel, forbruk og marked');
  ...
  assert(chapter.id === 'handel-forbruk-marked' && chapter.chapter_id === chapter.id, 'Kapittel-ID er usynkron');
  ...
  assert(equalSet(chapter.emne_ids, domain.emne_ids), 'Kapittelet dekker ikke nøyaktig fagområdets fem emner');
  assert(equalSet(chapter.method_ids, domain.method_ids), 'Kapittelet dekker ikke nøyaktig fagområdets elleve metoder');
  ...
  for (const [moduleIndex, module] of modules.entries()) {
    ...
    for (const section of module.sections || []) {
      ...
      assert(completeTrace(section.paragraphClaimIds), `${section.id}: minst ett avsnitt mangler claim-ID`);
      ...
      if (section.keyPoints) {
        ...
        assert(completeTrace(section.keyPointClaimIds), `${section.id}: minst ett nøkkelpunkt mangler claim-ID`);
  ...
  assert(claims.length >= 40, 'Kapittelet har for få påstandsposter');
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = auditNaeringslivHandelForbrukMarkedChapter({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') });
    console.log(`Handel, forbruk og marked OK: ${report.summary.sectionCount} seksjoner, ${report.summary.claimCount} claims, ${report.summary.sourceCount} kilder.`);
  } catch (error) {
    console.error(`Handel, forbruk og marked FEIL: ${error.message}`);
    process.exitCode = 1;
  }
}
