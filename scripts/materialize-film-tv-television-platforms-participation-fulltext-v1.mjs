...
export function materializeFilmTvTelevisionPlatformsParticipationFulltextV1({ force = false } = {}) {
  ...
  assert([INPUT_GATE, OUTPUT_GATE, DOCUMENTARY_SOURCE_BRIEF_GATE, DOCUMENTARY_FULLTEXT_GATE, REPRESENTATION_SOURCE_BRIEF_GATE, REPRESENTATION_FULLTEXT_GATE].includes(currentGate), `Uventet Film & TV-port: ${currentGate}`);
  if ([OUTPUT_GATE, DOCUMENTARY_SOURCE_BRIEF_GATE, DOCUMENTARY_FULLTEXT_GATE, REPRESENTATION_SOURCE_BRIEF_GATE, REPRESENTATION_FULLTEXT_GATE].includes(currentGate) && !force) {
