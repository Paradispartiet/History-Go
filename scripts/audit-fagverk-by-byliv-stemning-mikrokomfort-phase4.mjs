...
export async function auditByBylivStemningMikrokomfortPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters.length === 4, 'Fjerde By Fase 4-batch skal registrere nøyaktig fire kapitler totalt');
  ...
  assert(model.chapters.length === 4, 'Normalisert By-modell skal vise fire kapitler');
  ...
  const report = {
    ...
    gates: {
      ...
      exactlyFourRegisteredByChapters: true,
