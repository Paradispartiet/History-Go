// Split Historie Innlandet batch 13 place aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/historie/innlandet');
const sourceFile = 'places_historie_innlandet_batch13.json';
const sourceStem = 'places_historie_innlandet_batch13';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/historie/innlandet/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/historie/innlandet/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/historie/innlandet/${sourceFile}`,
