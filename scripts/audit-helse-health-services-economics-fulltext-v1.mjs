...
function build(){
 const source=read(P.source),chapter=read(P.chapter),brief=read(P.brief),claimsDoc=read(P.claims),assessment=read(P.assessment),registry=read(P.registry),status=read(P.status),pensum=read(P.pensum),emners=read(P.emners),release=read(P.release),proof=read(P.proof),reg=registry.subjects.helse,health=status.subjects.find(x=>x.id==='helse');
 ...
 const gates={
   ...
   health_is_12_of_12_and_complete:reg.editorialPlan.registeredChapterCount===12&&reg.editorialPlan.targetDomainCount===12&&reg.chapters.filter(x=>x.id===ID).length===1&&health.editorialStatus==='complete'&&health.nextGate==='complete',
   release_has_twelve_health_chapters:release.subjects.helse.chapter_count===12,
   strict_completion_proven:proof.status==='STRICTLY_PROVEN'&&proof.summary?.fieldsStrictlyProven===12&&reg.editorialPlan.strictCompletionProof?.status==='strictly_proven'
 };
 ...
 const report={schema:'history_go_health_health_services_economics_fulltext_audit_v1',version:'1.0.0',updated_at:DATE,status:'high_quality_fulltext_strictly_complete',subject_id:'helse',chapter_id:ID,summary:{registered_health_chapters:12,target_health_domains:12,module_count:4,section_count:8,paragraph_count:32,verified_claim_count:32,inspectable_source_count:15,assessment_question_count:8,scenario_count:source.decision_scenarios.length},gates,quality_assessment:{correctness_and_evidence:{score:5},coverage_and_completion:{score:5},editorial_and_scientific_quality:{score:5},technical_integrity:{score:4},safety_and_responsibility:{score:5},maintainability_and_auditability:{score:5},total:29,maximum:30,conclusion:'high_quality_fulltext_strictly_complete'},next_gate:'complete'};
