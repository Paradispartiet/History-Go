...
export function auditKunstPhase3({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(statusEntry?.editorialStatus === 'structure_ready', 'Kunst må stå structure_ready før kapittelproduksjon');
  assert(statusEntry?.nextGate === 'chapter_production', 'Kunst har feil neste port');
  ...
  assert(model.chapters.length === 0, 'Structure-ready kan ikke late som Kunst-kapitler finnes');
  ...
  const report = {
    ...
    version: '1.0.0',
    status: 'kunst_phase_3_structure_ready',
    generatedFrom: P,
    subject: {
      id: model.subject.id,
