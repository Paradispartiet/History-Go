...
export function auditPsykologiTraumeKriseResiliensOmsorgPhase4({writeReport=false,checkReport=true}={}){
  ...
  assert(registrySubject.editorialPlan?.targetChapterCount===6&&registrySubject.editorialPlan?.nextGate===NEXT_GATE,'Feil 6/6 universitetsplan');
  ...
  assert(statusEntry?.editorialStatus==='expanded_and_audited'&&statusEntry?.nextGate===NEXT_GATE,'Feil editorial status etter 6/6-baseline');
