...
export function auditPsykologiTraumeKriseResiliensOmsorgPhase4({writeReport=false,checkReport=true}={}){
  ...
  assert(registrySubject.editorialPlan?.targetChapterCount===6&&registrySubject.editorialPlan?.nextGate==='maintenance_source_refresh_and_place_case_expansion','Feil 6/6-plan');
  ...
  assert(statusEntry?.editorialStatus==='complete'&&statusEntry?.nextGate==='maintenance_source_refresh_and_place_case_expansion','Feil editorial status');
