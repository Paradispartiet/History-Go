...
const splitRoot = path.join(root, 'data/places/litteratur/oslo/places_litteratur');
...
// Existing enriched split records are authoritative for these two stale aggregate rows.
const places = read(aggregatePath);
for (const id of ['nasjonalbiblioteket', 'deichman_grunerlokka']) {
  const split = read(path.join(splitRoot, `${id}.json`));
  const index = places.findIndex((place) => place?.id === id);
  if (index < 0) throw new Error(`Missing canonical record ${id}`);
  places[index] = split;
}
...
for (const spec of evidenceSpecs) {
  const evidence = {
    ...
    decision: {
      ...
      blockedReason: '',
      nextAction: 'Verified geometry is applied to canonical place data and has been visually checked before merge.',
    },
...
if (!protocol.includes('| 30 | `oscar_braaten_statuen`')) {
  if (!protocol.includes(anchor)) throw new Error('Batch 29 protocol anchor missing');
  protocol = protocol.replace(
    anchor,
...
if (!protocol.includes('150 verifiserte eller kildekontrollerte canonical steder')) {
  throw new Error('Protocol count was not updated to 150');
}
