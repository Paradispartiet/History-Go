...
const P = Object.freeze({
  chapter: 'data/fagverk/politikk/valg-partier-velgeratferd.json',
  brief: 'data/fagverk/politikk/valg-partier-velgeratferd/brief.json',
  claims: 'data/fagverk/politikk/valg-partier-velgeratferd/claims.json',
  module1: 'data/fagverk/politikk/valg-partier-velgeratferd/01-grunnlag.json',
  module2: 'data/fagverk/politikk/valg-partier-velgeratferd/02-fordypning.json',
  module3: 'data/fagverk/politikk/valg-partier-velgeratferd/03-anvendelse.json',
  ...
  report: 'reports/fagverk/politikk-valg-partier-velgeratferd-audit.json'
});
...
export function auditPolitikkValgChapter({ writeReport = false, checkReport = true } = {}) {
  ...
  const domain = (pensum.domains || []).find((row) => row.domain_id === 'valg_partier_velgeratferd');
  assert(domain, 'Pensum mangler fagområdet valg, partier og velgeratferd');
  ...
  assert(chapter.id === 'valg-partier-velgeratferd' && chapter.chapter_id === chapter.id, 'Kapittel-ID er usynkron');
  ...
  for (const [moduleIndex, module] of modules.entries()) {
    assert((module.sections || []).length >= 3, `Modul ${moduleIndex + 1} har for få redigerte seksjoner`);
  ...
  assert(sources.length >= 10, 'Kapittelet har for få inspectable kilder');
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = auditPolitikkValgChapter({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') });
