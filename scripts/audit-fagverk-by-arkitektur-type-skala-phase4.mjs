...
export async function auditByArkitekturTypeSkalaPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters.length === 6, 'Første Arkitektur-batch skal gi nøyaktig seks registrerte By-kapitler totalt');
  ...
  assert(model.chapters.length === 6, 'Normalisert By-modell skal vise seks kapitler');
  ...
  const report = {
    ...
    gates: {
      ...
      exactlySixRegisteredByChapters: true,
