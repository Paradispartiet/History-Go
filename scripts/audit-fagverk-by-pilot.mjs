...
export function auditByPilot({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(model.chapters.length === 10 && model.chapters.some((chapter) => chapter.id === 'byliv-offentlige-rom') && model.chapters.some((chapter) => chapter.id === 'byliv-sosial-offentlighet') && model.chapters.some((chapter) => chapter.id === 'byliv-hendelser-midlertidighet') && model.chapters.some((chapter) => chapter.id === 'byliv-stemning-mikrokomfort') && model.chapters.some((chapter) => chapter.id === 'byliv-rytmer-miks-konflikt') && model.chapters.some((chapter) => chapter.id === 'arkitektur-type-skala-byform') && model.chapters.some((chapter) => chapter.id === 'arkitektur-gatekant-makt-ombruk') && model.chapters.some((chapter) => chapter.id === 'bolig-nabolag-tilgang-endring'), 'By skal registrere fem Byliv-kapitler og første Arkitektur-kapittel i Fase 4');
