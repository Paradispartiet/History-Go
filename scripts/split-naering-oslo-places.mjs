// Split Oslo naering places aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/naeringsliv/oslo');
const sourceFile = 'places_naeringsliv.json';
const sourceStem = 'places_naeringsliv';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/naeringsliv/oslo/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/naeringsliv/oslo/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/naeringsliv/oslo/${sourceFile}`,
