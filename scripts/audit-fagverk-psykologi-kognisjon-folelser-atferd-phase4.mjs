...
export function auditPsykologiKognisjonFolelserAtferdPhase4({writeReport=false,checkReport=true}={}) {
  ...
  const registrySubject=registry.subjects?.psykologi; assert(registrySubject?.chapters?.length===4,'Psykologi skal ha 4/6 registrerte kapitler');
  ...
  assert(registrySubject.editorialPlan?.targetChapterCount===6&&registrySubject.editorialPlan?.nextGate==='remaining_domain_chapter_production','Feil 4/6-plan');
