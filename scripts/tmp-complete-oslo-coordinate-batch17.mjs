...
const sources = {
  by: {
    ...
    indexRel: 'data/places/by/oslo/places_by_index.json'
  },
  film: {
...
function parseFinderOutput(file, label) {
...
const finderDefs = {
  saga_kino: {
    ...
    identitySource: 'Nordisk Film Kino – Saga', identityUrl: 'https://www.nfkino.no/kino/saga',
  ...
  klingenberg_kino: {
    ...
    identitySource: 'Nordisk Film Kino – Klingenberg', identityUrl: 'https://www.nfkino.no/kino/klingenberg',
  ...
  gimle_kino: {
    ...
    identitySource: 'Nordisk Film Kino – Gimle', identityUrl: 'https://www.nfkino.no/kino/gimle',
  ...
  vika_kino: {
    ...
    identitySource: 'Nordisk Film Kino – Vika', identityUrl: 'https://www.nfkino.no/kino/vika',
  ...
  hartvig_nissens_skole_skam: {
    label: 'Hartvig Nissens skole (SKAM)', address: "President Harbitz' gate 11 Oslo",
    identitySource: 'VisitOSLO SKAM-kart + Oslo byleksikon', identityUrl: 'https://www.visitoslo.com/no/artikler/skam/',
    resolvedIdentity: "det historiske Hartvig Nissen-bygget i President Harbitz' gate 11, tidligere Niels Juels gate 56, brukt som SKAM-skole",
    note: "Offisiell adressekoordinat fra Geonorge Adresser API for President Harbitz' gate 11, det historiske Hartvig Nissen-bygget. VisitOSLO dokumenterer SKAM-skolen under den tidligere adressen Niels Juels gate 56, mens Oslo byleksikon dokumenterer at samme bygning nå har adressen President Harbitz' gate 11. Punktet representerer den konkrete innspillingslokasjonen, ikke hele dagens skolecampus."
  }
};

const finderResults = {};
for (const [id, def] of Object.entries(finderDefs)) {
  finderResults[id] = parseFinderOutput(path.join(REPORT_DIR, `${id}-geonorge.json`), def.label);
}
...
const evidenceDefs = {
  grorud: { file: 'oslo/by/grorud.json', placeFile: sources.by.aggregateRel, currentName: 'Grorud', resolvedIdentity: 'Grorud som strøk i øvre Groruddalen', finding: 'Oslo byleksikon identifiserer Grorud som eget bolig-, industri- og knutepunktstrøk, adskilt fra den større administrative Bydel Grorud.' },
  sagene: { file: 'oslo/by/sagene.json', placeFile: sources.by.aggregateRel, currentName: 'Sagene', resolvedIdentity: 'Sagene som historisk industri- og boligstrøk langs Akerselva', finding: 'Oslo byleksikon identifiserer Sagene som eget strøk og beskriver utstrekningen, adskilt fra den større administrative Bydel Sagene.' }
};

for (const [id, def] of Object.entries(finderDefs)) {
  evidenceDefs[id] = {
    file: `oslo/film/${id}.json`, placeFile: sources.film.aggregateRel, currentName: def.label,
    resolvedIdentity: def.resolvedIdentity,
    finding: `${def.identitySource} dokumenterer identiteten på adressen ${def.address}; Geonorge gir ett anvendbart offisielt adressepunkt.`
  };
}
...
for (const [id, d] of Object.entries(evidenceDefs)) {
  ...
  writeJson(path.join(EVIDENCE_ROOT, d.file), {
    ...
    evidence: [{ sourceProvider: place.sourceProvider, sourceName: place.coordSource, sourceUrl: place.coordSourceUrl, sourceObjectId: place.sourceObjectId, sourceQuality: finderDef ? 'official_address_plus_official_identity' : 'stable_area_definition', finding: d.finding, canVerifyCoordinate: true, reason: place.coordNote }],
    ...
    notes: [place.coordNote, ...(finderDef ? [`Identitetskilde: ${finderDef.identitySource} – ${finderDef.identityUrl}`] : [])]
  });
}

const evidenceManifest = readJson(EVIDENCE_MANIFEST);
for (const d of Object.values(evidenceDefs)) if (!evidenceManifest.files.includes(d.file)) evidenceManifest.files.push(d.file);
...
protocol = replaceRequired(
  ...
  'Oslo-tabellen inneholder nå 104 verifiserte eller kildekontrollerte canonical steder. Batch 17 legger til sju godkjente kontroller: områdeankrene Grorud og Sagene samt fem adresseverifiserte film- og kinolokasjoner. Fem fullførte Oslo-kontroller står fortsatt separat uten godkjent koordinat. Senere visuell kontroll korrigerte ankrene for Oslo domkirke, Tronsmo Bokhandel, Grønland basarene, Møllergata 19 og Villa Grande uten at de ble telt på nytt.',
  'Oslo summary'
);
...
const rows = [
  ...
  `| 17 | \`hartvig_nissens_skole_skam\` | Hartvig Nissens skole (SKAM) | verified | \`${finderResults.hartvig_nissens_skole_skam.sourceObjectId}\` |`
].join('\n');
protocol = replaceRequired(protocol, last, last + '\n' + rows, 'batch 17 rows');
protocol = protocol.replace('Disse kontrollene er fullført, men teller ikke blant de 97 verifiserte eller kildekontrollerte canonical stedene.', 'Disse kontrollene er fullført, men teller ikke blant de 104 verifiserte eller kildekontrollerte canonical stedene.');
protocol = replaceRequired(
  protocol,
  ...
  '- Neste nye Oslo-kontroll er nummer 108 og starter batch 18.\n- Batch 17 er fullført med to godkjente strøksankre og fem adresseverifiserte film- og kinolokasjoner.\n- Fortsett i canonical filrekkefølge når det gir en naturlig arbeidskø, men velg alltid koordinatmetode etter fysisk objekttype.\n- Før alle fullførte `needs_review`-kontroller i den separate Oslo-tabellen samme dag som avgjørelsen tas.',
  'next work'
);
fs.writeFileSync(PROTOCOL, protocol);

fs.mkdirSync(REPORT_DIR, { recursive: true });
fs.writeFileSync(REPORT, `# Oslo koordinatkontroll – batch 17\n\nDato: ${DATE}\n\nSju nye canonical Oslo-steder er kontrollert og godkjent. Grorud og Sagene er eksplisitte strøks-/områdeankre. De fem filmstedene er konkrete bygg/lokasjoner og er derfor kjørt gjennom den normative Geonorge-adresseflyten med lagret terminaloutput.\n\n| placeId | status | kildeobjekt |\n|---|---|---|\n| \`grorud\` | verified_geometry | \`oslobyleksikon:grorud-strok\` |\n| \`sagene\` | verified_geometry | \`oslobyleksikon:sagene-strok\` |\n| \`saga_kino\` | verified | \`${finderResults.saga_kino.sourceObjectId}\` |\n| \`klingenberg_kino\` | verified | \`${finderResults.klingenberg_kino.sourceObjectId}\` |\n| \`gimle_kino\` | verified | \`${finderResults.gimle_kino.sourceObjectId}\` |\n| \`vika_kino\` | verified | \`${finderResults.vika_kino.sourceObjectId}\` |\n| \`hartvig_nissens_skole_skam\` | verified | \`${finderResults.hartvig_nissens_skole_skam.sourceObjectId}\` |\n\n## Identitets- og overlap-audit\n\n- Grorud og Sagene behandles som strøk, ikke som administrative bydeler eller enkeltstasjoner.\n- Saga, Klingenberg, Gimle og Vika behandles som konkrete kinobygg på dokumenterte besøksadresser.\n- Hartvig Nissen-recorden forankres i det historiske skolebygget som brukes som SKAM-lokasjon. VisitOSLO oppgir den tidligere adressen Niels Juels gate 56; dagens gateadresse for samme historiske bygg er President Harbitz' gate 11.\n- Ingen av de fem filmstedene gjenbruker et nærliggende canonical plass-, teater- eller stasjonsanker.\n`);

console.log('Completed Oslo coordinate batch 17');
