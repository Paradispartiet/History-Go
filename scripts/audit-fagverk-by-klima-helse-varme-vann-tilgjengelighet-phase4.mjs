...
export async function auditByKlimaHelseVarmeVannTilgjengelighetPhase4({writeReport=false,checkReport=true}={}){
 ...
 assert(statusEntry?.assessmentStatus==='audited'&&statusEntry?.editorialStatus==='chapters_in_progress'&&statusEntry?.nextGate==='chapter_production','By skal fortsette chapter_production');
 assert(rs&&Array.isArray(rs.chapters)&&rs.chapters.length===14,'Klima og helse-batch skal gi fjorten By-kapitler');
 ...
 assert(model.chapters.length===14,'Normalisert By skal ha fjorten kapitler');
