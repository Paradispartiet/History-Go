// Split Lisbon politics aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/politikk/europe/portugal/lisbon');
const sourceFile = 'places_lisbon_politikk.json';
const sourceStem = 'places_lisbon_politikk';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/politikk/europe/portugal/lisbon/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/politikk/europe/portugal/lisbon/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/politikk/europe/portugal/lisbon/${sourceFile}`,
