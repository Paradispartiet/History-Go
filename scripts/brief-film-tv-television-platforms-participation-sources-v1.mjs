...
export function auditFilmTvTelevisionPlatformsParticipationSourceBriefV1({ writeFiles = false, checkFiles = true } = {}) {
  ...
  assert([INPUT_GATE, SOURCE_BRIEF_GATE, FULLTEXT_GATE, DOCUMENTARY_SOURCE_BRIEF_GATE, DOCUMENTARY_FULLTEXT_GATE, REPRESENTATION_SOURCE_BRIEF_GATE, REPRESENTATION_FULLTEXT_GATE].includes(currentGate), `Uventet Film & TV-port: ${currentGate}`);
  if ([FULLTEXT_GATE, DOCUMENTARY_SOURCE_BRIEF_GATE, DOCUMENTARY_FULLTEXT_GATE, REPRESENTATION_SOURCE_BRIEF_GATE, REPRESENTATION_FULLTEXT_GATE].includes(currentGate)) {
