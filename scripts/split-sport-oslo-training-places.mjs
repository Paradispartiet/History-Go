// Split Oslo playground and training aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/sport/europa/norway');
const sourceFile = 'places_oslo_lekeplasser_trening.json';
const sourceStem = 'places_oslo_lekeplasser_trening';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/sport/europa/norway/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/sport/europa/norway/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/sport/europa/norway/${sourceFile}`,
