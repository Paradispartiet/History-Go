...
export function auditFilmTvKinoerVisningsstederPublikumPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters.length === 1 && registryChapter, 'Film & TV-registeret skal ha nøyaktig ett kapittel');
  ...
  assert(phase3.report.summary.registeredChapterCount === 1, 'Fase 3-auditen ser ikke første Film & TV-kapittel');
  ...
  const report = {
    ...
    canonicalCoverage: {
      ...
      coveredEmneIds: chapter.emne_ids, exactCoverage: '20/20', remainingDomainCount: 5
    },
    summary: {
