...
export function auditFilmTvHistoryMovementsHistoriographySourceBriefV1({ writeFiles = false, checkFiles = true } = {}) {
  ...
  assert([INPUT_GATE, SOURCE_BRIEF_GATE, FULLTEXT_GATE, LATER_SOURCE_BRIEF_GATE, TELEVISION_FULLTEXT_GATE, DOCUMENTARY_SOURCE_BRIEF_GATE, DOCUMENTARY_FULLTEXT_GATE].includes(currentGate), `Uventet Film & TV-port: ${currentGate}`);
  if ([FULLTEXT_GATE, LATER_SOURCE_BRIEF_GATE, TELEVISION_FULLTEXT_GATE, DOCUMENTARY_SOURCE_BRIEF_GATE, DOCUMENTARY_FULLTEXT_GATE].includes(currentGate)) {
