...
BRANCH='agent/scenekunst-oslo-ci-frozen-runner'
BASE_BRANCH='agent/scenekunst-oslo-ci-base-20260723'
...
printf '%s\n' '== Prepare Oslo Scenekunst resync on frozen base =='
git config user.name 'github-actions[bot]'
git config user.email '41898282+github-actions[bot]@users.noreply.github.com'
git fetch origin "${BASE_BRANCH}" "${BRANCH}" "${SOURCE}"
git show "origin/${BASE_BRANCH}:scripts/check-places.sh" > /tmp/check-places-original.sh
...
git checkout "origin/${BASE_BRANCH}" -- scripts/check-places.sh
...
git commit -m 'Build five Oslo Scenekunst split venues on frozen base'
git push origin HEAD:"${BRANCH}"

echo 'Oslo Scenekunst production result published to frozen feature branch.'
