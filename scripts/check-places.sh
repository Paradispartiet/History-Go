...
BRANCH='agent/scenekunst-oslo-split-venues-01-data-only'

printf '%s\n' '== Synchronize Oslo Scenekunst data branch with current main =='
...
printf '%s\n' '== Restore canonical runner and publish clean index commit =='
...
git commit -m 'Finalize Oslo Scenekunst global place index'
git push origin HEAD:"${BRANCH}"

echo 'Clean Oslo Scenekunst index commit published.'
