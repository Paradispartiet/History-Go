...
const P = {
  ...
  report: 'reports/fagverk/utdanning-history-of-education-fulltext-v1-audit.json',
};
...
export function audit({ writeReport = false } = {}) {
  ...
  assert(!brief.safety.linearProgressMyth && !brief.safety.lawAsImplementationProof && !brief.safety.majorityOnlyNarrative && !brief.safety.reformNameAsCause && !brief.safety.archiveSilenceAsFreeInference, 'Historiefaglige og etiske grenser mangler');
  ...
  assert(registryRow?.chapters?.filter((row) => row.id === CHAPTER_ID).length === 1 && registryRow.editorialPlan.registeredChapterCount === 10 && registryRow.editorialPlan.completedSourceBriefCount === 11, 'Registry må være 10/14 med elleve source briefs');
  ...
  assert(pensum.domains.filter((domain) => domain.status === 'materialized').length === 10 && pensum.domains[9].domain_id === 'utdanningshistorie' && !pensum.complete_ready, 'Pensum skal være monotont 10/14');
  assert(nextBrief.scope.primary_domain_id === 'utdanningspolitikk' && !nextBrief.runtime_registration.registered, 'Utdanningspolitikk må være neste uregistrerte source-first-domene');

  const report = {
    schema: 'history_go_utdanning_history_of_education_fulltext_audit_v1',
    version: '1.0.0',
    ...
    gates: {
      ...
      normativeTextVsPracticeBoundary: true,
      periodizationNonCausalityBoundary: true,
      accessParticipationBoundary: true,
      localCapacityBoundary: true,
      minorityHistoryBoundary: true,
      professionalMemoryBoundary: true,
      rightCompletionBoundary: true,
      reformImplementationImpactBoundary: true,
      archivePowerBoundary: true,
      noLinearProgressBoundary: true,
      chapterRegisteredExactlyOnce: true,
      educationMaterializedAndAudited: true,
      prematureSubjectCompletionBlocked: true,
      educationPolicySourceFirstNext: true,
    },
    six_part_quality_review: {
      source_authority_and_provenance: 5,
      claim_trace_and_verifiability: 5,
      history_of_education_theory_and_boundary_quality: 5,
      historical_method_power_and_representation: 5,
      pedagogy_and_assessment: 4,
      architecture_and_reproducibility: 5,
      total: 29,
      maximum: 30,
      note: 'Utdanningshistorie-pakken er intern; 4 domener og strict subject-proof gjenstår før Utdanning kan bli complete.',
    },
  };
...
try {
  ...
  console.log(`Utdanning Utdanningshistorie fulltekstaudit OK: ${report.counts.paragraphs} avsnitt, ${report.counts.verifiedClaims} claims, ${report.counts.inspectableSources} kilder; ${report.six_part_quality_review.total}/30.`);
} catch (error) {
  console.error(`Utdanning Utdanningshistorie fulltekstaudit FEIL: ${error.message}`);
  process.exitCode = 1;
}
