...
const CHAPTER_ID = 'profesjoner-laererrollen-kunnskap-skjonn-etikk-og-samarbeid';
...
const P = {
  ...
  sourceBrief: 'data/fag/utdanning/professions_teacher_role_source_claim_brief_v1.json', nextBrief: 'data/fag/utdanning/technology_media_learning_source_claim_brief_v1.json',
  ...
  report: 'reports/fagverk/utdanning-professions-teacher-role-fulltext-v1-audit.json',
};
...
export function audit({ writeReport = false } = {}) {
  ...
  assert(!brief.safety.qualificationAsCompleteCompetence && !brief.safety.autonomyAsNoAccountability && !brief.safety.judgmentAsPreference && !brief.safety.agencyAsPersonality && !brief.safety.relationshipAsBoundarylessness && !brief.safety.collaborationAsEffectProof && !brief.safety.selfReportAsCausalProof && !brief.safety.ethicalFrameworkAsEffectProof, 'Profesjonsfaglige grenser mangler');
  ...
  assert(registryRow?.chapters?.filter((row) => row.id === CHAPTER_ID).length === 1 && registryRow.editorialPlan.registeredChapterCount === 13 && registryRow.editorialPlan.completedSourceBriefCount === 14, 'Registry må være 13/14 med fjorten source briefs');
  ...
  assert(pensum.domains.filter((domain) => domain.status === 'materialized').length === 13 && pensum.domains[12].domain_id === 'profesjoner_laererrollen' && !pensum.complete_ready, 'Pensum skal være monotont 13/14');
  assert(nextBrief.scope.primary_domain_id === 'teknologi_medier_laering' && !nextBrief.runtime_registration.registered, 'Teknologi, medier og læring må være neste uregistrerte source-first-domene');
  const report = {
    schema: 'history_go_utdanning_professions_teacher_role_fulltext_audit_v1', version: '1.0.0', updated_at: '2026-08-27', status: 'pass', conclusion: 'high_quality_fulltext_unit_complete_subject_not_scientifically_complete', subject_id: 'utdanning', chapter_id: CHAPTER_ID,
    counts: { domainsCovered: 13, targetDomains: 14, modules: 4, sections: 8, paragraphs: 32, verifiedClaims: 32, inspectableSources: 13, assessmentQuestions: 8, decisionScenarios: sourceBrief.decision_scenarios.length, nextSourceBriefDomains: 1 },
    gates: { plannedClaimsResolvedOneToOne: true, paragraphClaimTraceReciprocalAndComplete: true, paragraphsEditoriallyUnique: true, everyUsedSourceInspectable: true, everySourceSupportsFinalClaim: true, qualificationCompetenceBoundary: true, autonomyAccountabilityBoundary: true, judgmentPreferenceBoundary: true, ecologicalAgencyBoundary: true, relationshipBoundary: true, collectiveProfessionalismEvidenceBoundary: true, selfReportBoundary: true, inductionResourceBoundary: true, ethicsEvidenceBoundary: true, chapterRegisteredExactlyOnce: true, educationMaterializedAndAudited: true, prematureSubjectCompletionBlocked: true, technologyMediaLearningSourceFirstNext: true },
    six_part_quality_review: { source_authority_and_provenance: 5, claim_trace_and_verifiability: 5, profession_knowledge_judgment_agency_quality: 5, rights_relationships_ethics_work_conditions: 5, pedagogy_and_assessment: 4, architecture_and_reproducibility: 5, total: 29, maximum: 30, note: 'Profesjoner og lærerrollen-pakken er intern; ett domene og strict subject-proof gjenstår før Utdanning kan bli complete.' },
  };
...
try {
  ...
  console.log(`Utdanning Profesjoner og lærerrollen fulltekstaudit OK: ${report.counts.paragraphs} avsnitt, ${report.counts.verifiedClaims} claims, ${report.counts.inspectableSources} kilder; ${report.six_part_quality_review.total}/30.`);
} catch (error) { console.error(`Utdanning Profesjoner og lærerrollen fulltekstaudit FEIL: ${error.message}`); process.exitCode = 1; }
