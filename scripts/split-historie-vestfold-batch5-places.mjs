// Split Historie Vestfold batch 5 place aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/historie/vestfold');
const sourceFile = 'places_historie_vestfold_batch5.json';
const sourceStem = 'places_historie_vestfold_batch5';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/historie/vestfold/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/historie/vestfold/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/historie/vestfold/${sourceFile}`,
