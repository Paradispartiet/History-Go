// Split Kunst Oslo places aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/kunst/oslo');
const sourceFile = 'places_kunst.json';
const sourceStem = 'places_kunst';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/kunst/oslo/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/kunst/oslo/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/kunst/oslo/${sourceFile}`,
