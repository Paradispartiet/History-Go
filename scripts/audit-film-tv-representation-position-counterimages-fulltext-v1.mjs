...
const P = Object.freeze({
  chapter: `data/fagverk/film_tv/${CHAPTER_ID}.json`, brief: `data/fagverk/film_tv/${CHAPTER_ID}/brief.json`,
  ...
  learningPlan: 'data/fag/TV_og_Film/film_tv_learning_order_plan_v1.json', emners: 'data/fag/TV_og_Film/emner_film_tv_canonical_v4_5.json',
  methods: 'data/fag/TV_og_Film/methods_film_tv_canonical_v4_5.json', places: 'data/places/places_index.json',
...
export function auditFilmTvRepresentationPositionCounterimagesFulltextV1({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(chapter.id === CHAPTER_ID && chapter.primary_domain_id === 'samfunn_representasjon_identitet_makt', 'Kapittelet har feil ID eller eierdomene');
  ...
  assert(chapter.relatedPlaces.length === 2 && chapter.workCases.length === 21, 'Kapittelet skal ha 21 case og to canonicale anvendelsessteder');
  ...
  assert(isDeepStrictEqual(new Set(sections.map((row) => row.paragraphs.length)), new Set([3, 4])), 'Avsnittstallet følger ikke claimplanenes variable problemomfang');
  ...
  assert(claimsDoc.claims.length === 38 && claimIds.size === 38 && isDeepStrictEqual(claimIds, plannedClaimIds), 'De endelige claims løser ikke briefens 38 claimplaner eksakt');
  assert(claimsDoc.claims.every((row) => row.claim_plan_id === row.id && row.status === 'verified' && row.plan_resolution === 'verified_as_planned'), 'En claim mangler verifisert planoppløsning');
  ...
  assert(paragraphClaimIds.length === 38 && new Set(paragraphClaimIds).size === 38 && paragraphClaimIds.every((id) => claimIds.has(id)), 'Avsnittssporene dekker ikke alle claims');
  ...
  const combined = JSON.stringify({ chapter, brief, modules: modules.map((row) => row.value), claims: claimsDoc.claims });
  assert(/synlighet.{0,300}perspektiv.{0,240}agens.{0,300}(form|opphav|distribusjon)/is.test(combined), 'Representasjonsmaktens seks felt mangler');
  assert(/(utseende|ansikt).{0,180}(navn|nasjonal opprinnelse).{0,220}(ikke|aldri|ingen)/is.test(combined) || /(ikke|aldri|ingen).{0,220}(utseende|ansikt).{0,180}(navn|nasjonal opprinnelse)/is.test(combined), 'Vakten mot visuell identitetsinferens mangler');
  assert(/selvidentifikasjon.{0,220}historisk kategori.{0,220}rollefunksjon.{0,220}analytisk rasialisering/is.test(combined), 'Identitetsstatusvakten mangler');
  assert(/interseksjonalitet.{0,300}(strukturer|maktstrukturer).{0,300}(dokumenter|materiale|kilde)/is.test(combined), 'Interseksjonalitetsvakten mangler');
  assert(/korpus.{0,240}tidsrom.{0,240}(enhet|analyseenhet).{0,240}(ukjentdata|nevner)/is.test(combined), 'Kvantitativ synlighetsvakt mangler');
  assert(/casting.{0,220}arbeidsadgang.{0,220}narrativ kompleksitet.{0,220}(distribusjon|tilgjengelig)/is.test(combined), 'Funksjonsvariasjonsgrensen mangler');
  assert(/OFELAŠ.{0,500}(fortellereierskap|story ownership).{0,420}(konsultasjon|medbestemmelse)/is.test(combined), 'OFELAŠ-protokollen mangler');
  assert(/(samisk|Sámi).{0,420}(fortelling|bilde).{0,420}(språk|kilder).{0,520}(produksjon|finansiering|sirkulasjon)/is.test(combined), 'Samisk skjermsuverenitetsaudit mangler');
  assert(/nasjon.{0,360}(offentlighet|migrasjon).{0,500}(utenfor|excluded|hovedanalyse)/is.test(combined), 'Grensen mot neste enhet mangler');
  assert(/resepsjon.{0,320}(uten|egne|kilder)/is.test(combined), 'Resepsjonsgrensen mangler');

  const paragraphs = sections.flatMap((row) => row.paragraphs);
  const editorialWordCount = paragraphs.join(' ').split(/\s+/u).filter(Boolean).length;
  assert(editorialWordCount >= 1800, 'Fullteksten er for tynn for ti emner');
  const normalizedOpenings = paragraphs.map((row) => row.toLowerCase().replace(/[^a-zæøå0-9 ]/giu, '').split(/\s+/u).slice(0, 8).join(' '));
  assert(new Set(normalizedOpenings).size === paragraphs.length, 'Gjenbrukt generisk avsnittsmal er oppdaget');

  const report = {
    schema: 'history_go_film_tv_representation_position_counterimages_fulltext_v1_audit', version: '1.0.0', status: 'representation_position_counterimages_chapter_verified_registered', generatedFrom: P,
    ...
    gates: {
      ...
      noVisualIdentityInference: true, visibilityPerspectiveAgencyFormAuthorshipDistributionSeparated: true, quantitativeVisibilityBounded: true,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try { const { report } = auditFilmTvRepresentationPositionCounterimagesFulltextV1({ writeReport: args.has('--write-report'), checkReport: !args.has('--write-report') && !args.has('--no-check-report') }); console.log(`Film & TV Representasjon OK: ${report.canonicalCoverage.exactCoverage}, ${report.summary.sectionCount} seksjoner, ${report.summary.claimCount} claims og ${report.summary.sourceCount} kilder.`); }
  catch (error) { console.error(`Film & TV Representasjon FEIL: ${error.message}`); process.exitCode = 1; }
}
