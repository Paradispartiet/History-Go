...
const P = Object.freeze({
  chapter: 'data/fagverk/naeringsliv/arbeid-produksjon-verdiskaping.json',
  brief: 'data/fagverk/naeringsliv/arbeid-produksjon-verdiskaping/brief.json',
  claims: 'data/fagverk/naeringsliv/arbeid-produksjon-verdiskaping/claims.json',
  module1: 'data/fagverk/naeringsliv/arbeid-produksjon-verdiskaping/01-grunnlag.json',
  module2: 'data/fagverk/naeringsliv/arbeid-produksjon-verdiskaping/02-fordypning.json',
  module3: 'data/fagverk/naeringsliv/arbeid-produksjon-verdiskaping/03-anvendelse.json',
  ...
  report: 'reports/fagverk/naeringsliv-arbeid-produksjon-verdiskaping-audit.json'
});
...
export function auditNaeringslivArbeidProduksjonVerdiskapingChapter({ writeReport = false, checkReport = true } = {}) {
  ...
  const domain = (pensum.domains || []).find((row) => row.domain_id === 'arbeid_produksjon_verdiskaping');

  assert(domain, 'Pensum mangler fagområdet arbeid, produksjon og verdiskaping');
  ...
  assert(chapter.id === 'arbeid-produksjon-verdiskaping' && chapter.chapter_id === chapter.id, 'Kapittel-ID er usynkron');
  ...
  assert(equalSet(chapter.method_ids || [], domain.method_ids || []), 'Kapittelet dekker ikke nøyaktig fagområdets nitten metoder');
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = auditNaeringslivArbeidProduksjonVerdiskapingChapter({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') });
    console.log(`Næringsliv-kapittel OK: ${report.summary.sectionCount} seksjoner, ${report.summary.claimCount} claims, ${report.summary.sourceCount} kilder.`);
  } catch (error) {
    console.error(`Næringsliv-kapittel FEIL: ${error.message}`);
    process.exitCode = 1;
  }
}
