...
export function auditFilmTvVariableInventoryV1({ write = false, check = true } = {}) {
  ...
  assert(['canonical_inventory_migration', 'canonical_inventory_migrated_existing_chapter_reaudit', 'canonical_chapter_reaudit_complete_learning_order_plan', 'learning_order_plan_complete_first_chapter_source_brief', 'audiovisual_form_source_brief_complete_full_chapter_production'].includes(status?.nextGate), 'Film & TV skal stå på canonical migrasjon, kapittelreaudit, læringsrekkefølge, kildebrief eller fulltekstproduksjon etter gapdesign');
