...
export function audit() {
  ...
  assert(chapter.subject_id === 'politikk' && chapter.canonical_subcategory_id === 'sosiologi_antropologi' && chapter.domain_id === 'institusjoner_organisasjoner_arbeid_velferd', 'Kapittelet har feil canonicalt eierskap');
  ...
  assert(claimFile.sources.every((source) => source.url.startsWith('https://') && source.source_location && source.retrieval_status === 'verified_2026-08-29'), 'Alle kilder må være inspiserbare og ferskt verifisert');
  ...
  assert(assessment.caseTasks.length === 6 && brief.realDisagreements.length >= 3 && brief.methodsAndLimits.length >= 5, 'Scenarier, metodegrenser eller reell uenighet mangler');
  assert(Object.values(brief.safety).every((value) => value === false), 'Ansvarlighetsgrenser mangler');
  assert(production.progress.materializedDomains === 7 && production.progress.totalDomains === 12 && production.progress.strictCompletionProven === false, 'Produksjonsregister skal vise nøyaktig 7/12 og strict=false');
  assert(production.materialized.length === 7 && production.materialized.filter((row) => row.ordinal === 7 && row.chapter === P.chapter).length === 1, 'Felt 7 skal være registrert nøyaktig én gang');
  const subcategory = category.canonicalSubcategories.politikk.find((row) => row.id === 'sosiologi_antropologi');
  assert(subcategory?.status === 'expansion_planned', 'Underkategorien må forbli expansion_planned ved 7/12');
...
try {
  const report = audit();
  console.log(`Institusjoner, organisasjoner, arbeid og velferd fulltekstaudit OK: ${report.counts.paragraphs} avsnitt, ${report.counts.verifiedClaims} claims, ${report.six_part_quality_review.total}/30.`);
} catch (error) {
