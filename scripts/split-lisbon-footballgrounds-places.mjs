// Split Lisbon football grounds aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/sport/europa/portugal');
const sourceFile = 'footballgrounds_lisbon.json';
const sourceStem = 'footballgrounds_lisbon';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/sport/europa/portugal/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/sport/europa/portugal/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/sport/europa/portugal/${sourceFile}`,
