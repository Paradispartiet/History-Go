...
export function auditReligionTheoryCanon(){
 ...
 for(const file of registry.source_documents){const d=json(file);const areaSources=new Map((d.sources||[]).map(s=>[s.id,s]));sourceByArea.set(d.area_id,areaSources);for(const c of d.claims||[]){if(!claimsByTopic.has(c.topic_id))claimsByTopic.set(c.topic_id,[]);claimsByTopic.get(c.topic_id).push(c);}}
