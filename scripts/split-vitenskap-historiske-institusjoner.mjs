// Split Oslo vitenskap historical institutions aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/vitenskap/oslo');
const sourceFile = 'places_vitenskap_historiske_institusjoner.json';
const sourceStem = 'places_vitenskap_historiske_institusjoner';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/vitenskap/oslo/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/vitenskap/oslo/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/vitenskap/oslo/${sourceFile}`,
