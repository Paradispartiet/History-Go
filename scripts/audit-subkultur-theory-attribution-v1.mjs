...
export function auditSubkulturTheoryAttribution(){
 ...
 for(const l of linksDoc.links){assert(l.theory_id&&l.claim_id&&l.source_id,`Ufullstendig evidence-link ${l.evidence_link_id}`);assert(claimById.has(l.claim_id),`Evidence-link peker på ukjent claim ${l.claim_id}`);assert(sourceById.has(l.source_id),`Evidence-link peker på ukjent source ${l.source_id}`);const k=`${l.theory_id}::${l.source_id}`;if(!linksByTheorySource.has(k))linksByTheorySource.set(k,[]);linksByTheorySource.get(k).push(l);}
 ...
 const covered=new Set(),unattributed=[];let scholarlyCoreTheoryCount=0,proseBoundTheoryCount=0,claimBoundSourceCount=0;
 ...
 return {status:'strong_theory_attribution',theoryCount:theories.length,coveredEmneCount:covered.size,domainCount:domains.size,uniquePeopleCount:people.size,uniqueWorkCount:works.size,attributedSourceCount:attrBySource.size,scholarlyCoreTheoryCount,proseBoundTheoryCount,claimBoundSourceCount};
}
