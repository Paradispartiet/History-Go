...
export function auditSubkulturTheoryAttribution(){
 ...
 for(const t of theories){
  ...
  assert(t.claim_ids?.length>=2,`Theory mangler claim-binding: ${t.theory_id}`);const claims=t.claim_ids.map(id=>{const c=claimById.get(id);assert(c,`Ukjent theory claim ${t.theory_id}/${id}`);assert(c.theory_id===t.theory_id,`Claim/theory mismatch ${id}`);assert(c.domain_id===t.domain_id&&c.emne_ids?.length===1&&c.emne_ids[0]===emneId,`Claim scope mismatch ${id}`);assert(nonTrivial(c.statement)&&c.source_ids?.length>=1,`Tynn theory claim ${id}`);let scholarlyClaimSources=0;for(const sid of c.source_ids){assert(t.source_ids.includes(sid),`Claim source ligger utenfor theory source-sett: ${t.theory_id}/${id}/${sid}`);const s=sourceById.get(sid);assert(s&&(isScholarly(s)||isGuidanceSupplement(s)),`Theory-claim bruker uklassifisert source: ${t.theory_id}/${id}/${sid}/${s?.source_type}`);if(isScholarly(s))scholarlyClaimSources++;const matches=linksByTheoryClaimSource.get(`${t.theory_id}::${id}::${sid}`)||[];assert(matches.some(l=>['conceptual_support','critical_boundary'].includes(l.support_type)&&nonTrivial(l.inference_boundary)),`Claim source mangler eksakt evidence-link: ${t.theory_id}/${id}/${sid}`);claimBoundSourceCount++;}assert(scholarlyClaimSources>=1,`Theory-claim mangler scholarly kjerne: ${t.theory_id}/${id}`);return c;});
