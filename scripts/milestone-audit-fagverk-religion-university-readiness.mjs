...
const STATUS_NEXT_GATE = 'religion_concept_registry_and_final_completion_audit';
const COMPLETION_NEXT_GATE = 'canonical_concept_registry_and_final_source_resolution';
...
export function auditReligionUniversityReadiness({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(statusEntry?.nextGate === STATUS_NEXT_GATE, 'Religion skal fortsette gjennom den registrerte begreps- og sluttporten');
  ...
  assert(isDeepStrictEqual(progress.completed_area_ids, REQUIRED_AREA_IDS), 'Alle tolv produksjonsområdene skal være låst som artikkelkomplette');
  assert(isDeepStrictEqual(progress.materialized_topic_ids, topicIds), 'Produksjonsprogresjonen har feil emne-ID-er');
  assert(progress.standalone_topic_articles_materialized === 72 && progress.standalone_topic_articles_remaining === 0, 'Religion skal stå på 72/72 artikler');
  ...
  assert(progress.quality_score >= 27 && progress.complete_ready === false, 'Produksjonsprogresjonen har feil kvalitetsstatus eller hopper over begrepsporten');
  ...
  assert(completion.current_complete_ready === false, 'Religion kan ikke være completeReady før begrepsregister og sluttkontroll');
  ...
  const report = {
    ...
    gates: {
      ...
      allTwelveUniversityAreaArticlesCompleteAtHighQuality: completeAreaCount === 12 && progress.quality_score >= 27,
      canonicalConceptRegistryStillRequiredBeforeComplete: completion.requirements.includes('canonical_concept_registry_100_percent_materialized_and_sourced') && completion.current_complete_ready === false
    }
  };
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    ...
    console.log(`Religion-universitetsmatrise OK: ${report.target.universityAreaCount}/12 områder, ${report.gaps.standaloneTopicArticlesMaterialized}/72 artikler og ${report.target.requiredMethodCount} metoder; begrepsport gjenstår.`);
  } catch (error) {
