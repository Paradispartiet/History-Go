...
export function auditMediaOffentlighetYtringsfrihetEtikkPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters[0].id === previousChapter.id && registrySubject.chapters[1].id === chapter.id && registrySubject.chapters[2].id === 'kilder-kritikk-og-sannhet' && registrySubject.chapters[3].id === 'plattformer-algoritmer-og-distribusjon' && registrySubject.chapters[4].id === 'propaganda-pavirkning-og-informasjonskrig' && registrySubject.chapters[5].id === 'medieokonomi-eierskap-og-arbeid', 'Media-kapittelrekkefølgen er feil');
  ...
  const report = {
    ...
    subject: {
      ...
      registeredChapterCount: 6, editorialStatus: statusEntry.editorialStatus,
