...
export function auditMediaOffentlighetYtringsfrihetEtikkPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters.length === 2 && registryChapter, 'Media-registeret skal ha nøyaktig to kapitler');
  assert(registrySubject.chapters[0].id === previousChapter.id && registrySubject.chapters[1].id === chapter.id, 'Media-kapittelrekkefølgen er feil');
  ...
  assert(phase3.report.summary.registeredChapterCount === 2, 'Fase 3-auditen ser ikke begge Media-kapitlene');
  ...
  const report = {
    ...
    subject: {
      ...
      registeredChapterCount: 2, editorialStatus: statusEntry.editorialStatus,
