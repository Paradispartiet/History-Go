// Split Lisbon kunst aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/kunst/europe/portugal/lisbon');
const sourceFile = 'places_lisbon_kunst.json';
const sourceStem = 'places_lisbon_kunst';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/kunst/europe/portugal/lisbon/${sourceFile} to be a JSON array.`);
}
