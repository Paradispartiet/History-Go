...
const AREA_ID = 'ritual_materiality_space';
const P = Object.freeze({
  ...
  claims: 'data/fagverk/religion/ritual-materialitet-hellige-rom/claims.json',
  articleDir: 'data/fagverk/religion/emneartikler',
  status: 'data/fagverk/subject_status.json',
  report: 'reports/fagverk/religion-ritual-materiality-space-articles-v1-audit.json'
});
...
export function auditReligionRitualMaterialitySpaceArticles({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(isDeepStrictEqual(readiness.production_progress.completed_area_ids, ['theory_method', 'history_comparison', 'west_asian_abrahamic', 'south_asian_religions', 'east_asian_religions', 'indigenous_sami', AREA_ID]), 'Eksakt sju Religion-områder skal være komplette');
  assert(isDeepStrictEqual(readiness.production_progress.materialized_topic_ids.slice(36), requiredIds), 'Readiness har feil ritual, materialitet og hellige rom-emner');
  ...
  assert(isDeepStrictEqual(actualFiles, expectedFiles), 'Mangler eksakt én artikkelfil per Ritual, materialitet og hellige rom-emne');
  ...
  assert(registry.source_documents.includes(P.claims), 'Religion-kilderegisteret peker ikke til Ritual, materialitet og hellige rom-claims');
  ...
  assert(Object.values(claimsDocument.source_policy || {}).every(Boolean), 'Batchens ritual-, rom-, materialitets- eller representasjonsvern er ikke låst');
  ...
  assert(sourceById.size === 20, 'Ritual, materialitet og hellige rom skal ha 20 unike kilderegistreringer');
  assert(claimById.size === 36, 'Ritual, materialitet og hellige rom skal ha 36 unike claims');
  ...
  assert(REQUIRED_AREA_METHOD_IDS.every((id) => usedMethodIds.has(id) && readiness.production_progress.materialized_required_method_ids.includes(id)), 'Områdets ritual-, materialitets- og rommetoder er ikke materialisert og brukt');
  ...
  const report = {
    ...
    version: '1.0.0',
    status: complete ? 'religion_ritual_materiality_space_articles_complete' : 'religion_ritual_materiality_space_articles_in_progress',
    generatedFrom: P,
    subject: { id: 'religion', areaId: AREA_ID, editorialStatus: status.editorialStatus, nextGate: status.nextGate, completeReady: false },
    coverage: { requiredArticleCount: 6, materializedArticleCount: articles.length, completedUniversityAreaCount: 7, totalUniversityAreaCount: 12, completedTopicCount: 42, totalTopicCount: 72, articleIds: requiredIds },
    ...
    gates: {
      exactSixRitualMaterialitySpaceArticles: true,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const { report } = auditReligionRitualMaterialitySpaceArticles({ writeReport: args.has('--write-report'), checkReport: !args.has('--write-report') && !args.has('--no-check-report') });
    console.log(`Religion ritual, materialitet og hellige rom OK: ${report.coverage.materializedArticleCount}/6 artikler, ${report.depth.totalEditorialWordCount} ord, ${report.evidence.registeredClaimCount} claims, kvalitet ${report.quality.total}/30.`);
  } catch (error) {
    console.error(`Religion ritual, materialitet og hellige rom FEIL: ${error.message}`);
    process.exitCode = 1;
  }
}
