...
const P={canon:'data/fag/TV_og_Film/theory_objects_film_tv_canonical_v1.json',scholarly:'data/fag/TV_og_Film/film_tv_theory_scholarly_registry_v1.json',registry:'data/fagverk/fagverk_registry.json'};
const ACADEMIC=new Set(['peer_reviewed_scholarship','scholarly_book']);

export function auditFilmTvTheoryCanon(){
 const canon=json(P.canon),sch=json(P.scholarly),registry=json(P.registry);const chapters=registry.subjects?.film_tv?.chapters||[];
 assert(canon.schema==='history_go_film_tv_theory_objects_v1'&&canon.status==='canonical','Ugyldig Film & TV theory canon');assert(sch.schema==='history_go_film_tv_theory_scholarly_registry_v1'&&sch.status==='canonical','Ugyldig Film & TV scholarly theory registry');assert(/provenance, ikke trivia/.test(canon.production_rule),'Film & TV theory canon må blokkere navnetrivia');
 ...
 assert(canon.theory_objects.length===20,'Film & TV theory canon skal ha 20 teoriobjekter = 2 per domene');const counts={},people=new Set(),works=new Set(),usedSch=new Set();
 ...
 return {status:'strong_theory_canon',domainCount:domainData.size,canonicalEmneCount:allEmnes.size,theoryObjectCount:canon.theory_objects.length,scholarlySourceCount:schById.size,uniquePeopleCount:people.size,uniqueWorkCount:works.size};
}
