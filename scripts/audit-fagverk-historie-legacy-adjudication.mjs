...
function runAnchorAudit() {
  const result = spawnSync(process.execPath, ['scripts/audit-fagverk-historie-legacy-theory.mjs'], {
    cwd: ROOT,
    encoding: 'utf8'
  });
  if (result.status !== 0) {
    throw new Error(`Historie legacy anchor audit feilet:\n${result.stderr || result.stdout}`);
  }
...
if (adjudication.subject_id !== 'historie') throw new Error('Adjudiseringen må eie subject_id=historie.');
...
if (adjudication.policy?.redirect_target !== EXPECTED_TARGET) throw new Error('Uventet Historie redirect-target.');
...
if (missingDecisions.length) throw new Error(`Mangler eksplisitt Historie-adjudisering: ${missingDecisions.join(', ')}`);
if (unknownDecisions.length) throw new Error(`Ukjente Historie-adjudiseringsseksjoner: ${unknownDecisions.join(', ')}`);
...
const rows = anchorAudit.rows.map((anchorRow) => {
  ...
  if (nonCanonicalOwnerFiles.length) throw new Error(`${anchorRow.id} peker utenfor canonical Historie manifest/registry-korpus: ${nonCanonicalOwnerFiles.join(', ')}`);
...
const report = {
  schema: 'history_go_fagverk_historie_legacy_adjudication_audit_v1',
  subject: 'historie',
  inputs: {
