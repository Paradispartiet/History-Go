// Split Oslo popular culture aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/popkultur/oslo');
const sourceFile = 'places_oslo_populaerkultur.json';
const sourceStem = 'places_oslo_populaerkultur';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/popkultur/oslo/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/popkultur/oslo/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/popkultur/oslo/${sourceFile}`,
