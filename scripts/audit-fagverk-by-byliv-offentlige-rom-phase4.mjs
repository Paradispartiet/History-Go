...
export async function auditByBylivOffentligeRomPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(statusEntry?.editorialStatus === 'complete', 'By skal stå complete etter første redigerte kapittel');
  ...
  assert(model.chapters.length === 17, 'Normalisert By-modell skal vise ni kapitler etter første Arkitektur-batch');
  ...
  const report = {
    ...
    gates: {
      ...
      firstChapterPreservedAcrossTwelveChapterRegistry: true,
