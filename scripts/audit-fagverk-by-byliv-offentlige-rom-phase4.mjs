...
export async function auditByBylivOffentligeRomPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters.length === 2, 'By skal nå ha to registrerte Fase 4-kapitler');
  ...
  assert(model.chapters.length === 2, 'Normalisert By-modell skal vise to kapitler etter andre Byliv-batch');
  ...
  const report = {
    ...
    gates: {
      ...
      firstChapterPreservedAcrossTwoChapterRegistry: true,
