...
const P = Object.freeze({
  chapter: 'data/fagverk/politikk/normer-identitet-hverdagsliv.json',
  brief: 'data/fagverk/politikk/normer-identitet-hverdagsliv/brief.json',
  claims: 'data/fagverk/politikk/normer-identitet-hverdagsliv/claims.json',
  module1: 'data/fagverk/politikk/normer-identitet-hverdagsliv/01-grunnlag.json',
  module2: 'data/fagverk/politikk/normer-identitet-hverdagsliv/02-fordypning.json',
  module3: 'data/fagverk/politikk/normer-identitet-hverdagsliv/03-anvendelse.json',
  ...
  report: 'reports/fagverk/politikk-normer-identitet-hverdagsliv-audit.json'
});
...
export function auditPolitikkNormerIdentitetHverdagslivChapter({ writeReport = false, checkReport = true } = {}) {
  ...
  const domain = (pensum.domains || []).find((row) => row.domain_id === 'normer_identitet_hverdagsliv');
  ...
  assert(chapter.id === 'normer-identitet-hverdagsliv' && chapter.chapter_id === chapter.id, 'Kapittel-ID er usynkron');
  ...
  assert(equalSet(chapter.method_ids || [], domain.method_ids || []), 'Kapittelet dekker ikke nøyaktig fagområdets seksten metoder');
  ...
  const report = {
    schema: 'history_go_politikk_chapter_normer_identitet_hverdagsliv_audit_v1',
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = auditPolitikkNormerIdentitetHverdagslivChapter({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') });
