...
export function auditFilmTvLegacyTheory(){
  ...
  return{schema:'history_go_fagverk_film_tv_legacy_theory_audit_v1',subject:'film_tv',legacy:{badgePage:LEGACY,originalBlobSha:ORIGINAL,activeBlobSha:activeSha,sourcePreserved:true,sectionCount:rows.length,knowledgeSectionCount:knowledge.length},canonical:{jsonFileCount:files.length,corpusCharacterCount:corpus.length,corpusTruncationFloor:MIN_CORPUS,domainCount:10,emneCount:192,methodCount:119,chapterCount:17,claimCount:663,sourceCount:416,semanticOwnerEvidence:{technicianBlindSpot:{ownerFile:PRODUCTION_OWNER,productionTeamEmne,invisibleProductionAlias,responsibilityEvidence}}},navigation:{badgePage:portal.badgePage,subjectPage:portal.subjectPage,futureTarget:TARGET,legacyRouteActive:true,routeRetired:false},summary:{knowledgeSectionCount:knowledge.length,anchorCompleteCount:knowledge.filter(r=>r.anchorCoverage===1).length,manualReviewCount:manual.length,manualReview:manual,redirectReady:false,redirectBlockReason:'Raw Film & TV coverage never authorizes redirect by itself. Explicit section adjudication and any proven gap migration are required.'},rows};
}
