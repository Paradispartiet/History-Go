...
const ARCHIVE='data/fag/TV_og_Film/archive/merke_film_tv_legacy_20260830.html';
...
const POLICY={
  ...
  blindsoner:[['tekniker','produksjonsteam'],['klipper'],['lyd'],['kino'],['filmklubb'],['kvinner','kjønn'],['minoritet','representasjon'],['arbeid'],['tv-histor','fjernsynshistor'],['arkiv'],['dokumentar'],['nyhets'],['kommersielle','kommersialisering'],['politisk']],
...
export function auditFilmTvLegacyTheory(){
  for(const f of [LEGACY,ARCHIVE,PORTAL,HOLISTIC,PRODUCTION_OWNER])assert(fs.existsSync(abs(f)),`Mangler ${f}`);
  const archiveBuffer=fs.readFileSync(abs(ARCHIVE)),archiveSha=blobSha(archiveBuffer);assert(archiveSha===ORIGINAL,`Film & TV archive-blob mismatch: ${archiveSha}`);
  const wrapper=read(LEGACY);assert(/location\.replace/.test(wrapper),'Film & TV compatibility-side mangler location.replace');assert(wrapper.includes(`../../../${TARGET}`),'Film & TV compatibility-side peker ikke til Progresjon');assert(!/merke-blokk|FILM\s*&\s*TV\s*[–-]\s*full teoretisk beskrivelse|<h2>1\. Felt<\/h2>|id=["']begreper["']/i.test(wrapper),'Film & TV compatibility-side beholder legacy-teori');
  const legacySections=sections(archiveBuffer.toString('utf8'));assert(JSON.stringify(legacySections.map(x=>x.id))===JSON.stringify(EXPECTED),`Uventet Film & TV archive-struktur: ${legacySections.map(x=>x.id).join(', ')}`);
  const files=ROOTS.flatMap(r=>walk(r)).filter(f=>f!==ADJUDICATION);const strings=[];for(const f of files){try{flatten(json(f),strings)}catch{}}
  ...
  const portal=json(PORTAL).categories?.find(x=>x.id==='film_tv');assert(portal?.badgePage===TARGET,`Film & TV portal peker ikke til Progresjon: ${portal?.badgePage}`);
  const knowledge=rows.filter(r=>r.role==='knowledge'),manual=knowledge.filter(r=>r.anchorCoverage<1).map(r=>r.id);
  return{schema:'history_go_fagverk_film_tv_legacy_theory_audit_v1',subject:'film_tv',legacy:{badgePage:LEGACY,archivePage:ARCHIVE,originalBlobSha:ORIGINAL,archiveBlobSha:archiveSha,sourcePreserved:true,sectionCount:rows.length,knowledgeSectionCount:knowledge.length},canonical:{jsonFileCount:files.length,corpusCharacterCount:corpus.length,corpusTruncationFloor:MIN_CORPUS,domainCount:10,emneCount:192,methodCount:119,chapterCount:17,claimCount:663,sourceCount:416,semanticOwnerEvidence:{technicianBlindSpot:{ownerFile:PRODUCTION_OWNER,productionTeamEmne,invisibleProductionAlias,responsibilityEvidence}}},navigation:{badgePage:portal.badgePage,subjectPage:portal.subjectPage,futureTarget:TARGET,legacyRouteActive:false,routeRetired:true},summary:{knowledgeSectionCount:knowledge.length,anchorCompleteCount:knowledge.filter(r=>r.anchorCoverage===1).length,manualReviewCount:manual.length,manualReview:manual,redirectReady:false,redirectBlockReason:'Raw Film & TV coverage never authorizes route retirement by itself; the explicit adjudication gate is also required.'},rows};
}
