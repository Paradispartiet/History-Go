...
export function auditKunstFeltInstitusjonPhase4({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(registrySubject.chapters.length === 1 && registryChapter, 'Kunst-registeret skal ha nøyaktig ett kapittel');
  ...
  assert(statusEntry.editorialStatus === 'chapters_in_progress', 'Kunst kan ikke stå complete etter ett av seks domener');
  ...
  assert(phase3.report.summary.registeredChapterCount === 1, 'Fase 3-auditen ser ikke det nye kapittelet');
