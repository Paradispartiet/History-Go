...
const defs = {
  oslo_hospital: { label: 'Oslo Hospital', address: 'Ekebergveien 1 Oslo' },
  botsfengselet: { label: 'Botsfengselet', address: 'Åkebergveien 11 Oslo' },
  prinds_christian_augusts_minde: { label: 'Prinds Christian Augusts Minde', address: 'Storgata 36 Oslo' },
  gamle_radhus: { label: 'Gamle Rådhus', address: 'Nedre Slottsgate 1 Oslo' }
};
const results = {};
for (const [id, def] of Object.entries(defs)) results[id] = readFinder(path.join(REPORT_DIR, `${id}-geonorge.json`), def.label);
requireVerified(results.botsfengselet, defs.botsfengselet.label);
requireVerified(results.gamle_radhus, defs.gamle_radhus.label);
if (!['needs_review', 'verified_candidate'].includes(results.oslo_hospital?.status)) throw new Error('Uventet Oslo Hospital finder-status: ' + results.oslo_hospital?.status);
...
if (!prindsenVerified && results.prinds_christian_augusts_minde?.status !== 'needs_review') throw new Error('Prindsen ga uventet finder-status: ' + results.prinds_christian_augusts_minde?.status);

const hospitalUpdate = {
  locatorType: 'linear_area',
  sourceProvider: 'osm',
  sourceObjectId: 'osm-way:111555053',
  geocodeAccuracy: 'semantic_anchor',
  coordRole: 'area_anchor',
  coordType: 'historic_hospital_complex_anchor',
  coordStatus: 'verified_geometry',
  coordSource: 'OpenStreetMap way 111555053 – Oslo Hospital; kryssjekket mot Oslo Hospital/Ekebergveien 1',
  coordSourceId: 'osm-way:111555053',
  coordSourceUrl: 'https://www.openstreetmap.org/way/111555053',
  coordVerifiedAt: DATE,
  coordNote: 'Geonorge-adressen Ekebergveien 1 ga flere ikke-entydige treff fordi Oslo Hospital er et historisk bygningskompleks. Det eksisterende hovedpunktet beholdes på det sentrale historiske hospitalbygget modellert som OSM way 111555053. Den offisielle Oslo Hospital-kilden dokumenterer at tomten avgrenses av Ekebergveien, Oslo gate og Konows gate. Punktet er derfor et representativt kompleksanker, ikke et påstått sentrum for hele eiendommen.'
};
const botsUpdate = finderUpdate(results.botsfengselet, 'Offisiell adressekoordinat fra Geonorge Adresser API for fengselsanlegget i Åkebergveien 11. Oslo kommune dokumenterer Oslo fengsel på samme adresse, mens recorden gjelder det historiske Botsfengselet/Botsen innenfor anlegget. Punktet brukes som fysisk anker for fengselskomplekset.');
const prindsenUpdate = prindsenVerified ? finderUpdate(results.prinds_christian_augusts_minde, 'Offisiell adressekoordinat fra Geonorge Adresser API for Prindsen-komplekset i Storgata 36. Oslo kommune dokumenterer Prindsen-virksomhet på samme adresseområde. Punktet representerer det historiske anlegget, ikke bare én moderne aktivitet.') : null;
const radhusUpdate = finderUpdate(results.gamle_radhus, 'Offisiell adressekoordinat fra Geonorge Adresser API for Gamle Rådhus i Nedre Slottsgate 1. Oslo kommune dokumenterer samme besøksadresse. Punktet representerer selve rådhusbygningen.');

const added = readJson(ADDED);
const updates = { oslo_hospital: hospitalUpdate, botsfengselet: botsUpdate, ...(prindsenVerified ? { prinds_christian_augusts_minde: prindsenUpdate } : {}) };
...
for (const [id, update] of Object.entries(updates)) {
  ...
  const row = readJson(file); Object.assign(row, update); delete row.coordPrecision; delete row.coordPrecisionM; writeJson(file, row);
}
const addedManifest = readJson(ADDED_MANIFEST);
addedManifest.source_sha256 = sha256(ADDED); addedManifest.generated_at = new Date().toISOString();
for (const entry of addedManifest.places || []) if (updates[entry.id]) entry.sha256 = sha256(path.join(path.dirname(ADDED_MANIFEST), entry.file));
writeJson(ADDED_MANIFEST, addedManifest);
const addedIndex = readJson(ADDED_INDEX);
for (const id of Object.keys(updates)) {
  const row = addedIndex.find((p) => p?.id === id); const source = added.find((p) => p?.id === id);
...
const radhusData = readJson(GAMLE_RADHUS);
if (!Array.isArray(radhusData) || radhusData.length !== 1 || radhusData[0]?.id !== 'gamle_radhus') throw new Error('Uventet gamle_radhus-format');
Object.assign(radhusData[0], radhusUpdate); delete radhusData[0].coordPrecision; delete radhusData[0].coordPrecisionM; writeJson(GAMLE_RADHUS, radhusData);
const places = {
  ...
  gamle_radhus: radhusData[0]
};

function writeAppliedEvidence(id, placeFile, resolvedIdentity, finding, address = null) {
  const place = places[id];
  writeJson(path.join(EVIDENCE_ROOT, `oslo/${id === 'gamle_radhus' ? 'by' : 'historie'}/${id}.json`), {
    placeId: id, placeFile, evidenceStatus: 'applied_to_place', coordinateDecision: 'do_not_change_coordinates_yet', currentCoordinate: snapshot(place),
    identity: { currentName: place.name, resolvedIdentity, identityStatus: 'resolved', identityProblem: '', locatorTypeCandidate: place.locatorType, requiresSplit: false, splitReason: '' },
    ...
    evidence: [{ sourceProvider: place.sourceProvider, sourceName: place.coordSource, sourceUrl: place.coordSourceUrl, sourceObjectId: place.sourceObjectId, sourceQuality: address ? 'official_address_plus_documented_identity' : 'stable_geometry_plus_official_site_identity', finding, canVerifyCoordinate: true, reason: place.coordNote }],
    addressCandidates: address ? [{ address, sourceProvider: 'official_address', sourceObjectId: place.sourceObjectId, canApplyToPlace: true }] : [{ address: 'Ekebergveien 1 Oslo', sourceProvider: 'official_address', canApplyToPlace: false }],
...
writeAppliedEvidence('oslo_hospital', ADDED_REL, 'det historiske Oslo Hospital-komplekset i Ekebergveien 1', 'Geonorge-adressen er flertydig, men OSM way 111555053 gir et konkret fysisk anker på det sentrale historiske hospitalbygget, mens den offisielle Oslo Hospital-kilden avgrenser hele tomten.');
writeAppliedEvidence('botsfengselet', ADDED_REL, 'det historiske Botsfengselet/Botsen i fengselsanlegget Åkebergveien 11', 'Geonorge gir ett entydig offisielt adressepunkt for Åkebergveien 11.', 'Åkebergveien 11 Oslo');
writeAppliedEvidence('gamle_radhus', GAMLE_RADHUS_REL, 'Gamle Rådhus i Nedre Slottsgate 1', 'Geonorge gir ett entydig offisielt adressepunkt for Nedre Slottsgate 1.', 'Nedre Slottsgate 1 Oslo');

const prindsenEvidenceFile = 'oslo/historie/prinds_christian_augusts_minde.json';
if (prindsenVerified) {
  writeAppliedEvidence('prinds_christian_augusts_minde', ADDED_REL, 'det historiske Prindsen/Mangelsgården-komplekset i Storgata 36', 'Geonorge gir ett entydig offisielt adressepunkt for Storgata 36.', 'Storgata 36 Oslo');
} else {
  ...
  writeJson(path.join(EVIDENCE_ROOT, prindsenEvidenceFile), {
    placeId: 'prinds_christian_augusts_minde', placeFile: ADDED_REL, evidenceStatus: 'needs_research', coordinateDecision: 'needs_geometry', currentCoordinate: snapshot(place),
    ...
    addressCandidates: [{ address: 'Storgata 36 Oslo', sourceProvider: 'official_address', canApplyToPlace: false }, { address: 'Storgata 36B Oslo', sourceProvider: 'official_address', canApplyToPlace: false }], sourceObjectCandidates: [], geometryCandidates: [], coordinateCandidates: [],
    decision: { canBecomeVerified: false, blockedReason: blocked, nextAction: 'Finn offisiell kompleks-/eiendomsgeometri eller dokumentert representativt anker før koordinaten endres.' }, notes: ['Eksisterende koordinat beholdes uendret. Ikke velg første eller nærmeste adressekandidat.']
  });
}

const evidenceManifest = readJson(EVIDENCE_MANIFEST);
for (const file of ['oslo/historie/oslo_hospital.json', 'oslo/historie/botsfengselet.json', prindsenEvidenceFile, 'oslo/by/gamle_radhus.json']) if (!evidenceManifest.files.includes(file)) evidenceManifest.files.push(file);
...
const batchText = prindsenVerified
  ? 'Batch 20 avslutter Oslo-klyngen i globalmanifestet med fire godkjente historiske bygg og anlegg: Oslo Hospital som geometriforankret kompleksanker, samt Botsfengselet, Prinds Christian Augusts Minde og Gamle Rådhus som adresseverifiserte steder.'
  : 'Batch 20 avslutter Oslo-klyngen i globalmanifestet med tre godkjente historiske bygg og anlegg: Oslo Hospital som geometriforankret kompleksanker samt Botsfengselet og Gamle Rådhus som adresseverifiserte steder. Prinds Christian Augusts Minde står separat som `needs_review` fordi Storgata 36 gir flere ikke-entydige adressekandidater for det historiske komplekset.';
protocol = replaceRequired(protocol, oldSummary, `Oslo-tabellen inneholder nå ${totalApproved} verifiserte eller kildekontrollerte canonical steder. ${batchText} ${reviewCount} fullførte kontroller fra Oslo-køen står dermed separat uten godkjent Oslo-koordinat. Senere visuell kontroll korrigerte ankrene for Oslo domkirke, Tronsmo Bokhandel, Grønland basarene, Møllergata 19 og Villa Grande uten at de ble telt på nytt.`, 'Oslo summary');
