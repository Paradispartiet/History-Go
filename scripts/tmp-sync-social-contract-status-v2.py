...
def replace_header(path: str, header: str) -> None:
    text = read(path)
    updated, count = re.subn(
        r"(?:Date|Contract date): 2026-06-30  \nStatus:.*?\n(?:Current implementation:.*?\nSist kontrollert:.*?\n\nThis document.*?\n\n## Current implementation status\n.*?\n\n)?(?=## 1\. Purpose)",
        header,
        text,
        count=1,
        flags=re.S,
    ...
    if count != 1:
        raise SystemExit(f"Kunne ikke synkronisere statusheader i {path}")
    write(path, updated)
