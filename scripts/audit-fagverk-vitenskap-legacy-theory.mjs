...
export function auditVitenskapLegacyTheory(){
  for(const f of [COMPAT,ARCHIVE,PORTAL,MANIFEST,REGISTRY,INTEGRITY])assert(fs.existsSync(abs(f)),`Mangler ${f}`);
  const buffer=fs.readFileSync(abs(ARCHIVE)); const archiveSha=blobSha(buffer); assert(archiveSha===ORIGINAL,`Vitenskap archive-blob mismatch: ${archiveSha}`);
  const compatibility=read(COMPAT);assert(/location\.replace\s*\(/.test(compatibility),'Vitenskap compatibility-side mangler location.replace');assert(compatibility.includes('../../../fagverk.html?subject=vitenskap#fagverkIaProgresjon'),'Vitenskap compatibility-side peker ikke til Progresjon');assert(!/merke-blokk|VITENSKAP\s*&\s*TEKNOLOGI\s*[–-]\s*full teoretisk beskrivelse|id=[\"']felt[\"']|emner-vitenskap/i.test(compatibility),'Vitenskap compatibility-side beholder legacy teori/UI');
  ...
  const portal=json(PORTAL).categories?.find(x=>x.id==='vitenskap');assert(portal?.badgePage===TARGET,`Vitenskap-portalen er ikke pensjonert til Progresjon: ${portal?.badgePage}`);assert(portal?.subjectPage==='fagverk.html?subject=vitenskap','Vitenskap subjectPage er endret utilsiktet');
  const knowledge=rows.filter(r=>r.role==='knowledge'),manual=knowledge.filter(r=>r.anchorCoverage<1).map(r=>r.id);
  return{schema:'history_go_fagverk_vitenskap_legacy_theory_audit_v1',subject:'vitenskap',legacy:{compatibilityPage:COMPAT,archivePage:ARCHIVE,originalBlobSha:ORIGINAL,archiveBlobSha:archiveSha,sourcePreserved:true,sectionCount:rows.length,knowledgeSectionCount:knowledge.length,productSectionCount:2},canonical:{manifestGraphFileCount:g1.files.length,registryGraphFileCount:g2.files.length,registryChapterCount:chapterCount,corpusCharacterCount:corpus.length,corpusTruncationFloor:MIN_CORPUS,strictMajorFields:strict.summary.canonicalMajorFields,strictEmners:strict.summary.universalCanonicalEmnesValidated,strictClaims:strict.lockedBaseline.claims,strictSources:strict.lockedBaseline.sources},navigation:{badgePage:portal.badgePage,subjectPage:portal.subjectPage,compatibilityPage:COMPAT,target:TARGET,legacyRouteActive:false,routeRetired:true,portalRedirected:true},summary:{knowledgeSectionCount:knowledge.length,anchorCompleteCount:knowledge.filter(r=>r.anchorCoverage===1).length,manualReviewCount:manual.length,manualReview:manual,redirectReady:false,redirectBlockReason:'Raw Vitenskap coverage never authorizes redirect by itself; the retired route is valid only together with the explicit adjudication contract.'},rows};
}
