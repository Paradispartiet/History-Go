...
function runAnchorAudit() {
  const result = spawnSync(process.execPath, ['scripts/audit-fagverk-by-legacy-theory.mjs'], {
    cwd: ROOT,
    encoding: 'utf8'
  });
  if (result.status !== 0) {
    throw new Error(`By legacy anchor audit feilet:\n${result.stderr || result.stdout}`);
  }
...
if (adjudication.schema !== 'history_go_fagverk_legacy_theory_adjudication_v1') {
  throw new Error(`Ukjent adjudiseringsschema: ${adjudication.schema}`);
}
if (adjudication.subject_id !== 'by') throw new Error('Adjudiseringen må eie subject_id=by.');
...
if (missingDecisions.length) throw new Error(`Mangler eksplisitt adjudisering: ${missingDecisions.join(', ')}`);
if (unknownDecisions.length) throw new Error(`Ukjente adjudiseringsseksjoner: ${unknownDecisions.join(', ')}`);
...
const rows = anchorAudit.rows.map((anchorRow) => {
  ...
  const missingMigrationFiles = migrationRefs
    .filter((ref) => ref.includes('/') && !ref.startsWith('PR #'))
  ...
  if (nonCanonicalOwnerFiles.length) throw new Error(`${anchorRow.id} peker utenfor canonical manifest/registry-korpus: ${nonCanonicalOwnerFiles.join(', ')}`);
  if (missingMigrationFiles.length) throw new Error(`${anchorRow.id} peker til manglende migreringsfiler: ${missingMigrationFiles.join(', ')}`);

  if (anchorRow.role === 'knowledge') {
    if (!KNOWLEDGE_DISPOSITIONS.has(decision.disposition)) {
      throw new Error(`Ugyldig knowledge-disposition for ${anchorRow.id}: ${decision.disposition}`);
    }
    if (!ownerFiles.length) throw new Error(`Knowledge-seksjonen ${anchorRow.id} må ha minst én canonical owner.`);
    if (anchorRow.anchorCoverage !== 1) throw new Error(`Knowledge-seksjonen ${anchorRow.id} har fortsatt canonical ankerhull.`);
    if (decision.disposition === 'migrated_to_canonical' && !migrationRefs.length) {
      throw new Error(`${anchorRow.id} er markert migrated_to_canonical uten migration_refs.`);
    }
  } else {
    if (!PRODUCT_DISPOSITIONS.has(decision.disposition)) {
      throw new Error(`Ugyldig legacy-product disposition for ${anchorRow.id}: ${decision.disposition}`);
    }
  ...
  return {
    ...
    role: anchorRow.role,
    duplicateOfTheory: anchorRow.duplicateOfTheory,
...
const byPortal = portal.categories?.find((item) => item.id === 'by');
if (!byPortal) throw new Error('By mangler i fagverk_portal.json.');
