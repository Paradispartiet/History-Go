// Split Lisbon nature aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/natur/europe/portugal/lisbon');
const sourceFile = 'places_lisbon_natur.json';
const sourceStem = 'places_lisbon_natur';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/natur/europe/portugal/lisbon/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/natur/europe/portugal/lisbon/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/natur/europe/portugal/lisbon/${sourceFile}`,
