// Split Telemark historie batch 3 aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/historie/telemark');
const sourceFile = 'places_historie_telemark_batch3.json';
const sourceStem = 'places_historie_telemark_batch3';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/historie/telemark/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/historie/telemark/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/historie/telemark/${sourceFile}`,
