...
export function audit() {
  const sourceBrief = read('data/fag/politikk/juss_rettsvitenskap/contracts_obligations_contract_law_source_claim_brief_v1.json');
  const chapter = read('data/fagverk/politikk/juss_rettsvitenskap/avtaler-obligasjoner-og-kontraktsrett.json');
  const claims = read('data/fagverk/politikk/juss_rettsvitenskap/avtaler-obligasjoner-og-kontraktsrett/claims.json');
  const assessment = read('data/fagverk/politikk/juss_rettsvitenskap/avtaler-obligasjoner-og-kontraktsrett/assessment.json');
  ...
  assert(sourceBrief.sources.length === 13 && sourceBrief.topic_briefs.length === 8 && plannedClaims.length === 32, 'Felt 8 source-first-kontrakt');
  assert(modules.length === 4 && sections.length === 8 && paragraphs.length === 32, 'Felt 8 krever 4/8/32 fulltekst');
  ...
  for (const token of ['avtaleloven', 'cisg', 'fullmakt', 'standardvilkår', 'direktiv 93/13', '§ 36', 'risikoovergang', 'tilleggsfrist', 'digitalytelsesloven', 'mangel', 'retting', 'omlevering', 'prisavslag', 'kontrollansvar', 'tapsbegrensning', 'gjeldsbrev', 'foreldelse', 'angrerett']) {
    assert(joined.includes(token), `Mangler kontraktsrettslig gate: ${token}`);
  }
  ...
  const report = {
    schema: 'history_go_juss_rettsvitenskap_contracts_obligations_contract_law_fulltext_audit_v1',
    version: '1.0.0',
    ...
    gates: {
      ...
      formation_scope_and_cisg: true,
      authority_terms_transparency: true,
      invalidity_and_unfairness_distinguished: true,
      performance_delivery_payment_and_risk: true,
      delay_and_remedies_distinguished: true,
      conformity_and_cure_by_contract_type: true,
      price_reduction_termination_damages_mitigation: true,
      assignment_limitation_withdrawal_and_cisg_scope: true,
      assessment: true,
    ...
    next_gate: 'register_domain_8_only_after_domain_9_tort_property_priority_source_first_is_ready'
  };
  write('reports/fagverk/juss-rettsvitenskap-contracts-obligations-contract-law-fulltext-v1-audit.json', report);
  return report;
}

try {
  const report = audit();
  console.log(`Avtaler/obligasjoner fulltext audit OK: ${report.counts.paragraphs} avsnitt, ${report.six_part_quality_review.total}/30.`);
} catch (error) {
