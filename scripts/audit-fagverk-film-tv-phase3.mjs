...
export function auditFilmTvPhase3({ writeReport = false, checkReport = true } = {}) {
  ...
  assert(statusEntry?.editorialStatus === 'structure_ready', 'Film & TV må stå structure_ready før kapittelproduksjon');
  assert(statusEntry?.nextGate === 'chapter_production', 'Film & TV har feil neste port');
  ...
  assert(model.chapters.length === 0, 'Structure-ready kan ikke late som Film & TV-kapitler finnes');
  ...
  const report = {
    ...
    version: '1.0.0',
    status: 'film_tv_phase_3_structure_ready',
    generatedFrom: P,
    subject: {
      id: model.subject.id,
