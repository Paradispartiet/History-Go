...
const P = {
  ...
  report: 'reports/fagverk/utdanning-special-education-fulltext-v1-audit.json',
};
...
export function audit({ writeReport = false } = {}) {
  ...
  assert(!brief.safety.individualDiagnosis && !brief.safety.fixedLearnerTyping && !brief.safety.diagnosisFatalism && !brief.safety.universalMethodClaim && !brief.safety.placementAsInclusionProof, 'Etiske og spesialpedagogiske grenser mangler');
  ...
  assert(registryRow?.chapters?.filter((row) => row.id === CHAPTER_ID).length === 1 && registryRow.editorialPlan.registeredChapterCount === 8 && registryRow.editorialPlan.completedSourceBriefCount === 9, 'Registry må være 8/14 med ni source briefs');
  ...
  assert(pensum.domains.filter((domain) => domain.status === 'materialized').length === 8 && pensum.domains[7].domain_id === 'spesialpedagogikk' && !pensum.complete_ready, 'Pensum skal være monotont 8/14');
  assert(nextBrief.scope.primary_domain_id === 'inkludering_tilpasset_opplaering' && !nextBrief.runtime_registration.registered, 'Inkludering og tilpasset opplæring må være neste uregistrerte source-first-domene');

  const report = {
    schema: 'history_go_utdanning_special_education_fulltext_audit_v1',
    version: '1.0.0',
    ...
    gates: {
      ...
      screeningNonDiagnosisBoundary: true,
      rtiNonFatalismBoundary: true,
      targetedSupportHeterogeneityBoundary: true,
      aacLanguageEnvironmentBoundary: true,
      iopImplementationAndEvaluationBoundary: true,
      transitionContinuityBoundary: true,
      chapterRegisteredExactlyOnce: true,
      educationMaterializedAndAudited: true,
      prematureSubjectCompletionBlocked: true,
      inclusionAdaptedEducationSourceFirstNext: true,
    },
    six_part_quality_review: {
      ...
      total: 29,
      maximum: 30,
      note: 'Spesialpedagogikkpakken er intern; 6 domener og strict subject-proof gjenstår før Utdanning kan bli complete.',
    },
  };
...
try {
  ...
  console.log(`Utdanning Spesialpedagogikk fulltekstaudit OK: ${report.counts.paragraphs} avsnitt, ${report.counts.verifiedClaims} claims, ${report.counts.inspectableSources} kilder; ${report.six_part_quality_review.total}/30.`);
} catch (error) {
  console.error(`Utdanning Spesialpedagogikk fulltekstaudit FEIL: ${error.message}`);
  process.exitCode = 1;
}
