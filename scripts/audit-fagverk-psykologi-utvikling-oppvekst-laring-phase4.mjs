...
const MODULES = [`${CHAPTER_DIR}/01-tidlig-utvikling-og-relasjoner.json`,`${CHAPTER_DIR}/02-laring-skole-og-motivasjon.json`,`${CHAPTER_DIR}/03-ungdom-identitet-og-livslop.json`];
const EXPECTED_RUNTIME_PLACE_IDS = ['psykologisk_institutt_uio'];
const EXPECTED_DEVELOPMENT_CASES = ['Tidlig utvikling og responsiv omsorg','Tilknytningsforskning','Skole, læring og motivasjon','Ungdom, sosial utvikling og identitet'];
const abs = (file) => path.join(ROOT, file);
const readJson = (file) => JSON.parse(fs.readFileSync(abs(file), 'utf8'));
const assert = (condition, message) => { if (!condition) throw new Error(message); };
const committedProjection = (report) => ({schema:report.schema,version:report.version,status:report.status,generatedFrom:report.generatedFrom,subject:report.subject,chapter:report.chapter,summary:report.summary,canonicalEmneIds:report.canonicalEmneIds,methodIds:report.methodIds,runtimePlaceIds:report.runtimePlaceIds,developmentCaseNames:report.developmentCaseNames,gates:report.gates});

export function auditPsykologiUtviklingOppvekstLaringPhase4({ writeReport = false, checkReport = true } = {}) {
  for (const file of [P.pensum,P.methods,P.registry,P.status,P.chapter,P.brief,P.claims,P.internalPlace,...MODULES]) assert(fs.existsSync(abs(file)), `Mangler ${file}`);
  const pensum = readJson(P.pensum), methodsDoc = readJson(P.methods), registry = readJson(P.registry), status = readJson(P.status), chapter = readJson(P.chapter), brief = readJson(P.brief), claimsDoc = readJson(P.claims), modules = MODULES.map(readJson);
  ...
  assert(canonicalEmneIds.length === 9 && isDeepStrictEqual(chapter.emne_ids, canonicalEmneIds), 'Kapittelet dekker ikke 9 canonicale emner i eksakt rekkefølge');
  ...
  const sectionEmneIds = new Set(sections.flatMap((s) => s.emne_ids || [])), sectionMethodIds = new Set(sections.flatMap((s) => s.method_ids || []));
  ...
  const sources = claimsDoc.sources || [], claims = claimsDoc.claims || [], externalSources = sources.filter((s) => s.type !== 'internal_place_record'), sourceIds = new Set(sources.map((s) => s.id)), claimIds = new Set(claims.map((c) => c.id));
  ...
  assert(externalSources.every((s) => /^https:\/\//.test(s.url)), 'Ekstern kilde mangler HTTPS');
  ...
  const runtimePlaceIds = (chapter.relatedPlaces || []).map((p) => p.id);
  assert(isDeepStrictEqual(runtimePlaceIds, EXPECTED_RUNTIME_PLACE_IDS), 'Kapittelet har ukjent runtime-place-ID');
  assert(sources.find((s) => s.id === 'src-hg-uio-place')?.url === P.internalPlace, 'UiO-place-kilden peker feil');
  const developmentCases = chapter.developmentCases || [], developmentCaseNames = developmentCases.map((item) => item.name);
  assert(isDeepStrictEqual(developmentCaseNames, EXPECTED_DEVELOPMENT_CASES) && developmentCases.every((item) => item.caseStatus === 'documented_case_not_runtime_place'), 'Utviklingscase-kontrakten avviker');
  const registrySubject = registry.subjects?.psykologi;
  const registeredChapterCount = registrySubject?.chapters?.length ?? 0;
  assert(registeredChapterCount >= 3 && registeredChapterCount <= 6, 'Psykologi skal ha mellom tre og seks registrerte kapitler under videre produksjon');
  ...
  assert(['chapters_in_progress','complete','expanded_and_audited'].includes(statusEntry?.editorialStatus), 'Psykologi har ugyldig redaksjonell fremdrift');
  if (registeredChapterCount < 6) assert(statusEntry?.nextGate === 'remaining_domain_chapter_production', 'Psykologi har feil nextGate under videre produksjon');
  const forbiddenPatterns = [/du har (?:en|et) [a-zæøå-]+lidelse/i,/du er (?:deprimert|psykotisk|bipolar)/i,/testen viser at du har/i,/barnet er (?:unormalt|utrygt tilknyttet)/i,/ungdom gjør dette fordi frontallappen ikke er ferdig/i];
  ...
  const report = {
    schema:'history_go_fagverk_psykologi_utvikling_oppvekst_laring_phase4_audit_v1',version:'1.1.0',status:'psykologi_utvikling_oppvekst_laring_chapter_ready',generatedFrom:P,
    subject:{id:'psykologi',editorialStatus:statusEntry.editorialStatus,nextGate:statusEntry.nextGate,registeredChapterCount,targetChapterCount:registrySubject.editorialPlan.targetChapterCount},
    ...
    summary:{emneCount:9,methodCount:18,moduleCount:3,sectionCount:9,paragraphCount:27,claimCount:27,sourceCount:19,externalSourceCount:18,runtimePlaceCount:runtimePlaceIds.length,developmentCaseCount:developmentCases.length},
    canonicalEmneIds,methodIds:chapter.method_ids,runtimePlaceIds,developmentCaseNames,
    gates:{exactCanonicalEmneCoverage:true,exactCanonicalMethodCoverage:true,threeModulesNineSectionsTwentySevenParagraphs:true,paragraphClaimTraceComplete:true,allClaimsUsedAndSourceResolved:true,eighteenExternalSourcesPresent:true,sourceLocationsComplete:true,doNotDiagnosePeopleGuardPresent:true,noIndividualTreatmentAdviceGuardPresent:true,noScreeningInterpretationGuardPresent:true,noDevelopmentalLabelingGuardPresent:true,noInventedRuntimePlaces:true,developmentCasesExplicitlyNonRuntime:true,registrySynchronizedDuringFurtherProduction:true,editorialProgressForwardCompatible:true}
  };
  if (writeReport) { fs.mkdirSync(path.dirname(abs(P.report)), {recursive:true}); fs.writeFileSync(abs(P.report), `${JSON.stringify(committedProjection(report), null, 2)}\n`); }
  if (checkReport) { assert(fs.existsSync(abs(P.report)), `${P.report} mangler. Kjør audit med --write-report`); assert(isDeepStrictEqual(readJson(P.report), committedProjection(report)), `${P.report} er utdatert`); }
  return {report:committedProjection(report),chapter,brief,claimsDoc,modules};
}

function main() {
  const args = new Set(process.argv.slice(2));
  try { const result = auditPsykologiUtviklingOppvekstLaringPhase4({writeReport:args.has('--write-report'),checkReport:!args.has('--no-check-report') && !args.has('--write-report')}); console.log(`Psykologi Utvikling Phase 4 OK: ${result.report.summary.emneCount}/9 emner, ${result.report.summary.methodCount} metoder, ${result.report.summary.paragraphCount} avsnitt, ${result.report.summary.claimCount} claims og ${result.report.summary.externalSourceCount} eksterne kilder.`); }
  catch (error) { console.error(`Psykologi Utvikling Phase 4 FEIL: ${error.message}`); process.exitCode = 1; }
}
