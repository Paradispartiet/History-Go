// Split Oslo psychology aggregate into one JSON file per place.
import { mkdir, readFile, rm, writeFile } from 'node:fs/promises';
import { join } from 'node:path';
import { createHash } from 'node:crypto';

const root = process.cwd();
const outDir = join(root, 'data/places/psykologi/oslo');
const sourceFile = 'places_psykologi.json';
const sourceStem = 'places_psykologi';
...
if (!Array.isArray(places)) {
  throw new Error(`Expected data/places/psykologi/oslo/${sourceFile} to be a JSON array.`);
}
...
const manifest = {
  ...
  source_path: `data/places/psykologi/oslo/${sourceFile}`,
...
const report = [
  ...
  '',
  `Source: data/places/psykologi/oslo/${sourceFile}`,
