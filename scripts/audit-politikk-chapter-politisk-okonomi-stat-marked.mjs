...
const P = Object.freeze({
  chapter: 'data/fagverk/politikk/politisk-okonomi-stat-marked.json',
  brief: 'data/fagverk/politikk/politisk-okonomi-stat-marked/brief.json',
  claims: 'data/fagverk/politikk/politisk-okonomi-stat-marked/claims.json',
  module1: 'data/fagverk/politikk/politisk-okonomi-stat-marked/01-grunnlag.json',
  module2: 'data/fagverk/politikk/politisk-okonomi-stat-marked/02-fordypning.json',
  module3: 'data/fagverk/politikk/politisk-okonomi-stat-marked/03-anvendelse.json',
  ...
  report: 'reports/fagverk/politikk-politisk-okonomi-stat-marked-audit.json'
});
...
export function auditPolitikkPolitiskOkonomiStatMarkedChapter({ writeReport = false, checkReport = true } = {}) {
  ...
  const domain = (pensum.domains || []).find((row) => row.domain_id === 'politisk_okonomi_stat_marked');
  assert(domain, 'Pensum mangler fagområdet politisk økonomi, stat og marked');
  ...
  assert(chapter.id === 'politisk-okonomi-stat-marked' && chapter.chapter_id === chapter.id, 'Kapittel-ID er usynkron');
  ...
  const report = {
    schema: 'history_go_politikk_chapter_politisk_okonomi_stat_marked_audit_v1',
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    const report = auditPolitikkPolitiskOkonomiStatMarkedChapter({ writeReport: args.has('--write-report'), checkReport: !args.has('--no-check-report') });
