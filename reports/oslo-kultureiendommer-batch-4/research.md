# Oslo kultureiendommer completeness pass — batch 4

Date: 2026-07-18

Source set: Oslo kommune / Kulturetaten cultural properties, cross-checked against current History Go repository search.

## Selected canonical gaps

### Geitmyra gård

No canonical place was found for `Geitmyra gård`, `Geitmyra matkultursenter` or Tåsenveien 2. Oslo kommune documents the property back to 1696 and the present main building to 1755. The municipality bought the property in 1884 for Nordre gravlund, and the preserved farm buildings are now used by Geitmyra matkultursenter for barn.

Representation: `historie`, combining historic farm architecture, municipal land-use change and present food-culture reuse.

Address-first result:
- `59.93719464243419, 10.744320304993959`
- `geonorge-adresser-v1:0301:17894:2`
- Tåsenveien 2, 0462 Oslo

### Grønland politistasjon

No canonical place was found for `Grønland politistasjon`, `Bækkegatens politistasjon` or Tøyenbekken 5. The building opened as a police station in 1903; the preserved arrest wing was used by the German Schnellkommando during the occupation. The police moved out in 1978, and the building now houses cultural and museum activities.

Representation: `historie`, with policing, occupation and adaptive cultural reuse as separate layers.

Address-first result:
- `59.91310411421603, 10.76346350302876`
- `geonorge-adresser-v1:0301:17872:5`
- Tøyenbekken 5, 0188 Oslo

### Tøyen trafo

No canonical place was found for `Tøyen trafo`, the former Oslo Lysverker transformer station, or Hovinveien 1. Oslo kommune dates the transformer station to 1915 and documents later cultural use from the 1970s; today Oslo teatersenter and artist studios use the building.

Representation: `naeringsliv`, focusing on electrical infrastructure, municipal utility history, industrial architecture and cultural reuse.

Address-first result:
- `59.91708427752621, 10.780668130159562`
- `geonorge-adresser-v1:0301:13143:1`
- Hovinveien 1, 0576 Oslo (Geonorge normative postcode; Oslo kommune page currently displays 0608)

## Coordinate method

All three candidates use the repository's normative address-first Geonorge workflow. The lookup results are saved under `reports/oslo-kultureiendommer-batch-4/coordinates/` and the source records reuse those exact verified candidates.

## Validation path

The dedicated integration workflow registers both category sources, rebuilds the global place index and runs coordinate parity, strict new-coordinate intake, canonical emne validation and the place-health report. The temporary workflows are removed before merge.

`main` received the separate seventh Oslo coordinate batch while this branch was in progress. The generated runtime index must therefore be rebuilt from the branch sources after that upstream coordinate work rather than manually patched or inferred.
