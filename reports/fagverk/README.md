...
Politikk står `materialized`, `audited` og `chapters_in_progress`. Ni av tretten fagområder har registrerte kapitler, mens fire fortsatt mangler fullverdig hovedkapittel. `Regimer og institusjoner`, `Valg, partier og velgeratferd`, `Offentlig politikk, beslutning og implementering`, `Internasjonal politikk, sikkerhet og samarbeid`, `Politisk økonomi, stat og marked`, `Statsvitenskapelig metode og sammenligning` og `Norsk politikk, EØS/EU og flernivåstyring` har egne briefs, påstandsregistre, avsnittssporing og permanente kapittelaudits. Faget har i tillegg permanente kvalitetsgater for 123 emner, 71 operative metoder, 123 mappingrader, 152 hooks, normal quizåpning, Knowledge-leveranse og canonical ID–navn-integritet for teoripersoner.
