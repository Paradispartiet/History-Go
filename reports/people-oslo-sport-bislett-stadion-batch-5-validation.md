...
Add a fifth dedicated people batch for `bislett_stadion`, continuing from batch 4 already present on `main`.
...
This batch shifts from world-record-only anchors to Bislett Games meeting-record anchors, still requiring an explicit Bislett Stadion / Bislett Games result.
...
- batches 1–4 Bislett Stadion people
- earlier `people_sport_oslo.json` Bislett-linked entries
