...
Denne reine batch 19-branchen er oppretta frå `main` etter:

- den korrekte Etne batch 18 for `sjokanten_trivsel_skanevik` vart merga
- den generiske Skakke-leiarbatchen vart reversert

Det betyr at batch 19 ikkje byggjer vidare på dei avviste Skakke-leiarane.

Etter den gyldige batch 18 er Etne-dekninga 59 av 81 aktive stader, med 22 udekte stader. `litledalen_kraftverk` står framleis på restlista. Batch 19 skal derfor flytte dekninga til 60/81 og restgjelda til 21 dersom ingen parallelle Etne-place-endringar kjem inn før merge.
...
Før skriving vart fersk repo-state søkt etter `halfdan_greve` og `Halfdan Greve`; det vart ikkje funne nokon eksisterande canonical people-identitet.

Batchtesten kontrollerer i tillegg normalisert ID, namn og utvalde variantar på tvers av heile people-manifestet etter integrasjon.
