...
for(const id of domains){
 ...
 const gp=generator.domain_quality_profiles[id]; ok(!!gp,'Generator har profil for '+id); ok(gp.hook_count===(id==='komparativ_politikk_regimer_institusjoner'?20:10),id+' generatorprofil har forventet hook-antall');
 for(const h of c.topic_hooks){ ok(h.quality_revision===R||(id==='komparativ_politikk_regimer_institusjoner'&&h.quality_revision===P1),h.id+' har gyldig revisjon'); ok(h.mechanisms.length>=6,h.id+' har minst seks mekanismer'); ok(h.critical_distinctions.length>=4,h.id+' har fire distinksjoner'); ok(h.theory_lenses.length===3,h.id+' har tre teorispor'); ok(h.recommended_method_ids.length>=3,h.id+' har minst tre metoder'); ok(h.generator_constraints.require_defined_outcome_or_dependent_variable===true,h.id+' krever definert utfall'); ok(h.generator_constraints.require_method_match_to_claim===true,h.id+' krever metodekobling'); }
}
for(const id of emneIds){ const e=emner.find(x=>x.emne_id===id); ok(!!e,'Emne finnes: '+id); ok(e.quality_revision===(id.startsWith('em_pol_institusjonsfordypning_')?P1:R),id+' har gyldig revisjon'); ok(e.method_ids.length>=4,id+' har metodebredde'); ok(e.generator_constraints.require_defined_unit_and_outcome===true,id+' krever analyseenhet og utfall'); const mp=mappings.find(x=>x.emne_id===id); ok(!!mp,'Mapping finnes: '+id); ok(mp.mappings.length===2,id+' har to mappinger'); for(const x of mp.mappings){ok(x.mechanism_options.length>=5,id+' mapping har mekanismer');ok(x.critical_distinction_options.length>=4,id+' mapping har distinksjoner');ok(x.recommended_method_ids.length>=3,id+' mapping har metoder');} }
for(const id of methodIds){ const m=methods.methods.find(x=>x.method_id===id); ok(!!m,'Metode finnes: '+id); ok(m.quality_revision===(id.startsWith('met_pol_parlamentarisk_')||id.startsWith('met_pol_vetospiller')||id.startsWith('met_pol_foderalisme_')||id.startsWith('met_pol_konstitusjonell_')||id.startsWith('met_pol_institusjonell_tilbakegang')||id.startsWith('met_pol_lovgivningsstruktur_')?P1:R),id+' har gyldig revisjon'); ok(m.mechanism_explanation_required===true,id+' krever mekanisme'); ok(m.critical_distinction_required===true,id+' krever distinksjon'); ok(m.question_build_sequence.length>=7,id+' har komplett byggesekvens'); }
