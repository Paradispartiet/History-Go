...
for (const topic of subcultureProduction.subcultureTopics) {
  topic.siteSpecificRationale = `Emnet brukes på dokumenterte relasjoner, forflytning, kontroll og rombruk ved ${placeId}; ingen person klassifiseres gjennom synlighet.`;
}
addUnique(subcultureProduction.subcultureTopics, { emneId: E, siteSpecificRationale: "KORUS, Oslo kommune og FHR dokumenterer både åpent marked, sosial møteplass, forflytning og omsorgslandskap ved krysset.", caseIds: [`case_${placeId}_environment`] }, row => row.emneId);
addUnique(subcultureProduction.subcultureTopics, { emneId: P, siteSpecificRationale: "Personvern er et stedsspesifikt fagkrav fordi synlige, sårbare personer aldri kan brukes som identitets- eller rollebevis.", caseIds: [`case_${placeId}_environment`] }, row => row.emneId);
