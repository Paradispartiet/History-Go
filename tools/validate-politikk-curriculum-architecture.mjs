...
export function validatePolitikkCurriculumArchitecture({ root = DEFAULT_ROOT } = {}) {
  ...
  assert(architecture.editorial_status === 'complete', 'Politikk-arkitekturen er ikke redaksjonelt komplett');
  ...
  assert(conceptDocument.schema === 'history_go_politikk_concepts_v1' && conceptDocument.status === 'editorially_complete', 'Begrepsverket er ikke aktivt og komplett');
  ...
  assert(statusEntry?.editorialStatus === 'complete', 'Politikk er ikke redaksjonelt complete i statusregisteret');
