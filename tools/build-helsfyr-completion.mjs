...
const urls = {
  ...
  sporveienLogoAsset: "https://www.sporveien.no/contentassets/d3761554e74a43d4b05a8742038b8ff7/logoer_web_rgb.zip",
...
await outputImage({ url: urls.commonsBruskelandWorkAsset, file: "bilder/kort/people/guttorm_bruskeland.webp", width: 900, height: 1200, fit: "contain" });
...
bruskeland.source_urls = [...new Set([...(bruskeland.source_urls || []), urls.lokalwikiBruskeland, urls.commonsBruskelandWorkPage])];
...
const sporveienBrand = {
  ...
  imageMeta: { sourcePage: urls.sporveienProfile, sourceAsset: urls.sporveienLogoAsset, creator: "Sporveien", credit: "Sporveien", rightsBasis: "official_brand_asset_referential_identification", reviewStatus: "manually_approved", assetKind: "official_main_logo", usageContext: "referential_identification", noEndorsement: true, generated: false, reconstructed: false, transformation: "Offisiell sort/hvit web-logo fra Sporveiens RGB-pakke er proporsjonalt innpasset på 900 × 520 uten rekonstruksjon.", outputDimensions: "900x520", reviewedAt: verifiedAt }
};
...
const sets = phases.map((phase, setIndex) => ({
  ...
  questions: specs.slice(setIndex * 7, setIndex * 7 + 7).map((spec, localIndex) => {
    ...
    const q = {
      ...
      categoryId: "by", placeId, targetId: placeId, question_scope: "place", question, options: balancedOptions, answer, answerIndex: desiredAnswerIndex,
    ...
    if (setIndex === 3 && localIndex === 6) {
      q.topic_hook_id = "byliv_opphold_vs_gjennomgang";
      q.thinker_id = "michel_de_certeau";
      q.work = "The Practice of Everyday Life";
      q.theory_ref = { topic_hook_id: q.topic_hook_id, thinker_id: q.thinker_id, work: q.work, why_it_helps: "De Certeaus perspektiv på hverdagspraksis hjelper å lese observerbare ruter og bevegelsesmønstre uten å tilskrive enkeltpersoner udokumenterte motiver." };
    }
    return q;
  })
}));
...
const curriculum = { module_ids: ["kur_by_03_infrastruktur_og_bevegelse"], emne_ids: [I, G], topic_hook_ids: ["byliv_opphold_vs_gjennomgang"], method_ids: ["met_feltobservasjon", "met_gaanalyse", "met_for_etter"], thinker_ids: ["michel_de_certeau"], works: ["The Practice of Everyday Life"] };
...
write(briefFile, {
  ...
  sources: Object.fromEntries(sourceRegistry.map(source => [source.id, source])), selected_curriculum: curriculum, existing_quiz_audit: existingQuizAudit, profile_decision: profileDecision, held_back_candidates: heldBackCandidates,
  claims: sets.flatMap(set => set.questions).map((q, i) => ({ claim_id: q.claim_id, order: i + 1, planned_phase: sets[Math.floor(i / 7)].phase, family: q.question_type === "fact" ? "fact" : q.question_type === "context" ? "context" : "concept_theory", statement: q.knowledge, source_ids: q.source, source_origin: "external", emne_id: q.emne_id }))
});
