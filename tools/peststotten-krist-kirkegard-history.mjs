...
export function buildHistoryProduction(place, briefFile, contextFile) {
  const sources = [
    ...
    { id: `source_${id}_oslo_current`, url: urls.osloKrist, sourceLocation: "nåværende Krist kirkegård og minnepark, kontrollert mot kommunens løpende stedsinformasjon", sourceType: "official", verifiedAt, temporalCoverage: "current", provenance: "Oslo kommunes løpende offisielle presentasjon av Krist kirkegård.", limitations: "Dokumenterer nåværende sted og forvaltning; historiske detaljer kontrolleres mot egne retrospektive kilder." }
  ];
  ...
  return {
    ...
    caseRealizations: [{
      ...
      comparativeScale: { localFinding: "Et lite kirkegårdsrom på Hammersborg gjør epidemihåndtering og minnekultur fysisk lesbar.", widerContext: "Caset kan sammenlignes regionalt med andre gravplasser og epidemispor i Christiania- og Akershusområdet, og med senere helsekriser der areal, begravelse og offentlig minne måtte organiseres.", scale: "regional", sourceIds: [sources[0].id, sources[1].id, sources[3].id] },
