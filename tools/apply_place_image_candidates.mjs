#!/usr/bin/env node
// Apply approved image candidates for History Go places.
// New structure only. The candidate file must contain sourceFile values under data/places/...
//
// Workflow:
//   1. node tools/build_place_image_candidates.mjs
//   2. Open data/places/place_image_candidates.json
//   3. Mark exactly one candidate per place with "approved": true
//   4. node tools/apply_place_image_candidates.mjs
//
// The script:
//   - downloads approved image files into bilder/places/auto/
//   - updates only missing image/cardImage by default
//   - stores imageMeta for attribution and later UI display
//
// Optional overwrite:
...
function assertNewSourceFile(sourceFile) {
  if (typeof sourceFile !== "string" || !sourceFile.startsWith("data/places/") || !sourceFile.includes("/oslo/")) {
