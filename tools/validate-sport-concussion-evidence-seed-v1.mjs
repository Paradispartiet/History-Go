...
const expectedRegistrationCommit = "bd774539101304f3e0f62333e9950678983152d3";
...
require(protocol.registration_anchor?.commit_sha === expectedRegistrationCommit, "protokollen bruker feil registreringscommit");
require(protocol.registration_anchor?.path === paths.protocol, "registreringsankeret peker til feil fil");
require(protocol.registration_anchor?.immutable_snapshot === true, "registreringsankeret er ikke uforanderlig");
require((protocol.scope?.subquestions || []).length === 3, "protokollen skal ha tre avgrensede delspørsmål");
require(protocol.screening_and_extraction?.independent_reviewers === 2, "protokollen krever ikke to uavhengige vurderere");
require(protocol.screening_and_extraction?.full_text_dual_screening === true, "protokollen krever ikke dobbel fulltekstscreening");
require(protocol.screening_and_extraction?.critical_fields_dual_extraction === true, "protokollen krever ikke dobbelt kritisk datauttrekk");
require(protocol.screening_and_extraction?.ai_may_assist_but_not_be_sole_reviewer === true, "KI er ikke blokkert som eneste vurderer");
...
require(protocol.publication_status?.startsWith("blocked_"), "protokollen blokkerer ikke tidlig publisering");
...
require(evidencePackage?.registration_anchor?.commit_sha === expectedRegistrationCommit, "pakken bruker feil registreringsanker");
require((evidencePackage?.candidate_record_ids || []).length >= 7, "pakken har for få kandidatposter");
...
const candidateIds = candidateRecords.map((item) => item.candidate_id);
...
require(new Set(candidateIds).size === candidateIds.length, "kandidat-ID-er er ikke unike");
require(candidateIds.every((id) => evidencePackage?.candidate_record_ids?.includes(id)), "pakkeregisteret mangler kandidat-ID");
for (const candidate of candidateRecords) {
  require(hasText(candidate.candidate_id), "kandidat mangler ID");
  require(hasText(candidate.title, 10), "kandidat mangler identifiserbar offisiell tittel", candidate.candidate_id);
  require(hasText(candidate.authors_short, 3), "kandidat mangler ansvarlig forfatter eller institusjon", candidate.candidate_id);
  require(Number.isInteger(candidate.year), "kandidat mangler år", candidate.candidate_id);
  require(hasText(candidate.source_type), "kandidat mangler kildetype", candidate.candidate_id);
  require(hasText(candidate.identifier), "kandidat mangler stabil identifikator", candidate.candidate_id);
  require(/^https:\/\//.test(candidate.url || ""), "kandidat mangler gyldig HTTPS-URL", candidate.candidate_id);
  require((candidate.subquestion_ids || []).length >= 1, "kandidat mangler delspørsmål", candidate.candidate_id);
  require(hasText(candidate.seed_role), "kandidat mangler søkefrørolle", candidate.candidate_id);
  require(hasText(candidate.retrieval_date), "kandidat mangler hentet dato", candidate.candidate_id);
  require(hasText(candidate.update_due), "kandidat mangler kontrollfrist", candidate.candidate_id);
...
require((searchLog.database_searches || []).every((item) => item.status === "not_run"), "databasesøk er feilaktig markert kjørt");
...
require(phasePackage?.registration_anchor?.commit_sha === expectedRegistrationCommit, "parent-protokollen bruker feil registreringsanker");
...
const report = {
  ...
  gates: {
    ...
    immutable_commit_anchor_present: protocol.registration_anchor?.commit_sha === expectedRegistrationCommit,
    candidate_provenance_complete: candidateRecords.every((item) => hasText(item.title, 10) && hasText(item.authors_short, 3) && hasText(item.identifier) && /^https:\/\//.test(item.url || "")),
