...
await writeImage(coning, "bilder/kort/productions/akershus_festning_beleiring_1716.webp", 900, 520);
...
Object.assign(place, {
  ...
  place_card_profile: { schema: "history_go_place_card_profile_v2", profile: "major", collection_ids: ["people", "objects", "brands", "productions"], category_collection_label: "Historiske hendelser", reason: "Historie-kontrakten eier avgrensede historiske hendelser som Productions. Beleiringen i 1716 har direkte stedsevidens, selvstendig hendelsesidentitet, kildebåret innhold og et rettighetsavklart kontekstbilde. De navngitte bygningsdelene beholdes som Structure-data, men er ikke kategoriuttrykket i PlaceCard.", verifiedAt },
  ...
  productions: [{ id: "akershus_beleiringen_1716", name: "Beleiringen av Akershus 1716", title: "Beleiringen av Akershus 1716", year: 1716, type: "historical_event", kind: "siege", desc: "Karl 12.s svenske styrker inntok Christiania, men klarte ikke å erobre Akershus. Festningen bandt svenske styrker til beleiringen fram til tilbaketrekningen i slutten av april.", image: "bilder/kort/productions/akershus_festning_beleiring_1716.webp", imageMeta: coningMeta, source_urls: [urls.siege, urls.invasion1716, urls.coning] }],
  structures: [
...
write(`data/places/production/${placeId}.json`, { schemaVersion: "4.2", validatorVersion: "4.2.1", placeId, placeFile, status: "ready_v4_2", identity: { status: "resolved", represents: "Akershus festning as the complete fortified state and defence complex on Akersneset.", period: "ca. 1300–", excludes: ["Akershus slott as a separate building identity","Forsvarsmuseet","Norges Hjemmefrontmuseum","individual named courtyards and microplaces","legacy typo id akerhus_slott"] }, claims: packetClaims, sentenceCoverage: { desc: descClaims.map((claim,index)=>({sentence:index+1,claimIds:[claim.id]})), popupDesc: popupClaims.map((claim,index)=>({sentence:index+1,claimIds:[claim.id]})) }, metadataSnapshot: { name: place.name, category: place.category, year: place.year, coordinates: { lat: place.lat, lon: place.lon } }, collections: { people: [personId], objects: ["akershus_retterstedet_minnesmerke"], brands: [brandId], productions: ["akershus_beleiringen_1716"] }, quizReadiness: { status: "canonical_major_8x7", quizTargetId: placeId, sourceBrief: briefFile, productionContext: contextFile, totalQuestions: 56, reuseDecision: "Legacy By-category 4x7 was replaced by a History major 8x7 progression; source-correct facts were retained only where they fit the History contract." }, roundsReadiness: { status: "ready", exactCollectionCount: 4 }, source_conflicts: [{ claim: "1299 is the exact construction year.", status: "rejected", reason: "SNL supports a likely start in 1299–1304, not one exact year." }, { claim: "Forsvarsbyggs downloadable logo can be reused freely.", status: "rejected", reason: "Forsvarsbygg restricts external logo use; the Brand preview instead uses an open-licensed photograph of authentic on-site signage for referential identification." }, { claim: "Structures is the History category expression at Akershus festning.", status: "rejected", reason: "The canonical History profile owns source-backed historical events as Productions. The fortress structures remain useful place data but do not replace that category expression." }], reviews: { factual: { status: "passed", reviewedAt: verifiedAt, reviewer: "Akershus source review", notes: "Official Forsvarsbygg material, SNL subject articles and inspectable Commons provenance cross-checked." }, editorial: { status: "passed", reviewedAt: verifiedAt, reviewer: "Akershus identity and memory review", introducedNewFacts: false, notes: "Castle, museums, microplaces, occupation site and memorial object remain distinct layers; the 1716 siege is an historical-event Production as required by the History collection contract; imprisonment and executions are not sensationalized." } }, completion: { completedUnder: "4.2", currentStatus: "current", sourceVerifiedAt: verifiedAt, claimsVerified: { verified: packetClaims.length, total: packetClaims.length }, factualReview: "passed", editorialReview: "passed", validatorVersion: "4.2.1" }, textHashes: { algorithm: "sha256", desc: sha256(desc), popupDesc: sha256(popupDesc) } });

write(`data/places/historie-production/${placeId}.json`, { schema: "history_go_historie_place_production_v1", place_id: placeId, status: "complete", verified_at: verifiedAt, source_registry: sourceRegistry, gates: { A_identity: "PASS", B_sources: "PASS", C_chronology: "PASS", D_people_objects_brands_productions: "PASS", E_story_language_reading: "PASS", F_quiz: "PASS", G_runtime: "PENDING_GENERATION", H_editorial: "PASS" }, chronology, collections: place.place_card_profile.collection_ids, stories: stories.map(s=>s.id), quiz: { size_class:"major_8x7", sets:8, questions:56 }, notes: ["Verified geometry is preserved unchanged.", "Akershus slott, Forsvarsmuseet, Norges Hjemmefrontmuseum and route microplaces remain separate identities.", "Brand visual uses authentic on-site signage under CC BY-SA 4.0, not the restricted official logo package.", "The 1716 siege is an historical-event Production under the canonical History collection contract; chronology and Story retain their separate temporal and narrative roles."] });
