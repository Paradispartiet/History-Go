...
Object.assign(place, {
  ...
  place_card_profile: { schema: "history_go_place_card_profile_v2", profile: "major", collection_ids: ["people", "objects", "brands", "structures"], category_collection_label: "Bygninger og festningsanlegg", reason: "Jomfrutårnet, Akershus slottskirke og Det kongelige mausoleum utgjør en stedsspesifikk, substansiell og bildeklar Structure-samling på tvers av middelalder, tidlig moderne tid og etterkrigstid. Beleiringen i 1716 er en historisk hendelse og hører hjemme i kronologi og Story, ikke som Production.", verifiedAt },
...
write(languageFile, { place_id: placeId, verified_at: verifiedAt, entries: [
...
write(`data/places/production/${placeId}.json`, { schemaVersion: "4.2", validatorVersion: "4.2.1", placeId, placeFile, status: "ready_v4_2", identity: { status: "resolved", represents: "Akershus festning as the complete fortified state and defence complex on Akersneset.", period: "ca. 1300–", excludes: ["Akershus slott as a separate building identity","Forsvarsmuseet","Norges Hjemmefrontmuseum","individual named courtyards and microplaces","legacy typo id akerhus_slott"] }, claims: packetClaims, sentenceCoverage: { desc: descClaims.map((claim,index)=>({sentence:index+1,claimIds:[claim.id]})), popupDesc: popupClaims.map((claim,index)=>({sentence:index+1,claimIds:[claim.id]})) }, metadataSnapshot: { name: place.name, category: place.category, year: place.year, coordinates: { lat: place.lat, lon: place.lon } }, collections: { people: [personId], objects: ["akershus_retterstedet_minnesmerke"], brands: [brandId], structures: ["akershus_jomfrutarnet", "akershus_slottskirke", "akershus_kongelige_mausoleum"] }, quizReadiness: { status: "canonical_major_8x7", quizTargetId: placeId, sourceBrief: briefFile, productionContext: contextFile, totalQuestions: 56, reuseDecision: "Legacy By-category 4x7 was replaced by a History major 8x7 progression; source-correct facts were retained only where they fit the History contract." }, roundsReadiness: { status: "ready", exactCollectionCount: 4 }, source_conflicts: [{ claim: "1299 is the exact construction year.", status: "rejected", reason: "SNL supports a likely start in 1299–1304, not one exact year." }, { claim: "Forsvarsbyggs downloadable logo can be reused freely.", status: "rejected", reason: "Forsvarsbygg restricts external logo use; the Brand preview instead uses an open-licensed photograph of authentic on-site signage for referential identification." }, { claim: "Beleiringen i 1716 er en Production.", status: "rejected", reason: "Beleiringen er en historisk hendelse og formidles i kronologi, Story og historieinnhold; den er ikke et verk eller en produksjon." }], reviews: { factual: { status: "passed", reviewedAt: verifiedAt, reviewer: "Akershus source review", notes: "Official Forsvarsbygg material, SNL subject articles and inspectable Commons provenance cross-checked." }, editorial: { status: "passed", reviewedAt: verifiedAt, reviewer: "Akershus identity and memory review", introducedNewFacts: false, notes: "Castle, museums, microplaces, occupation site and memorial object remain distinct layers; the 1716 siege remains a historical event rather than a Production; imprisonment and executions are not sensationalized." } }, completion: { completedUnder: "4.2", currentStatus: "current", sourceVerifiedAt: verifiedAt, claimsVerified: { verified: packetClaims.length, total: packetClaims.length }, factualReview: "passed", editorialReview: "passed", validatorVersion: "4.2.1" }, textHashes: { algorithm: "sha256", desc: sha256(desc), popupDesc: sha256(popupDesc) } });
