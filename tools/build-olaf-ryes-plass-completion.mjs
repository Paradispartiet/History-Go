...
function buildContent() {
  ...
  Object.assign(place, {
    ...
    frontImage: "bilder/places/olaf_ryes_plass_front_portrait.webp",
    frontImageMeta: { source: "wikimedia_commons", sourcePage: sourceUrls.after, creator: "Helge Høifødt", credit: "Helge Høifødt / Wikimedia Commons", license: "CC BY-SA 3.0", licenseUrl: "https://creativecommons.org/licenses/by-sa/3.0/", sourceDimensions: "3872x2592", outputDimensions: "900x1200", orientation: "portrait", aspectRatio: "3:4", transformation: "Sentrert stående utsnitt av originalfotografiet; ingen innholdsgenerering.", verified: true, verifiedAt: "2026-08-26" },
...
Parkteatret ligger ved Olaf Ryes plass 11, men bygningen og virksomheten er ikke en del av den canonicale plassflaten. Parkteatrets egen presentasjon daterer kinohistorien til 1907 og beskriver dagens bruk som konsertsted. Venue-identiteten vises som en kildebelagt Brand ved plassen, mens bygningen og institusjonshistorien forblir nabokontekst.
...
function buildQuiz(place, production) {
  ...
  const held_back_candidates = ["Parkteatrets bygning og fulle institusjonshistorie som park-eid innhold; bare venue-identiteten materialiseres i Brands.", "Eksakt optisk før/etter-påstand for 1903/2009.", "Generalisering fra daterte arrangementer til permanent bruk.", "Den feilaktige 1848-formuleringen om Olaf Ryes død."];
  ...
  const conceptByEmne = {
    [E1]: { label: "offentlig park", id: "co_by_offentlig_park_4d9043d740" },
    [E2]: { label: "opphold og gjennomgang", id: "co_by_opphold_og_gjennomgang_38f317fab0" },
    [E3]: { label: "historiske lag", id: "co_by_historiske_lag_b5eb5eb432" }
  };
  const questions = specs.map((row, index) => {
    ...
    const concept = conceptByEmne[emne];
...
function buildIntegration() {
  ...
  place.place_card_profile = { schema: "history_go_place_card_profile_v2", collection_ids: ["people","objects","brands","related"], reason: "Olaf Ryes plass er et vanlig bysted. People og Objects har direkte stedsevidens, Related viser separate canonicale nabosteder, og Brands viser Parkteatrets dokumenterte venue-identitet ved adressen Olaf Ryes plass 11 uten å overføre byggets historie til parkflaten. Alle fire samlinger har et bildeklart canonical medlem.", verifiedAt: "2026-08-26" };
  place.related_people_ids = ["olaf_rye","eilert_sundt"];
  place.related_place_ids = ["sofienbergparken","birkelunden","markveien","daelenenga_idrettspark"];
  place.objects = [
    { id: "olaf_ryes_plass_eilert_sundt_bust", title: "Eilert Sundt-bysten", type: "bronsebyste", kind: "physical_object", desc: "Mathias Skeibroks bronsebyste av samfunnsforskeren Eilert Sundt ble reist i sørenden av plassen i 1892.", why_here: "Bysten gjør forbindelsen mellom folkeopplysning, samfunnsforskning og det offentlige parkrommet fysisk lesbar.", placeSpecificReason: "Oslo byleksikon plasserer og daterer bysten eksplisitt på Olaf Ryes plass.", historicalFunction: "Offentlig minnespor finansiert av Oslo Arbeidersamfund og Selskabet for Folkeoplysningens Fremme.", physicalObject: true, placeSpecific: true, collectable: true, storePrice: 30, currency: "PC", collection: "olaf_ryes_plass_minnespor", unlock: "Finn bysten i sørenden og les navnet før du åpner Eilert Sundt-kortet.", image: "bilder/kort/objects/olaf_ryes_plass_eilert_sundt_bust.webp", imageMeta: { sourcePage: "https://commons.wikimedia.org/wiki/File:Olaf_Ryes_plass_bysten_av_Eilert_Sundt.jpg", creator: "Helge Høifødt", credit: "Helge Høifødt / Wikimedia Commons", license: "CC BY-SA 4.0", licenseUrl: "https://creativecommons.org/licenses/by-sa/4.0/", depictedObject: "Mathias Skeibroks Eilert Sundt-byste på Olaf Ryes plass", transformation: "Stedstro utsnitt, skalering og WebP-normalisering.", verifiedAt: "2026-08-26" }, source_urls: [sourceUrls.byleksikon,sourceUrls.sundt] },
    { id: "olaf_ryes_plass_fontene", title: "Fontenen", type: "fontene", kind: "physical_object", desc: "Fontenen ble anlagt midt på Olaf Ryes plass i 1927 og er fortsatt et fast orienteringspunkt i parkrommet.", why_here: "Fontenen viser at parken fikk nye fysiske lag også etter opparbeidelsen i 1890.", placeSpecificReason: "Oslo byleksikon daterer fontenen, og Oslo kommune lister den som anlegg på selve plassen.", historicalFunction: "Sentralt park- og oppholdselement fra mellomkrigstiden.", physicalObject: true, placeSpecific: true, collectable: true, storePrice: 25, currency: "PC", collection: "olaf_ryes_plass_parkstruktur", unlock: "Finn fontenen og bruk den som anker når du sammenligner ganglinjer og oppholdssoner.", image: "bilder/kort/objects/olaf_ryes_plass_fontene.webp", imageMeta: { sourcePage: "https://commons.wikimedia.org/wiki/File:764_Oslo._Font%C3%A6nen_Olaf_Ryes_plass_-_no-nb_digifoto_20151109_00068_bldsa_PK08110.jpg", creator: "Ukjent fotograf / Nasjonalbiblioteket", credit: "Nasjonalbiblioteket / Wikimedia Commons", license: "Public domain (PD-Norway)", depictedObject: "Fontenen på Olaf Ryes plass", transformation: "Stedstro utsnitt, skalering og WebP-normalisering.", verifiedAt: "2026-08-26" }, source_urls: [sourceUrls.byleksikon,sourceUrls.municipality] }
  ];
  ...
  const brands = read("data/brands/brands_by_place.json"); brands.olaf_ryes_plass = ["parkteatret"]; write("data/brands/brands_by_place.json",brands);
  const brandMasterFile = "data/brands/brands_master.json";
  const brandMaster = read(brandMasterFile);
  const parkteatret = brandMaster.find(brand => brand.id === "parkteatret");
  Object.assign(parkteatret, { status:"active",verification:"verified",verified_at:"2026-08-26",popupdesc:"Parkteatret er en selvstendig venue-identitet ved Olaf Ryes plass 11. Kinohistorien går tilbake til 1907, navnet Parkteatret kom i 1922, og virksomheten drives i dag som bar og konsertscene. Brand-koblingen gjelder den dokumenterte venue-identiteten ved plassen; bygningen og den fulle institusjonshistorien overtas ikke av parkflaten.",desc:"Venue-brand for bar og konsertscene ved Olaf Ryes plass 11.",tags:["brand","venue","music","grunerlokka","olaf_ryes_plass"],place_ids:["olaf_ryes_plass"],source_urls:["https://www.parkteatret.no/om-parkteateret","https://www.parkteatret.no/kontakt-oss","https://www.parkteatret.no/"],logo:"bilder/kort/brands/parkteatret.webp",imageMeta:{sourcePage:"https://www.parkteatret.no/",sourceSelector:"header a[href='/'] svg",creator:"Parkteatret",credit:"Parkteatret",rightsBasis:"official_brand_site_referential_identification",reviewStatus:"manually_approved",assetKind:"official_logo",sourceForm:"official_inline_svg_logo",temporalScope:"current",usageContext:"referential_identification",noEndorsement:true,generated:false,reconstructed:false,transformation:"Uendrede offisielle SVG-banepunkter fra nettstedets header, sentrert på en nøytral 900 × 520-flate og WebP-normalisert.",reviewedAt:"2026-08-26"}});
  write(brandMasterFile,brandMaster);
  ...
  Object.assign(sundt,{image:"bilder/kort/people/eilert_sundt.webp",cardImage:"bilder/kort/people/eilert_sundt.webp",imageMeta:{source:"wikimedia_commons",sourcePage:"https://commons.wikimedia.org/wiki/File:Eilert_Sundt_OB.F03353C.jpg",creator:"Frederik Klem",credit:"Oslo Museum / Wikimedia Commons",license:"Public domain",mediaType:"historic_portrait",transformation:"Skalering og WebP-normalisering.",verifiedAt:"2026-08-26"}});
  ...
  production.roundsReadiness = { status:"production_ready",reviewedAt:"2026-08-26",auditFile:"reports/place-production/olaf-ryes-plass-phase20-24-gate-audit-v1.json",badgePlacement:"separate_header",contentRoundIds:place.place_card_profile.collection_ids,placeCardProfile:place.place_card_profile.schema,peopleIds:place.related_people_ids,objectIds:place.objects.map(item=>item.id),brandIds:["parkteatret"],previewImageCoveragePercent:100,frontImageOrientation:"portrait",relatedPlaceIds:place.related_place_ids,objectSourceCoveragePercent:100,routeStopResolutionPercent:100 };
  production.reviewsNotes = "Full phase 0–24 production: exact two-person target, two sourced Objects, Parkteatret as a logo-verified venue Brand, four related canonical places, 100% collection-preview coverage, portrait frontImage, rich 5x7 quiz/Knowledge, safe onsite tasks and final gate evidence.";
  write("data/places/production/olaf_ryes_plass.json",production);
