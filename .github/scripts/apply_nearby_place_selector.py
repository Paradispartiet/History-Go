...
'''
    selector_decl = '''    HGNearbyPlaceSelector?: {
      select?: () => {
        items: Array<any & {
          _d: number | null;
...
'''
    if filters_end not in globals:
        raise SystemExit("HGNearbyFilters declaration anchor missing")
    globals = globals.replace(filters_end, filters_end + selector_decl, 1)
globals_path.write_text(globals)

lists_path = Path("js/ui/lists.js")
lists = lists_path.read_text()

start_marker = '  const PLACES = window.PLACES || [];\n'
end_marker = '  const renderSignature = JSON.stringify({\n'
start = lists.find(start_marker, lists.find('function renderNearbyPlaces()'))
end = lists.find(end_marker, start)
if start < 0 or end < 0:
