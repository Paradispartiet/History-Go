...
Disse story-lignende JSON-filene finnes i `data/stories/`, men er ikke registrert i `data/stories/stories_manifest.json` og teller derfor ikke i aktiv coverage. Manifest- og støttefiler som `stories_manifest.json`, `stories_generation_manifest.json`, grafdata, kandidatdata og legacy people/place-dumper er holdt utenfor listen.

| Story file | Stories | place_id-er | Teller? |
|---|---:|---|---|
| `data/stories/oslo_density.json` | 6 | `youngstorget`, `barcode`, `bjorvika`, `stortinget`, `grand_cafe`, `ekely` | Nei |
| `data/stories/oslo_potential.json` | 6 | `youngstorget`, `barcode`, `bjorvika` | Nei |
| `data/stories/places_by.json` | 3 |  | Nei |
| `data/stories/stories_by.json` | 1 |  | Nei |
| `data/stories/stories_gamlebyen.json` | 1 | `gamlebyen` | Nei |

## Places med stories

| place_id | Navn | Kategori | Story title(s) | Story file(s) |
|---|---|---|---|---|
...
## Prioriterte anbefalinger for neste storybatch

- Prioriter kategorier med lav dekning og mange aktive places uten story, særlig `by`, `musikk`, `naeringsliv`, `subkultur`, `litteratur`, `natur` og `sport`.
- Hold neste PR analyse- eller data-endringsmessig avgrenset: ikke bland coverage-rapport, storyregistrering og datakorrigering i samme endring.
- Ikke registrer `stories_gamlebyen.json` før `gamlebyen` eventuelt igjen er en aktiv place-ID eller storyen flyttes til en aktiv place-ID i en egen datakorrigerings-PR.
