...
285  ui/CivicationDayPhaseUI.js          (eget fasepanel + Gå til neste fase)
...
| **DayPhaseUI** | `ui/CivicationDayPhaseUI.js` (`CivicationDayPhaseUI`) | Eget panel (`#civiDayPhasePanel`): gjeldende fase, åpne items, "Gå til neste fase"-knapp, outcome/læring/reentry-bannere | leser `CivicationDayProgression.inspect()` | lytter `civi:dayPhaseChanged/inboxChanged/booted/updateProfile` | Nei | **Ja** (knapp → `advancePhaseIfReady`) | **Ja** (eget panel) | Andre fasevisning, separat fra WorkdayPanel |
