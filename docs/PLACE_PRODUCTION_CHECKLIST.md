...
DIALEKTLAG — KUN `placeScope: "area"` / N/A. Dialektinnhold kan kun eies av et område-Place med `placeScope: "area"`; enkeltsteder kan ha Språkleksikon, men skal ikke eie dialektinnhold. `coordRole` beskriver koordinatgeometri og avgjør ikke dialekteierskap. Når Språkleksikon produseres, skal relevante dialektord og lokale uttrykk vurderes og kildebelegges etter språk-/dialektkontrakten.
