...
| 188 | `sagene_kvernhus` | retired duplicate → `glads_molle` | retired_duplicate | `geonorge-adresser-v1:0301:16161:10A` |

Batch 188 (2026-07-23) pensjonerer `sagene_kvernhus` som en duplikatproxy for den allerede canonical og verifiserte `glads_molle` (Glads mølle). Collision-gaten dokumenterte at begge identitetene løste til samme fysiske bygg og samme eksakte Geonorge-adresseobjekt i Sandakerveien 10A. Reelle aktive relasjoner flyttes til `glads_molle`; separat næringslivs-place, coordinate evidence, Civication-mapping, i18n-duplikater og duplikatquiz fjernes. `sagene_kvernhus` registreres i den etablerte legacy place-id alias-guarden slik at ID-en ikke kan gjeninnføres i aktive place-/quiz-/Civication-data. Ingen ny fysisk koordinat opprettes og verified-totalen økes ikke.
