...
`sport/supporter`, `by/nabolagskjenner`, `film_tv/filmklubbmenneske` og `filosofi/sofafilosof` er source-backed `ready`-posisjoner og er nå alle ferdigstilt som Role Worlds. Readiness-auditen bruker tre separate klasser — `ready`, `needs_authored_depth` og `not_a_standalone_world` — mens faktisk produksjonsstatus ligger separat i `role_world_status`. Fasit er nå **4 ready = 4 role_world_complete + 0 pending**, **156 needs_authored_depth** og **40 not_a_standalone_world**. Det finnes derfor ingen ny uferdig `ready`-kandidat før mer authored dybde er produsert.
